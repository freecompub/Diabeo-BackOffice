...
interface SettingsClientProps {
  /** Rôle injecté par le Server Component wrapper (header middleware `x-user-role`). */
  role: Role
}

/** Sections visibles selon le rôle : VIEWER = patient (tout), PS = sans patient-only. */
function visibleSectionsFor(role: Role): SectionMeta[] {
  if (role === "VIEWER") return SECTIONS
  return SECTIONS.filter((s) => !PATIENT_ONLY_SECTIONS.has(s.id))
}
...
export function SettingsClient({ role }: SettingsClientProps) {
  ...
  // #475 §7 — sections filtrées par rôle (patient-only masquées aux PS).
  const sections = React.useMemo(() => visibleSectionsFor(role), [role])

  // `personalInfo` est visible pour tous les rôles → init sûre.
