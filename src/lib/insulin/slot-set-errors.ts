...
export const SLOT_SET_ERROR_STATUS: Record<string, number> = {
  ...
  // US-2663 (S3e) — voie groupée BASALE STYLO (`replaceStyloBasalSet`) : patient POMPE — la config LIVE n'est pas
  // un stylo (jamais des U totales sur une config pompe → 409 récupérable) ; cardinalité incohérente avec le
  // `configType` LIVE (single↔split basculé entre génération et acceptation → 409, régénérer) ; CAS atomique en
  // base non satisfait — dose dérivée sous nous (voie par-valeur US-2660) ou effacée → 0 ligne (409, régénérer).
