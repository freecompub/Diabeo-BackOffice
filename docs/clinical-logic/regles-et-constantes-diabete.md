...
| **Bascule MOTEUR → émission GROUPÉE ISF/ICR (US-2663 S3b-1, derrière flag)** | Le générateur de propositions (`proposal-generator.service.ts`) émet, sous le flag `ENGINE_GROUPED_ISF_ICR` (env, **OFF par défaut, réversible**, un seul flag ISF **et** ICR), **une** `SlotSetProposal` `source: "algorithm"` par levier (disposition ENTIÈRE) **au lieu de** N `AdjustmentProposal` par-valeur. Assemblage (`emitGroupedIsfIcr`) : (1) **relecture LIVE** de la config une fois (T1, même source que `captureBaselineSlots`) = base de la disposition ; (2) **CAS par créneau CHANGÉ (R2)** — un changement n'est retenu QUE si la base analysée (`candidate.currentValue`, snapshot T0) == la valeur live (tolérance `1e-9`, bruit float `Decimal`) ; un créneau **dérivé** depuis l'analyse (édition médecin, autre acceptation) est **ABANDONNÉ** — jamais une magnitude périmée injectée. Réplique le `baselineMovedAtPersist` par-créneau de `createEngineProposal`, perdu au regroupement (le CAS d'ensemble de l'acceptation ne couvre que génération→accept, pas analyse→génération) ; fenêtre résiduelle (relecture → snapshot `baselineSlots`, même tick cron) bornée par le CAS d'ensemble S1, identique à la voie patient déjà livrée ; (3) **no-op (R4)** — aucune émission si 0 créneau ne change (0 candidat OU tous dérivés) ; (4) **`mealLabel` ICR préservé (R5)** ; (5) **rationale MOTEUR (R3)** — une entrée `SlotRationale` par créneau changé (motif/confiance/volume/moyenne/période en jours entiers), requise par `createSetProposal` pour `source: "algorithm"`. Décisions cliniques (analyzeurs, deadband, dé-escalade, cooldown, flags de haute variabilité) **INCHANGÉES** — seule la voie d'écriture change. Rejets `createSetProposal` fail-closed → **non fatals** (log info/erreur, run continue), parité par-valeur. Supersession par **classe d'origine** (D2) : la proposition moteur coexiste avec une demande humaine. |
