...
| **Exclusion mutuelle grouped ⇄ par-valeur (US-2663 S2b)** | À travers **les DEUX modèles** de proposition, il n'y a **qu'une file pending par (patient × paramètre)** : créer une proposition **d'ensemble** (`createSetProposal`) supersède les `AdjustmentProposal` **par-valeur** pending du paramètre (déjà en place), et **symétriquement** créer une proposition **par-valeur** (`createProposal` humain ET `createEngineProposal` moteur) supersède les `SlotSetProposal` **d'ensemble** pending du paramètre. Évite qu'une disposition groupée et un ajustement par-valeur du même paramètre coexistent (confusion médecin) ; cohérent avec la trajectoire **grouped-only**. Supersession programmatique (`reviewedByUserId: null`), hors ISF/ICR = no-op (pas de `SlotSetProposal`). Fail-safe complémentaire : le CAS d'ensemble S1 (`baselineMoved`) bloque de toute façon l'acceptation d'un jeu périmé. |
