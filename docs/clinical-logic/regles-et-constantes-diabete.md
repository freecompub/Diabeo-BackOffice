...
| **Exclusion mutuelle grouped ⇄ par-valeur (US-2663 S2b)** | À travers **les DEUX modèles** de proposition, **une seule file pending par (patient × paramètre)** : créer une proposition **d'ensemble** (`createSetProposal`) supersède les `AdjustmentProposal` **par-valeur** pending du paramètre, et **symétriquement** créer une proposition **par-valeur** — par **les 3 primitives** : `createProposal` (humain), `createEngineProposal` (moteur), `createManual` (DOCTOR) — supersède les `SlotSetProposal` **d'ensemble** pending du paramètre (helper unique `supersedeGroupedPending`). Évite qu'une disposition groupée et un ajustement par-valeur du même paramètre coexistent (confusion médecin) ; cohérent **grouped-only**. Supersession **programmatique** (`reviewedByUserId: null`) ; **compteur `supersededGroupedCount`** tracé dans l'audit `CREATE` (audit-readiness) ; hors ISF/ICR = no-op. Le **moteur supersède aussi une soumission groupée PATIENT** (symétrie **validée medical** : l'asymétrie ré-introduirait la coexistence contradictoire ; la demande patient est préservée en statut terminal `superseded`, à surfacer sur l'écran de revue — suivi UI). Fail-safe complémentaire : le CAS d'ensemble S1 (`baselineMoved`) bloque de toute façon l'acceptation d'un jeu périmé. |
