...
Source (serveur — grouped-only depuis S5) : `proposalGeneratorService` (moteur algorithme → `slotSetProposalService.createSetProposal`) ; routes humaines groupées `POST /api/slot-set-proposals` (PRO) + `PUT /api/patient/insulin-slot-set` (patient) ; écriture directe DOCTOR via `PUT /api/insulin-therapy/{sensitivity-factors,carb-ratios,basal-config/pump-slots}` (`replaceSlotSet`/`replacePumpSlotSet`) · ADR #13.
...
| **RBAC édition / proposition (US-2648a, S5 grouped-only)** | Écriture **directe** de la config insuline = **DOCTOR** (autorité clinique) via `PUT /api/insulin-therapy/*` (`replaceSlotSet`/`replacePumpSlotSet`). NURSE / patient → **proposition GROUPÉE** (`SlotSetProposal`, jeu entier) via `POST /api/slot-set-proposals` (PRO) + `PUT /api/patient/insulin-slot-set` (patient), validée par un médecin. ADMIN rejeté. Rôle proposeur dérivé de la **session** ; accès via `resolvePatientId` (VIEWER→son dossier / pro→`canAccessPatient`) ; réponse sans `proposerComment`. Routes : `src/app/api/insulin-therapy/*` (DOCTOR) + `src/app/api/slot-set-proposals` (POST NURSE/DOCTOR) + `src/app/api/patient/insulin-slot-set` (PUT patient). **ADMIN** : rejeté à la *proposition* (pas d'identité clinique) mais conserve l'*écriture directe* — bypass PHI V1 assumé (`access-control.ts`, levé V4/F1). |
| **Front grouped-only (US-2657, migration front — S5 complete)** | Suite au retrait serveur des écritures par-créneau, l'édition DOCTOR directe côté fiche patient passe **exclusivement** par les éditeurs de GROUPE : `InsulinSlotSetDialog` (ISF/ICR, heure entière `[0,23]`, US-2656) et `InsulinBasalSlotSetDialog` (basal, temps `"HH:MM"` minute-précis + `<input type="time">`, US-2657) — les deux `PUT` le jeu **complet**. `InsulinDirectEditDialog` (édition par-créneau `PATCH`, US-2648b) est **retiré**. Le bouton « Proposer » par-créneau et le composant `InsulinProposalDialog` (NURSE/patient, `POST /api/adjustment-proposals`) sont **retirés en S5** : la proposition humaine passe désormais **exclusivement par les éditeurs de GROUPE** (création `SlotSetProposal` groupée via `POST /api/slot-set-proposals` PRO ou `PUT /api/patient/insulin-slot-set` patient). Logique pure partagée : `insulin-slot-set-edit.ts` (ISF/ICR **et** basal — deux jeux de fonctions parallèles, modèle de créneau non superposable). La page réglages autonome du médecin (`/insulin-therapy`) mute l'état ISF/ICR localement à l'ajout/édition et envoie UN `PUT` par paramètre **réellement modifié** à l'enregistrement (pas de `PUT` sur un paramètre non touché — évite un rejet `emptySlotSet` sur un patient sans ISF/ICR configuré). |
| **Disposition groupée + snapshot de base + CAS d'ensemble (US-2663 épic, S0+S1+S2+S4+S5 LIVRÉS)** | Toute proposition d'ajustement — quelle que soit l'origine (patient/infirmière/médecin/**algorithme**), même si un seul créneau change — porte la **disposition ENTIÈRE** du levier (jeu de créneaux complet). Modèle unique `SlotSetProposal` (généralisé) ; typage cible = **union discriminée par levier** (`src/lib/insulin/grouped-proposal.ts` : ISF/ICR `startHour`·g/L·U ou g/U ; basale pompe `startTime`·U/h ; basale stylo `basalDoseKind`·U totales ; dose fixe `moment`·U). **`baselineSlots`** = snapshot de la base **PAR CRÉNEAU à la génération** (`captureBaselineSlots`, même encodage que `proposedSlots`). `source` dérivé **serveur** (ADR #27, jamais du body). **Garde-fou MDR — CAS d'ensemble fail-closed (S1, LIVRÉ)** : à l'acceptation, **sous verrou** `tryLockInsulinSlots` (lecture LIVE atomique, pas de TOCTOU), la base actuelle DOIT être identique au snapshot (`assertBaselineUnchanged`, `src/lib/insulin/slot-baseline-cas.ts` ; appariement par clé `startHour`, `mealLabel` non dosant **ignoré**). Toute dérive (valeur/borne/structure) → **`baselineMoved`** (409, rollback → reste `pending`, à régénérer) : n'écrase jamais un ajustement MÉDECIN concurrent (ex. baisse post-hypo). Snapshot `null` (legacy pré-S0) → **`baselineMissing`** (409, fail-closed : jamais d'apply sur une base non certifiable). Sémantique S1 = **rejet d'ensemble** (toute divergence rejette la disposition entière) ; diff-merge valeur-seule différé (S1bis). Chemin **DOCTOR direct** (`replaceSlotSet` sans `expectedBaseline`) = pas de CAS (écrasement explicite assumé). **Revue médecin (S2, LIVRÉ)** : `SlotSetProposal` **PENDING** surfacée sur `/patients/[id]/review` (`GroupedProposalReview`), jusqu'ici invisible à la revue (seules les `AdjustmentProposal` par-valeur y apparaissaient). `slotSetProposalService.listPendingForReview` expose `baselineSlots` (contrairement à `listSetProposals`, qui l'omet pour la liste patient — minimisation) : usage DOCTOR-gated légitime pour le decision-support. Diff (créneau PROPOSÉ vs valeur LIVE actuelle) pré-calculé **serveur** (`src/lib/insulin/slot-diff.ts`, `diffSlots`/`hasStructuralChange`) ; lignes divergentes surlignées à l'affichage. Variante NON-throwing du CAS (`isBaselineUnchanged`, `slot-baseline-cas.ts`) affiche un bandeau d'AVERTISSEMENT si la base a dérivé depuis la génération — **non bloquant à l'affichage** (le blocage réel reste le 409 `baselineMoved`/`baselineMissing` à l'acceptation, ci-dessus) : prévient le médecin AVANT une tentative d'acceptation vouée à l'échec. **S4 voie manuelle groupée (LIVRÉ)** : création humaine `SlotSetProposal` groupée (PRO + patient) + garde clinique patient. **S5 retrait par-valeur (LIVRÉ)** : routes et modèle `AdjustmentProposal` par-créneau supprimés (lecture seule conservée). |
| **Émission GROUPÉE ISF/ICR — grouped-only (US-2663 S3b-1 → S5)** | Le générateur de propositions (`proposal-generator.service.ts`) émet **UNE** `SlotSetProposal` `source: "algorithm"` par levier (disposition ENTIÈRE) — **grouped-only, unique voie depuis S5** (les flags `ENGINE_GROUPED_ISF_ICR` sont **supprimés**). Assemblage : `emitGroupedIsfIcr` (IO) relit la config LIVE une fois (T1) puis délègue à `assembleGroupedDisposition` (**cœur PUR, unit-testable**). Gardes PAR CRÉNEAU (parité de sûreté) : (1) **CAS R2** — un changement n'est retenu QUE si le créneau live est INCHANGÉ depuis l'analyse : **même `endHour`** (fenêtre horaire non restructurée) ET valeur live == base analysée (`candidate.currentValue`, snapshot T0 ; tolérance `1e-9`). Sinon **ABANDONNÉ** — jamais une magnitude/fenêtre périmée injectée (réplique `baselineMovedAtPersist`, perdu au regroupement) ; (2) **garde direction** — le sens du delta doit être cohérent avec le `reason` (`*TooLow`⇒hausse, `*TooHigh`⇒baisse) ; incohérence (défaut d'analyseur) → créneau abandonné + logué `reasonDirectionMismatch` (jamais une rationale trompeuse) ; (3) **no-op (R4)** — aucune émission si 0 créneau retenu ; (4) **`mealLabel` ICR préservé (R5)** ; (5) **rationale MOTEUR (R3)** — 1 `SlotRationale` par créneau changé (motif/confiance/volume/moyenne/période jours entiers), requise par `createSetProposal` pour `source: "algorithm"`. **Fenêtre TOCTOU FERMÉE** : la relecture LIVE (T1) sert À LA FOIS de base d'overlay ET de `baselineSlots` **injecté** à `createSetProposal` (param `baselineOverride` — pas de 2ᵉ lecture) ⇒ disposition et snapshot de base partagent UN instant ⇒ le CAS d'ensemble de l'acceptation (S1) couvre TOUS les créneaux, y compris ceux inchangés (une édition médecin d'un créneau non touché lève `baselineMoved` au lieu d'être revertée). Décisions cliniques (analyzeurs, deadband, dé-escalade, cooldown, flags haute-variabilité) **INCHANGÉES** — seule la voie d'écriture change. Rejets `createSetProposal` fail-closed → **non fatals**. ⚠️ **Parité imparfaite (D1)** : un créneau MOTEUR hors borne n'est pas rejeté — la valeur est **plafonnée** à la borne (D1, `clampProposed`) avec justification, et le médecin tranche à la revue. Supersession par **classe d'origine** (D2) : la proposition moteur coexiste avec une demande humaine (jamais supersession inter-classe). |

| **Émission GROUPÉE basale POMPE — grouped-only (US-2663 S3c → S5)** | Le pipeline groupé (modèle, CAS d'ensemble, apply à l'acceptation, moteur, UI de revue) est étendu à la **basale POMPE** (`basalRate`, créneaux `PumpBasalSlot` : bornes `"HH:MM"` minute-précis, débit U/h). **Socle mutualisé** : (a) CAS d'ensemble **générique par clé** (`assertBaselineUnchangedBy` — pompe apparie par `startTime`, borne `endTime`, valeur `rate`) ; (b) `createSetProposal` migré en **objet d'options** (`SlotSetParam` élargi à `basalRate` ; `parseSlots`/`captureBaselineSlots`/validation polymorphes) ; (c) `replacePumpSlotSet` **rétrofité** — param `cas` (CAS d'ensemble sous verrou avant écriture) + supersession des `SlotSetProposal` `basalRate` pending (l'invariant « aucune SlotSetProposal basale » **tombe** — anti dérive de base : un jeu pompe périmé n'écrase jamais un ajustement médecin récent). **Discriminateur pompe⇄stylo (CRITIQUE)** : l'apply route sur le `configType` **LIVE** (`basalConfigNotPump` fail-closed) — jamais des U/h écrits sur une dose stylo (U totales). **Moteur** : grouped-only depuis S5 (flag `ENGINE_GROUPED_PUMP` **supprimé**). Le générateur ne titre que le créneau **nocturne** mais émet la disposition **entière** (tous les créneaux pompe, débit nocturne changé superposé) via `assembleGroupedPumpDisposition` (pur) + `emitGroupedPumpBasal` — appariement candidat⇄live par `id`, CAS débit, garde direction, **temps `"HH:MM"` EXACTS préservés** (jamais réécrits à l'heure entière), baseline injecté (TOCTOU fermée). **UI de revue** : `diffPumpSlots` (par `startTime`, `timeLabel` exact), unité U/h (`PARAM_UNIT_KEY.basalRate`). **Parité imparfaite (D1)** : un créneau hors-borne est plafonné (voir D1, pas de rejet du levier entier). |
| **Dose fixe groupée — identification PAR USAGE + apply fail-closed (US-2663 S3d → S5)** | Mode « doses simples » (`fixedDose`). **Règle de clé (décision produit)** : une dose fixe est identifiée par **`(usage, moment)`**, PAS par `moment` seul — un même `moment` (ex. « soir ») peut porter une dose **BOLUS** (rapide) ET une dose **BASALE** (lente) sur deux `PatientInsulin` distincts (`InsulinUsage` ∈ `bolus`/`basal`/`both`). Écrire par `moment` seul ciblerait la mauvaise insuline. **Apply** (`replaceFixedDoseSet`) : **update-in-place par `id` RÉSOLU SERVEUR** par `(usage, moment)` (les `FixedDoseSlot` pendent de `PatientInsulin` distincts — pas de delete+create) ; **résolution fail-closed 0/1/>1** : 0 → `fixedDoseSlotNotFound` (dose supprimée depuis la génération, 409), **>1 → `fixedDoseSlotAmbiguous`** (deux insulines de même usage/moment → désambiguïser, 422), 1 → écriture. **Bornes** (`assertValidFixedDoseSet`) : plancher `FIXED_DOSE_MIN` 0,5 U, délivrabilité (multiple d'incrément), clé `(usage, moment)` unique, **pas de plafond dur** (WARN `FIXED_BOLUS_WARN_U` 25 / `FIXED_BASAL_WARN_U` 80 non bloquants), garde anti-overflow `≤ 999,99` (colonne `Decimal(5,2)`). CAS d'ensemble générique (clé `(usage, moment)`, sans borne). **Filtre insuline ACTIVE** (`active-insulin.ts`, `isActive` + `endDate`) sur TOUS les lecteurs/écriveurs de doses fixes (grouped + générateur) : jamais de dose fantôme ni de mésécriture sur une insuline discontinuée. **Correction d'un bug pré-existant** : l'apply `fixedDose` usage-blind pouvait toucher **plusieurs lignes silencieusement** → désormais `count===1` requis (0→notFound, >1→`fixedDoseSlotAmbiguous`). **Moteur** : grouped-only depuis S5 (flag `ENGINE_GROUPED_FIXED_DOSE` **supprimé**). Générateur usage-aware (`select += patientInsulin.usage`, collecte par `(usage, moment)`) → `assembleGroupedFixedDose` (pur) + `emitGroupedFixedDose` (baseline injecté, TOCTOU fermée). **D2 — ABANDON de la titration d'un moment MULTI-DOSES** (≥ 2 usages au même moment) : le signal glycémique (creux pré-dose) est par MOMENT, non attribuable à un usage → aucune proposition sur ce moment (log info, laissé au médecin) — reco `medical-domain-validator`. **UI de revue** : `diffFixedDoseSlots` (clé `(usage, moment)`, libellé « Bolus · Matin » traduit), unité U, badge d'avertissement `highDoseWarning` dérivé SERVEUR (bolus > `FIXED_BOLUS_WARN_U` 25 / basal **et `both`** > `FIXED_BASAL_WARN_U` 80, NON bloquant ; `both` = prémix bolus+basale rattaché au seuil basal — décision à confirmer medical). **Garde-fou hypo** : l'abandon D2 d'un candidat de **BAISSE** (dé-escalade hypo) sur un moment multi-doses **lève un `ClinicalReviewFlag`** (jamais une hypo silencieuse) ; une hausse abandonnée reste log-only. **Parité imparfaite (D1)** : un créneau hors-borne est plafonné (voir D1). |

| **Basale STYLO (MDI) groupée — apply fail-closed (US-2663 S3e PR1 → S5)** | 4ᵉ et dernier levier du pipeline groupé. La basale STYLO réutilise `parameterType = basalRate` (comme la pompe), **discriminée par la FORME** du jeu : stylo = `{ kind: daily/morning/evening, value }` (U **TOTALES**) vs pompe = `{ startTime, endTime, rate }` (U/h). Modèle : colonnes `BasalConfiguration.{dailyDose,morningDose,eveningDose}` (`Decimal(6,2)`, U totales), `configType ∈ single_injection/split_injection`. **Validation** (`assertValidStyloBasalSet`) : cohérence de MODALITÉ intra-jeu (exactement `{daily}` OU `{morning,evening}` — `invalidSlotSet` sinon), `kind` unique, plancher `MDI_BASAL_MIN_U` 0,5, **pas de plafond dur** (U300/dégludec ; WARN 80 U non bloquant), délivrable demi-unité, garde anti-overflow `≤ 9999,99`. ⚠️ **Bornes U TOTALES, JAMAIS `BASAL_MIN/MAX` (U/h)** — risque clinique majeur. **Apply** (`replaceStyloBasalSet`) : écriture ATOMIQUE du jeu (`updateMany` multi-colonnes scopé `settings.patientId`, ≤ 1 ligne/patient), **discriminateur pompe⇄stylo par `configType` LIVE** (`basalConfigNotStylo` fail-closed — « la config LIVE n'est pas un stylo » : jamais des U totales sur une config pompe), **cardinalité vs configType LIVE** (`styloBasalConfigMismatch` si single↔split basculé entre génération et acceptation), CAS d'ensemble par `kind` sous verrou AVANT écriture, **PUIS CAS ATOMIQUE EN BASE** : chaque colonne écrite verrouillée à sa valeur LIVE dans le `WHERE` ; une dérive concurrente ou une dose effacée matche 0 ligne → `styloBasalNotFound`, rollback ; jamais d'écrasement silencieux ni d'« appliqué » fantôme sur 0 ligne. Supersession `basalRate` pending. Projection LIVE filtrée par `configType` (alignée sur la capture baseline — un `dailyDose` résiduel chez un split ne fabrique pas de faux `baselineMoved`). `getStyloBasalSlots` = lecteur de forme unique (doses `null` exclues). `SlotRationale` étendue avec clé `basalDoseKind` (XOR ternaire). |

| **Basale STYLO (MDI) groupée — moteur single/split + UI de revue (US-2663 S3e PR2 → S5)** | Bascule le moteur MDI (US-2659 single_injection + split_injection) vers **une `SlotSetProposal` GROUPÉE** de forme STYLO — **grouped-only depuis S5** (flag `ENGINE_GROUPED_STYLO` **supprimé**). **Décisions cliniques INCHANGÉES** (matrice `decideMdiDose`, dé-escalade/cooldown/hold-zone, garde nocturne/jour, confondeur bolus-midi, flags par cible, priorité sécurité « une dose/run ») — seule la **voie d'écriture** change. **Socle** : `assembleGroupedStyloDisposition` (pur : appariement live par `kind`, CAS `1e-9` vs `currentValue`, no-op, garde direction `reason↔delta`, overlay de la dose changée sur le JEU de modalité ENTIER, rationale clé `basalDoseKind`) + `emitGroupedStyloBasal` (relit `getStyloBasalSlots`, baseline injecté = même lecture ⇒ TOCTOU fermée, rejets `createSetProposal` fail-closed non fatals). **Verrou « 1 pending »** : porté par l'unicité `SlotSetProposal` (`createSetProposal` supersède le pending `basalRate`) — la vue moteur la plus récente supersède (parité pompe/ISF/ICR groupés), donc **pas de « défer silencieux »**. Les flags de revue (Somogyi/hypo diurne, dé-escalades perdantes) restent levés (orthogonaux à la proposition). **⚠️ RÈGLE DE DÉCISION CLINIQUE — perte de l'invariant « 1 pending préservé » (validée medical, tradeoff explicite)** : la voie GROUPÉE supersède une proposition pending à chaque run — une dé-escalade GAGNANTE pending peut être effacée par une proposition ultérieure sur `basalRate` **sans flag dédié**. Tradeoff défendable : (a) toute hausse reste **hypo-guardée** par les nadirs de `decideMdiDose` (jamais d'augmentation si l'hypo persiste) ; (b) tant que le signal d'hypo persiste, la dé-escalade est **re-proposée** (magnitude fraîche, priorité sécurité) — elle n'est « perdue » que si l'hypo s'est réellement résolue ; (c) le canal `ClinicalReviewFlag` reste levé pour Somogyi / dé-escalade perdante / bloquée-sévère / non-actionnable — ⚠️ ne couvre PAS une dé-escalade gagnante actionnable effacée (couverture par (a)+(b)). **Supersession scopée** : `source:"algorithm"` (ne touche jamais une proposition humaine, qui coexiste). **UI de revue** : `basalRate` porte DEUX formes → détection SERVEUR par la forme du jeu (`kind` stylo vs `startTime` pompe) → `diffStyloBasalSlots` (clé `kind`), drapeau `isPenBasal` qui bascule l'affichage en **U TOTALES** (`insulinUnits.u`, jamais U/h) et le libellé de dose (`review.styloBasalKind.{daily,morning,evening}`, fr/en/ar), rationale appariée par `basalDoseKind`. Surfaçage des `ClinicalReviewFlag` ouverts AVANT une baisse : déjà assuré par l'écran de revue (US-2659 S3, `openReviewFlags`). Délivrabilité device-aware (moteur émet en multiples de `MDI_BASAL_DELIVERY_INCREMENT_U` = 1 U) ; validateur `assertValidStyloBasalSet` délivrable demi-unité. **Parité imparfaite (D1)** : un créneau hors-borne est plafonné (voir D1). |
| **Voie manuelle GROUPÉE + garde clinique patient (US-2663 S4, décision produit D3)** | La création manuelle d'une proposition (patient/infirmière/médecin) passe désormais en **GROUPÉE** (`SlotSetProposal`, jeu entier) : route PRO `POST /api/slot-set-proposals` (nurse/doctor, provenance dérivée session, ADMIN/VIEWER refusés) et route patient `PUT /api/patient/insulin-slot-set` étendue à la **basale**. Toujours `pending` → revue MÉDECIN (jamais auto-appliqué, ADR #13). **Garde clinique PATIENT** (`evaluatePatientGroupedGate`, pur, appelée UNIQUEMENT pour `source: patient` — les soignants ne sont PAS gatés, cliniciens qualifiés) : (1) **cap de variation `PATIENT_MAX_CHANGE_PERCENT` = 10 %** — ISF/ICR les DEUX sens (une baisse d'ISF = plus d'insuline → hypo), hausse basale ; (2) **BAISSE de basale** relâchée mais GATÉE (US-2659 porté au jeu) : maturité par mode (stylo → CONFIRME ; pompe → INTERMEDIATE+ ; JUNIOR = refus), **accusé DKA BLOQUANT stylo** (`sickDayAcknowledgedAt` immuable — jour-de-maladie : réduire une basale lente expose à l'acidocétose), cap d'amplitude (stylo `min(10 %, MDI_BASAL_PATIENT_MAX_DELTA_U = 2 U)` ; pompe 10 %), snap d'incrément stylo (`MDI_BASAL_DELIVERY_INCREMENT_U = 1 U`, délivrable ½ U) ; (3) **cohérence de mode** (`deliveryModeMismatch` — forme stylo/pompe vs `configType` LIVE, anti-usurpation) ; (4) **RESTRUCTURATION gatée par ENVELOPPE MINUTE-PAR-MINUTE** (US-2663 suivi, validé `medical-domain-validator`) — **remplace** l'ancien blocage total `structuralChangeNotAllowed`. La restructuration (déplacer/ajouter/supprimer un créneau) est **rouverte pour INTERMEDIATE+** (JUNIOR = valeurs-seules → `structuralChangeNotAllowed`). Sûreté ISF/ICR/**pompe** : pour **chaque minute** des 24 h, l'écart entre la valeur proposée couvrant cette minute et la valeur de base qui la couvrait doit rester ≤ `PATIENT_MAX_CHANGE_PERCENT` (10 %, bilatéral) → `patientDeltaTooLarge`. Cette enveloppe **subsume** l'ancien cap par-créneau ET ferme la faille S4 sous ses **deux** formes : (a) *déplacement de frontière* étendant une valeur agressive (ancrer au seul début du créneau ne suffit PAS — un créneau `[6,24) 0,25` sur une base `[6,10) 0,25 / [10,24) 0,50` est capté sur 10h→24h) ; (b) *redistribution pompe à total constant* (concentration nocturne — un gate sur la dose totale/jour seul serait insuffisant). **STYLO — modalité VERROUILLÉE** : la bascule single↔split (`{daily}` ⇄ `{morning,evening}`) est un changement de schéma thérapeutique (profil pharmacocinétique, décision de PRESCRIPTEUR) → **interdite au patient** (`structuralChangeNotAllowed`), quelle que soit la maturité ; à modalité identique, appariement par `kind` et gates de baisse inchangés (CONFIRME + accusé DKA). **Baisse basale POMPE** (une minute où proposé < base) exige INTERMEDIATE+ (`maturityTooLowForDecrease`, pas de DKA). **Base vide/incomplète** → `structuralChangeNotAllowed` (établir une config initiale est un acte clinicien). **Socle non négociable** : couverture 24 h stricte (`assertValidGroupedSet`, exécutée AVANT la garde) — c'est elle qui rend l'enveloppe minute bien définie. `src/lib/insulin/patient-grouped-gate.ts` (`buildCoveringMinutes`) ; verrou anti-drift `tests/unit/patient-grouped-gate.test.ts` (scénarios CRITICAL frontière/redistribution/bascule). **Garde anti-fragmentation** : à la restructuration, chaque créneau ISF/ICR/pompe doit durer ≥ `PATIENT_RESTRUCTURE_MIN_SLOT_MINUTES` (**30 min**, décision interne — ne mord que sur la pompe minute-précise, les ISF/ICR étant en heures entières ≥ 60 min ; cardinalité déjà bornée à 24 par la route) → `restructureSlotTooShort` (422). **Dose fixe** non exposée à la voie patient → rejet explicite `unsupportedSlotSetParam` (défensif fail-closed). **Décision produit D3 (validée utilisateur + medical)** : un jeu pouvant mélanger hausses ET baisses, **UN SEUL accusé DKA couvre TOUTES les baisses stylo du jeu** (l'accusé « jour de maladie » est un état patient global, pas par créneau) — porté au niveau proposition, non par créneau. Patient non insuliné → `nonInsulinNoDose` + flag d'orientation `reviewInConsultation` (parité ETP). **⚠️ CHANGEMENT DE COMPORTEMENT** : le cap 10 % s'applique désormais aux soumissions patient **ISF/ICR groupées** (avant S4 : `createSetProposal` sans garde). **Écarts assumés vs voie par-valeur (documentés)** : cooldown anti-churn US-2650 (`PATIENT_PROPOSAL_COOLDOWN_HOURS`) NON porté (la supersession borne le *stock* pending ; porter un cooldown 24 h empêcherait un patient de corriger une soumission rejetée — écart délibéré) ; audit de refus route (`INSULIN_SLOT_SUBMISSION`/`reason`) moins enrichi que le `PROPOSAL_REFUSED` par-valeur (direction/mode) — le code `reason` reste le signal forensic clé ; `fixedDose` accepté sur la route PRO mais OMIS de la route patient (un patient « doses simples » ne soumet pas ses doses fixes en groupé — intentionnel). |
| **Plafonnement moteur à la borne clinique (US-2663 S5, décision produit D1)** | Quand la valeur **calculée** par le moteur d'ajustement dépasse la borne clinique de son levier (ISF `ISF_GL_MIN/MAX`, ICR `ICR_MIN/MAX`, basale POMPE `BASAL_MIN/MAX` U/h, basale STYLO `MDI_BASAL_MIN_U`.. garde anti-overflow 9999,99, dose fixe `FIXED_DOSE_MIN`..999,99), la disposition groupée porte la **valeur À LA BORNE** (plafonnée, `clampProposed`), **jamais rejetée ni skippée**, et la `rationale` du créneau note `cappedToBound: true` + `cappedFromValue` (valeur brute calculée). La proposition reste `pending` → **le MÉDECIN tranche** (jamais auto-appliqué, ADR #13) ; l'écran de revue affiche un **badge « Plafonné (calcul X unité) »**. **Raison** : remonter l'information au clinicien plutôt que la cacher (skip) ou geler tout le levier (ancien fail-closed « un créneau hors-borne rejetait toute la disposition »). Cas dégénéré : un créneau DÉJÀ à la borne dont le calcul dépasse → plafonné = live → `delta 0` → no-op (pas de proposition, rien à changer). S'applique aux **5 leviers** (émetteurs `emitGrouped*` passent les bornes de leur forme à l'assembleur pur `assembleGrouped*` ; **le paramètre `bounds` est OBLIGATOIRE** — revue PR #749/#3 : la sûreté ne repose plus sur la discipline de l'appelant). ⚠️ La borne écrite est TOUJOURS dans les limites cliniques (jamais de dose hors-borne). **Garde d'INTÉGRITÉ DE BASE (revue PR #749/#1)** : une base LIVE **elle-même hors-borne** (donnée legacy, ex. ICR 32 g/U) ne sert **JAMAIS** de point de titration — le créneau est **ABANDONNÉ** (fail-closed, `isBaseOutOfBounds`), car un delta plafonné depuis une base invalide pourrait excéder le cap de titration (%) ; le clinicien corrige d'abord la base. **Nuance plafond stylo/dose fixe** : leur `max` (`COLUMN_OVERFLOW_GUARD_STYLO_U` 9999,99 / `_FIXED_DOSE_U` 999,99) est un **garde anti-overflow de colonne**, PAS une borne clinique ni un multiple délivrable — inatteignable en pratique (caps de variation par run) ; le **plancher** (`MDI_BASAL_MIN_U`/`FIXED_DOSE_MIN` = 0,5 U) est la borne structurante de ces 2 leviers. |
| **Grouped-only intégral (US-2663 S5, livré)** | **État final** : toute émission d'ajustement d'insuline (moteur + humain) est **GROUPÉE** (`SlotSetProposal`, jeu entier de créneaux) — voie par-valeur (`AdjustmentProposal` par-créneau) **SUPPRIMÉE en écriture** (modèle conservé en lecture/registre pour les propositions existantes, `accept`/`reject` fonctionnels). **Flags moteur retirés** : `ENGINE_GROUPED_ISF_ICR`, `ENGINE_GROUPED_PUMP`, `ENGINE_GROUPED_FIXED_DOSE`, `ENGINE_GROUPED_STYLO` → comportement grouped-only **par défaut et UNIQUE**. **Routes d'écriture** : moteur `proposalGeneratorService` → `slotSetProposalService.createSetProposal` (5 leviers) ; humain `POST /api/slot-set-proposals` (PRO) + `PUT /api/patient/insulin-slot-set` (patient) ; DOCTOR direct `PUT /api/insulin-therapy/*` (`replaceSlotSet`/`replacePumpSlotSet`). **UI** : composant `InsulinProposalDialog` (proposition par-créneau) **retiré** (S5) — proposition humaine via éditeurs GROUPE seuls. **Socle immuable** (S0–S4) : `SlotSetProposal` polymorphe (5 formes), `baselineSlots` snapshot à la génération, CAS d'ensemble (`baselineMoved`/`baselineMissing`), revue médecin intégrée, D1 plafonnement moteur, D2 coexistence humain↔moteur par classe d'origine, D3 garde clinique patient groupée. |
| **Accusé / actualisation patient GROUPÉS (US-2663 S5/c4 ; par-valeur retiré S6)** | Port grouped-only de l'accusé patient (US-2065) et de la vérification d'application réelle (US-2066) : la voie par-valeur étant retirée, l'app mobile acquitte/actualise désormais la proposition GROUPÉE. **US-2663 S6** : les services/routes/tables PAR-VALEUR (`proposalAckService`/`proposalActualizationService`, `POST/PUT /api/team/proposal-ack|actualization/[proposalId]`, tables `adjustment_proposal_acks`/`adjustment_proposal_actualizations`) sont **entièrement retirés** (plus aucun producteur/consommateur ; DROP TABLE `20260730100000`, sûr car hors prod / 0 donnée — D2). Conservés car partagés : `ACK_COMMENT_MAX`, `VERIFY_VIA_VALUES`/`VerifyVia`, valeurs d'audit `PROPOSAL_ACK`/`PROPOSAL_ACTUALIZATION`. Nouveaux modèles **1:1** `SlotSetProposalAck` / `SlotSetProposalActualization` (`slot_set_proposal_id @unique`, FK `onDelete: Cascade`, comment patient chiffré AES-256-GCM). **Décision produit D3 : 1 accusé = jeu ENTIER** (le patient valide la disposition complète, pas un créneau). Services `slotSetProposalAckService.{markRead,respond}` + `slotSetProposalActualizationService.{record,getProposalPatientId}` (`team-workflow.service.ts`) — sémantique identique à la voie par-valeur : upsert idempotent, garde **H4** anti-écrasement d'une actualisation de source différente (`alreadyActualized`), audit `PROPOSAL_ACK`/`PROPOSAL_ACTUALIZATION` (`metadata.model = "slotSet"`). Routes `POST/PUT /api/team/slot-set-proposal-ack/[proposalId]` + `POST /api/team/slot-set-proposal-actualization/[proposalId]` : **frontière de provenance MDR** répliquée (VIEWER n'acquitte QUE `source = patient` via `viewerProposalSources` ; 404 non-énumérant + audit `accessDenied` sur sonde d'UUID tiers ; 403 si pas de dossier patient propre). Migration additive `20260729100000` (rejouable). |
