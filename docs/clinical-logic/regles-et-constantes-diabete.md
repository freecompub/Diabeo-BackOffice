...
| **Disposition groupée + snapshot de base + CAS d'ensemble (US-2663 épic, S0+S1+S2)** | Toute proposition d'ajustement — quelle que soit l'origine (patient/infirmière/médecin/**algorithme**), même si un seul créneau change — porte la **disposition ENTIÈRE** du levier (jeu de créneaux complet). Modèle unique `SlotSetProposal` (généralisé) ; typage cible = **union discriminée par levier** (`src/lib/insulin/grouped-proposal.ts` : ISF/ICR `startHour`·g/L·U ou g/U ; basale pompe `startTime`·U/h ; basale stylo `basalDoseKind`·U totales ; dose fixe `moment`·U). **`baselineSlots`** = snapshot de la base **PAR CRÉNEAU à la génération** (`captureBaselineSlots`, même encodage que `proposedSlots`). `source` dérivé **serveur** (ADR #27, jamais du body). **Garde-fou MDR — CAS d'ensemble fail-closed (S1, LIVRÉ)** : à l'acceptation, **sous verrou** `tryLockInsulinSlots` (lecture LIVE atomique, pas de TOCTOU), la base actuelle DOIT être identique au snapshot (`assertBaselineUnchanged`, `src/lib/insulin/slot-baseline-cas.ts` ; appariement par clé `startHour`, `mealLabel` non dosant **ignoré**). Toute dérive (valeur/borne/structure) → **`baselineMoved`** (409, rollback → reste `pending`, à régénérer) : n'écrase jamais un ajustement MÉDECIN concurrent (ex. baisse post-hypo). Snapshot `null` (legacy pré-S0) → **`baselineMissing`** (409, fail-closed : jamais d'apply sur une base non certifiable). Sémantique S1 = **rejet d'ensemble** (toute divergence rejette la disposition entière) ; diff-merge valeur-seule différé (S1bis). Chemin **DOCTOR direct** (`replaceSlotSet` sans `expectedBaseline`) = pas de CAS (écrasement explicite assumé). **Revue médecin (S2, LIVRÉ)** : `SlotSetProposal` **PENDING** surfacée sur `/patients/[id]/review` (`GroupedProposalReview`), jusqu'ici invisible à la revue (seules les `AdjustmentProposal` par-valeur y apparaissaient). `slotSetProposalService.listPendingForReview` expose `baselineSlots` (contrairement à `listSetProposals`, qui l'omet pour la liste patient — minimisation) : usage DOCTOR-gated légitime pour le decision-support. Diff (créneau PROPOSÉ vs valeur LIVE actuelle) pré-calculé **serveur** (`src/lib/insulin/slot-diff.ts`, `diffSlots`/`hasStructuralChange`) ; lignes divergentes surlignées à l'affichage. Variante NON-throwing du CAS (`isBaselineUnchanged`, `slot-baseline-cas.ts`) affiche un bandeau d'AVERTISSEMENT si la base a dérivé depuis la génération — **non bloquant à l'affichage** (le blocage réel reste le 409 `baselineMoved`/`baselineMissing` à l'acceptation, ci-dessus) : prévient le médecin AVANT une tentative d'acceptation vouée à l'échec. *S4 voie manuelle groupée, S5 retrait par-valeur + iOS à suivre.* |
| **Bascule MOTEUR → émission GROUPÉE ISF/ICR (US-2663 S3b-1, derrière flag)** | Le générateur de propositions (`proposal-generator.service.ts`) émet, sous le flag `ENGINE_GROUPED_ISF_ICR` (env, **OFF par défaut, réversible**, un seul flag ISF **et** ICR), **une** `SlotSetProposal` `source: "algorithm"` par levier (disposition ENTIÈRE) **au lieu de** N `AdjustmentProposal` par-valeur. Assemblage : `emitGroupedIsfIcr` (IO) relit la config LIVE une fois (T1) puis délègue à `assembleGroupedDisposition` (**cœur PUR, unit-testable**). Gardes PAR CRÉNEAU (parité de sûreté avec la voie par-valeur remplacée) : (1) **CAS R2** — un changement n'est retenu QUE si le créneau live est INCHANGÉ depuis l'analyse : **même `endHour`** (fenêtre horaire non restructurée) ET valeur live == base analysée (`candidate.currentValue`, snapshot T0 ; tolérance `1e-9`). Sinon **ABANDONNÉ** — jamais une magnitude/fenêtre périmée injectée (réplique `baselineMovedAtPersist`, perdu au regroupement) ; (2) **garde direction** — le sens du delta doit être cohérent avec le `reason` (`*TooLow`⇒hausse, `*TooHigh`⇒baisse) ; incohérence (défaut d'analyseur) → créneau abandonné + logué `reasonDirectionMismatch` (jamais une rationale trompeuse « on hausse » sur une baisse = plus d'insuline) ; (3) **no-op (R4)** — aucune émission si 0 créneau retenu ; (4) **`mealLabel` ICR préservé (R5)** ; (5) **rationale MOTEUR (R3)** — 1 `SlotRationale` par créneau changé (motif/confiance/volume/moyenne/période jours entiers), requise par `createSetProposal` pour `source: "algorithm"`. **Fenêtre TOCTOU FERMÉE** : la relecture LIVE (T1) sert À LA FOIS de base d'overlay ET de `baselineSlots` **injecté** à `createSetProposal` (param `baselineOverride` — pas de 2ᵉ lecture) ⇒ disposition et snapshot de base partagent UN instant ⇒ le CAS d'ensemble de l'acceptation (S1) couvre TOUS les créneaux, y compris ceux inchangés (une édition médecin d'un créneau non touché lève `baselineMoved` au lieu d'être revertée). Décisions cliniques (analyzeurs, deadband, dé-escalade, cooldown, flags haute-variabilité) **INCHANGÉES** — seule la voie d'écriture change. Rejets `createSetProposal` fail-closed → **non fatals**. ⚠️ **Parité imparfaite documentée** : un unique créneau hors borne (`valueOutOfBounds`) rejette TOUTE la disposition du levier ce run (la voie par-valeur ne rejetait que le fautif) — fail-closed, à trancher (produit) au passage du flag ON. Supersession par **classe d'origine** (D2) : la proposition moteur coexiste avec une demande humaine. |

| **Généralisation GROUPÉE à la basale POMPE (US-2663 S3c / PR-A, derrière flag)** | Le pipeline groupé (modèle, CAS d'ensemble, apply à l'acceptation, moteur, UI de revue) est étendu à la **basale POMPE** (`basalRate`, créneaux `PumpBasalSlot` : bornes `"HH:MM"` minute-précis, débit U/h). **Socle mutualisé** : (a) CAS d'ensemble **générique par clé** (`assertBaselineUnchangedBy` — pompe apparie par `startTime`, borne `endTime`, valeur `rate`) ; (b) `createSetProposal` migré en **objet d'options** (`SlotSetParam` élargi à `basalRate` ; `parseSlots`/`captureBaselineSlots`/validation polymorphes) ; (c) `replacePumpSlotSet` **rétrofité** — param `cas` (CAS d'ensemble sous verrou avant écriture) + supersession des `SlotSetProposal` `basalRate` pending (l'invariant « aucune SlotSetProposal basale » **tombe** — anti dérive de base : un jeu pompe périmé n'écrase jamais un ajustement médecin récent). **Discriminateur pompe⇄stylo (CRITIQUE)** : l'apply route sur le `configType` **LIVE** (`basalConfigNotPump` fail-closed) — jamais des U/h écrits sur une dose stylo (U totales). **Moteur** (`ENGINE_GROUPED_PUMP`, flag DISTINCT d'ISF/ICR, OFF par défaut) : le générateur ne titre que le créneau **nocturne** mais émet la disposition **entière** (tous les créneaux pompe, débit nocturne changé superposé) via `assembleGroupedPumpDisposition` (pur) + `emitGroupedPumpBasal` — appariement candidat⇄live par `id`, CAS débit, garde direction, **temps `"HH:MM"` EXACTS préservés** (jamais réécrits à l'heure entière), baseline injecté (TOCTOU fermée). **UI de revue** : `diffPumpSlots` (par `startTime`, `timeLabel` exact), unité U/h (`PARAM_UNIT_KEY.basalRate`). **Non couverts par PR-A** : basale **STYLO** (PR dédiée — apply atomique US-2660, single/split) et **dose fixe** (PR dédiée — identification par **usage** basal/bolus, correction de l'aveuglement du générateur). Parité imparfaite hors-borne conservée (rejet du levier entier, fail-closed). Flag OFF ⇒ voie par-valeur pompe inchangée (pas de rupture iOS). |
| **Dose fixe groupée — identification PAR USAGE + apply fail-closed (US-2663 S3d PR1)** | Mode « doses simples » (`fixedDose`). **Règle de clé (décision produit)** : une dose fixe est identifiée par **`(usage, moment)`**, PAS par `moment` seul — un même `moment` (ex. « soir ») peut porter une dose **BOLUS** (rapide) ET une dose **BASALE** (lente) sur deux `PatientInsulin` distincts (`InsulinUsage` ∈ `bolus`/`basal`/`both`). Écrire par `moment` seul ciblerait la mauvaise insuline. **Apply** (`replaceFixedDoseSet`) : **update-in-place par `id` RÉSOLU SERVEUR** par `(usage, moment)` (les `FixedDoseSlot` pendent de `PatientInsulin` distincts — pas de delete+create) ; **résolution fail-closed 0/1/>1** : 0 → `fixedDoseSlotNotFound` (dose supprimée depuis la génération, 409), **>1 → `fixedDoseSlotAmbiguous`** (deux insulines de même usage/moment → désambiguïser, 422), 1 → écriture. **Bornes** (`assertValidFixedDoseSet`) : plancher `FIXED_DOSE_MIN` 0,5 U, délivrabilité (multiple d'incrément), clé `(usage, moment)` unique, **pas de plafond dur** (WARN `FIXED_BOLUS_WARN_U` 25 / `FIXED_BASAL_WARN_U` 80 non bloquants), garde anti-overflow `≤ 999,99` (colonne `Decimal(5,2)`). CAS d'ensemble générique (clé `(usage, moment)`, sans borne). **Filtre insuline ACTIVE** (`active-insulin.ts`, `isActive` + `endDate`) sur TOUS les lecteurs/écriveurs de doses fixes (grouped + par-valeur + générateur) : jamais de dose fantôme ni de mésécriture sur une insuline discontinuée. **Correction d'un bug par-valeur pré-existant** : l'apply `fixedDose` usage-blind (`updateMany` par `moment`+valeur) pouvait toucher **plusieurs lignes silencieusement** (`count>1` passait) → désormais `count===1` requis (0→notFound, >1→`fixedDoseSlotAmbiguous`). **PR1 = apply + accept SANS moteur** (aucune émission). **PR2 (moteur + UI)** : générateur usage-aware (`select += patientInsulin.usage`, collecte par `(usage, moment)`) derrière le flag `ENGINE_GROUPED_FIXED_DOSE` (distinct, OFF) → `assembleGroupedFixedDose` (pur) + `emitGroupedFixedDose` (baseline injecté, TOCTOU fermée). **D2 — ABANDON de la titration d'un moment MULTI-DOSES** (≥ 2 usages au même moment) : le signal glycémique (creux pré-dose) est par MOMENT, non attribuable à un usage → aucune proposition sur ce moment (log info, laissé au médecin) — reco `medical-domain-validator`. **UI de revue** : `diffFixedDoseSlots` (clé `(usage, moment)`, libellé « Bolus · Matin » traduit), unité U, badge d'avertissement `highDoseWarning` dérivé SERVEUR (bolus > `FIXED_BOLUS_WARN_U` 25 / basal **et `both`** > `FIXED_BASAL_WARN_U` 80, NON bloquant ; `both` = prémix bolus+basale rattaché au seuil basal — décision à confirmer medical). **Garde-fou hypo (revue medical PR2)** : l'abandon D2 d'un candidat de **BAISSE** (dé-escalade hypo) sur un moment multi-doses **lève un `ClinicalReviewFlag`** (jamais une hypo silencieuse) ; une hausse abandonnée reste log-only. Flag OFF ⇒ voie par-valeur dose fixe inchangée. |

| **Basale STYLO (MDI) groupée — apply fail-closed (US-2663 S3e PR1)** | 4ᵉ et dernier levier du pipeline groupé. La basale STYLO réutilise `parameterType = basalRate` (comme la pompe), **discriminée par la FORME** du jeu : stylo = `{ kind: daily/morning/evening, value }` (U **TOTALES**) vs pompe = `{ startTime, endTime, rate }` (U/h). Modèle : colonnes `BasalConfiguration.{dailyDose,morningDose,eveningDose}` (`Decimal(6,2)`, U totales), `configType ∈ single_injection/split_injection`. **Validation** (`assertValidStyloBasalSet`) : cohérence de MODALITÉ intra-jeu (exactement `{daily}` OU `{morning,evening}` — `invalidSlotSet` sinon), `kind` unique, plancher `MDI_BASAL_MIN_U` 0,5, **pas de plafond dur** (U300/dégludec ; WARN 80 U non bloquant), délivrable demi-unité, garde anti-overflow `≤ 9999,99`. ⚠️ **Bornes U TOTALES, JAMAIS `BASAL_MIN/MAX` (U/h)** — risque clinique majeur. **Apply** (`replaceStyloBasalSet`) : écriture ATOMIQUE du jeu (`updateMany` multi-colonnes scopé `settings.patientId`, ≤ 1 ligne/patient), **discriminateur pompe⇄stylo par `configType` LIVE** (`basalConfigNotStylo` fail-closed — « la config LIVE n'est pas un stylo » : jamais des U totales sur une config pompe), **cardinalité vs configType LIVE** (`styloBasalConfigMismatch` si single↔split basculé entre génération et acceptation), CAS d'ensemble par `kind` sous verrou AVANT écriture, **PUIS CAS ATOMIQUE EN BASE** (miroir de la voie par-valeur US-2660 : chaque colonne écrite verrouillée à sa valeur LIVE dans le `WHERE` ; une dérive concurrente de la voie par-valeur US-2660 — qui écrit les MÊMES colonnes SANS le verrou consultatif — ou une dose effacée sous nous matche 0 ligne → `styloBasalNotFound`, rollback ; jamais d'écrasement silencieux ni d'« appliqué » fantôme sur 0 ligne), supersession `basalRate` pending (par-valeur incl. verrou stylo + d'ensemble). Projection LIVE filtrée par `configType` (alignée sur la capture baseline — un `dailyDose` résiduel chez un split ne fabrique pas de faux `baselineMoved`). `getStyloBasalSlots` = lecteur de forme unique (doses `null` exclues). `SlotRationale` étendue avec clé `basalDoseKind` (XOR ternaire). **PR1 = apply SANS moteur ni UI** (le moteur single/split, l'UI unité U totales, l'alignement de la délivrabilité device-aware — validateur à 0,5 U vs moteur MDI à 1 U fail-closed — et la décision sick-day DKA sont en PR2). |

| **Basale STYLO (MDI) groupée — moteur single/split + UI de revue (US-2663 S3e PR2)** | Bascule le moteur MDI (US-2659 single_injection + split_injection) de l'émission par-valeur (`AdjustmentProposal.basalDoseKind`) vers **une `SlotSetProposal` GROUPÉE** de forme STYLO, **derrière le flag `ENGINE_GROUPED_STYLO`** (distinct, OFF par défaut ⇒ voie par-valeur inchangée, aucune rupture iOS avant S5). **Décisions cliniques INCHANGÉES** (matrice `decideMdiDose`, dé-escalade/cooldown/hold-zone, garde nocturne/jour, confondeur bolus-midi, flags par cible, priorité sécurité « une dose/run ») — seule la **voie d'écriture** change. **Socle** : `assembleGroupedStyloDisposition` (pur : appariement live par `kind`, CAS `1e-9` vs `currentValue`, no-op, garde direction `reason↔delta`, overlay de la dose changée sur le JEU de modalité ENTIER, rationale clé `basalDoseKind`) + `emitGroupedStyloBasal` (relit `getStyloBasalSlots`, baseline injecté = même lecture ⇒ TOCTOU fermée, rejets `createSetProposal` fail-closed non fatals). **Verrou « 1 pending »** : porté par l'unicité `SlotSetProposal` (`createSetProposal` supersède le pending `basalRate`) — le verrou par-valeur `one_pending_stylo_basal` ne s'applique pas à cette voie ; la vue moteur la plus récente supersède (parité pompe/ISF/ICR groupés), donc **pas de « défer silencieux »**. Les flags de revue (Somogyi/hypo diurne, dé-escalades perdantes) restent levés dans les deux modes (orthogonaux à la proposition). **⚠️ RÈGLE DE DÉCISION CLINIQUE — perte de l'invariant « 1 pending préservé » (validée medical, tradeoff explicite)** : contrairement à la voie par-valeur (où le verrou PRÉSERVE une dé-escalade pending non revue jusqu'à décision médecin), la voie GROUPÉE supersède une proposition pending à chaque run — une dé-escalade GAGNANTE pending (run N) peut être effacée par une proposition algorithme ultérieure sur `basalRate` (run N+1, ex. hausse d'une AUTRE dose split) **sans flag dédié**. Tradeoff jugé défendable car : (a) toute hausse ultérieure reste **hypo-guardée** par les nadirs de `decideMdiDose` (jamais d'augmentation si l'hypo persiste) ; (b) tant que le signal d'hypo persiste, la dé-escalade est **re-proposée à chaque run** (magnitude fraîche, priorité sécurité « dé-escalade d'abord ») — elle n'est « perdue » que si l'hypo s'est réellement résolue (cas où ne pas baisser est défendable). **La sécurité du scénario repose sur (a)+(b)** : une dé-escalade GAGNANTE actionnable ne peut être supersédée par une hausse d'une autre dose que si son propre signal d'hypo a disparu. (c) le canal `ClinicalReviewFlag` (jamais superséé — `createSetProposal` ne touche que les propositions) reste levé pour les sous-cas Somogyi / dé-escalade **perdante** / bloquée-sévère / non-actionnable — ⚠️ **il ne couvre PAS** le cas d'une dé-escalade gagnante actionnable effacée (celle-ci ne lève aucun flag ; sa couverture est (a)+(b), pas (c)). (d) supersession scopée `source:"algorithm"` (ne touche jamais une proposition humaine, qui coexiste). **UI de revue** : `basalRate` porte DEUX formes → détection SERVEUR par la forme du jeu (`kind` stylo vs `startTime` pompe, `page.tsx`) → `diffStyloBasalSlots` (clé `kind`), drapeau `isPenBasal` qui bascule l'affichage en **U TOTALES** (`insulinUnits.u`, jamais U/h) et le libellé de dose (`review.styloBasalKind.{daily,morning,evening}`, fr/en/ar), rationale appariée par `basalDoseKind`. Le surfaçage des `ClinicalReviewFlag` ouverts AVANT une baisse (exigence revue PR1) est **déjà assuré** par l'écran de revue (US-2659 S3, `openReviewFlags`). **Différé** : délivrabilité device-aware (le moteur n'émet que des multiples de `MDI_BASAL_DELIVERY_INCREMENT_U` = 1 U → jamais de valeur non délivrable ; l'alignement du validateur `assertValidStyloBasalSet` sur l'incrément dispositif concerne la **voie manuelle humaine** S4, où la valeur ne vient pas du moteur). |
