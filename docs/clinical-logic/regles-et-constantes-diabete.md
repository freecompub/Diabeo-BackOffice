...
| **Basale STYLO (MDI) groupée — apply fail-closed (US-2663 S3e PR1)** | 4ᵉ et dernier levier du pipeline groupé. La basale STYLO réutilise `parameterType = basalRate` (comme la pompe), **discriminée par la FORME** du jeu : stylo = `{ kind: daily/morning/evening, value }` (U **TOTALES**) vs pompe = `{ startTime, endTime, rate }` (U/h). Modèle : colonnes `BasalConfiguration.{dailyDose,morningDose,eveningDose}` (`Decimal(6,2)`, U totales), `configType ∈ single_injection/split_injection`. **Validation** (`assertValidStyloBasalSet`) : cohérence de MODALITÉ intra-jeu (exactement `{daily}` OU `{morning,evening}` — `invalidSlotSet` sinon), `kind` unique, plancher `MDI_BASAL_MIN_U` 0,5, **pas de plafond dur** (U300/dégludec ; WARN 80 U non bloquant), délivrable demi-unité, garde anti-overflow `≤ 9999,99`. ⚠️ **Bornes U TOTALES, JAMAIS `BASAL_MIN/MAX` (U/h)** — risque clinique majeur. **Apply** (`replaceStyloBasalSet`) : écriture ATOMIQUE du jeu (`updateMany` multi-colonnes scopé `settings.patientId`, ≤ 1 ligne/patient), **discriminateur pompe⇄stylo par `configType` LIVE** (`basalConfigNotPump` fail-closed — jamais des U totales sur une config pompe), **cardinalité vs configType LIVE** (`styloBasalConfigMismatch` si single↔split basculé entre génération et acceptation), CAS d'ensemble par `kind` sous verrou AVANT écriture, supersession `basalRate` pending (par-valeur incl. verrou stylo + d'ensemble). `getStyloBasalSlots` = lecteur de forme unique (doses `null` exclues). `SlotRationale` étendue avec clé `basalDoseKind` (XOR ternaire). **PR1 = apply SANS moteur ni UI** (le moteur single/split + l'UI unité U totales + la décision sick-day DKA sont en PR2). |
