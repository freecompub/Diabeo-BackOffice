...
| **(c) nonInsulin** | **Aucune proposition de dose** (frontière MDR). Uniquement des `ClinicalReviewFlag` d'orientation. **Livré** : `generateOrientationFlags` → **`hba1cStale`** (dernière HbA1c > 180 j ou absente) + **`tirBelowTarget`** (TIR 14 j < 70 %, pathology-aware, `null` si capture < 30 %) + **`hba1cAboveTarget`** (HbA1c **récente** `!isStale` au-dessus de la cible — individualisée `objectiveHba1c + 0,5` si plausible, sinon défaut adulte `8,0` / grossesse `6,0` ; comble le **trou BGM-only** : un non-insuliné sans CGM à HbA1c récente mais mauvaise). Partition propre récent↔périmé (pas de double-signal). **À venir** : `observance`. |
