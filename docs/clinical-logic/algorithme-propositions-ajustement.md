...
| **(c) nonInsulin** | **Aucune proposition de dose** (frontière MDR). Uniquement des `ClinicalReviewFlag` d'orientation. **Livré** : `generateOrientationFlags` → **`hba1cStale`** (dernière HbA1c > `HBA1C_STALE_DAYS` 180 j, ou absente) + **`tirBelowTarget`** (TIR 14 j < `DASHBOARD_TIR.TARGET_PERCENT` 70 %, bornes pathology-aware, `null` si capture CGM < 30 % → pas de flag ; helper réutilisable `objectivesService.computeTirPercent`). **À venir** : `observance` (définition à cadrer). La **cible** glycémique reste ajustable **par le médecin**. |
