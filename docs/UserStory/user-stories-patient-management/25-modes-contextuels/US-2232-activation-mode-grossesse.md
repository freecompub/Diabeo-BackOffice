# US-2232 — Activation mode grossesse

> 📌 **25. Gestion modes contextuels (grossesse / pédiatrie / Ramadan / voyage)** · Priorité **MVP**
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3193 (config mode grossesse patient) |
...
Le médecin active le mode grossesse pour une patiente : application des cibles strictes obstétriques, suivi SA, désactivation à J+1 mois post-partum ou validation manuelle.
...
**US miroir patient** : US-3193 (config mode grossesse patient)
...
Quand il tente d'accéder à « Activation mode grossesse »
...
```gherkin
Étant donné Une action sur « Activation mode grossesse » est effectuée
...
```
GET    /api/patients/[patientId]/activation-mode-grossesse                    Lecture configuration courante
PUT    /api/patients/[patientId]/activation-mode-grossesse                    Création nouvelle version
POST   /api/patients/[patientId]/activation-mode-grossesse/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/activation-mode-grossesse/history            Historique versions
POST   /api/patients/[patientId]/activation-mode-grossesse/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/activation-mode-grossesse.service.ts`.
...
- **Miroir app patient** : US-3193 (config mode grossesse patient)
