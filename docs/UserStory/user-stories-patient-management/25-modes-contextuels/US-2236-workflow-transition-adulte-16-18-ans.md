# US-2236 — Workflow transition adulte 16-18 ans

> 📌 **25. Gestion modes contextuels (grossesse / pédiatrie / Ramadan / voyage)** · Priorité **V2**
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3206 (transition adulte) |
...
Bascule progressive d'un compte pédiatrique vers un compte adulte autonome : période de transition, transfert progressif des droits du parent vers l'ado, validation finale.
...
**US miroir patient** : US-3206 (transition adulte)

### Archétype : workflow

Cette US couvre un **workflow métier** structuré avec plusieurs étapes (création → validation → application). Chaque transition d'état doit être auditée, et les règles d'autorisation par état doivent être strictement appliquées.
...
Quand il tente d'accéder à « Workflow transition adulte 16-18 ans »
...
```gherkin
Étant donné Une action sur « Workflow transition adulte 16-18 ans » est effectuée
Quand elle est exécutée avec succès
Alors un AuditLog est créé (action, resource, resourceId, userId, ip, ua, metadata)
```

### AC-3 — Transitions d'état contrôlées

```gherkin
Étant donné Un workflow est dans l'état X
Quand une transition vers Y est tentée
Alors seules les transitions valides selon la machine à états sont acceptées (rejet sinon)
```

### AC-4 — Audit de chaque transition

```gherkin
Étant donné Une transition d'état se produit
Quand elle est validée
Alors un AuditLog dédié est créé avec from_state, to_state, motif
```

### AC-5 — Notification patient si impactant

```gherkin
Étant donné Le workflow change d'état d'une manière qui impacte le patient
Quand la transition est validée
Alors le patient est notifié (push + email si configuré) avec le contexte
```
...
- **RM-6 : Machine à états documentée dans `docs/workflows/{slug}.md`, transitions validées par tests d'intégration.**
- **RM-7 : Notifications patient désactivables individuellement par le patient lui-même.**
...
```prisma
// Extensions probables selon l'archétype "workflow"
// Machine à états du workflow
model PatientWorkflow {
  id              String   @id @default(cuid())
  patientId       String
  workflowType    String   // "ETP_PROGRAM" | "MODE_TRANSITION" | "VALIDATION_REQUEST" etc.
  state           String   // selon FSM documentée
  contextJson     Json     // données du workflow
  ...
  @@index([patientId, workflowType, state])
}
...
model WorkflowTransition {
  ...
  workflow    PatientWorkflow @relation(fields: [workflowId], references: [id])
...
```
POST   /api/patients/[patientId]/workflows/workflow-transition-adulte-16-18-ans     Initier workflow
GET    /api/patients/[patientId]/workflows/[wfId]               Statut workflow
PATCH  /api/patients/[patientId]/workflows/[wfId]/transition    Transition d'état
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/workflow-transition-adulte-16-18-ans.service.ts`.
...
- AuditLog par transition
- Notifications patient déclenchées correctement
...
- Patterns Workflow / Machine à états

---

## 🔗 US liées

- **Miroir app patient** : US-3206 (transition adulte)
