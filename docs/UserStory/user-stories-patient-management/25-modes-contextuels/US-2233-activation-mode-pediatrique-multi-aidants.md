# US-2233 — Activation mode pédiatrique multi-aidants
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3201 (compte enfant multi-aidants) |
...
Configuration d'un compte enfant : association du parent administrateur, autres aidants (parent 2, école, nounou) avec niveaux de droits granulaires (lecture / saisie / config).
...
**US miroir patient** : US-3201 (compte enfant multi-aidants)
...
Quand il tente d'accéder à « Activation mode pédiatrique multi-aidants »
...
```gherkin
Étant donné Une action sur « Activation mode pédiatrique multi-aidants » est effectuée
...
```
GET    /api/patients/[patientId]/activation-mode-pediatrique-multi-aidants                    Lecture configuration courante
PUT    /api/patients/[patientId]/activation-mode-pediatrique-multi-aidants                    Création nouvelle version
POST   /api/patients/[patientId]/activation-mode-pediatrique-multi-aidants/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/activation-mode-pediatrique-multi-aidants/history            Historique versions
POST   /api/patients/[patientId]/activation-mode-pediatrique-multi-aidants/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/activation-mode-pediatrique-multi-aidants.service.ts`.
...
- **Miroir app patient** : US-3201 (compte enfant multi-aidants)
