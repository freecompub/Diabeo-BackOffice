# US-2234 — Configuration mode Ramadan
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3260 (conseils saisonniers Ramadan) |
...
Activation annuelle du mode Ramadan pour un patient : validation médicale obligatoire (T1 mal équilibré déconseillé), saisie horaires Sahur/Iftar, adaptation seuils, conseils contextualisés.
...
**US miroir patient** : US-3260 (conseils saisonniers Ramadan)
...
Quand il tente d'accéder à « Configuration mode Ramadan »
...
```gherkin
Étant donné Une action sur « Configuration mode Ramadan » est effectuée
...
```
GET    /api/patients/[patientId]/configuration-mode-ramadan                    Lecture configuration courante
PUT    /api/patients/[patientId]/configuration-mode-ramadan                    Création nouvelle version
POST   /api/patients/[patientId]/configuration-mode-ramadan/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/configuration-mode-ramadan/history            Historique versions
POST   /api/patients/[patientId]/configuration-mode-ramadan/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/configuration-mode-ramadan.service.ts`.
...
- **Miroir app patient** : US-3260 (conseils saisonniers Ramadan)
