# US-2260 — Templates messages par pathologie

> 📌 **30. Communication & messagerie pro → patient** · Priorité **V2**
...
|---|---|
| **ID** | `US-2260` |
| **Domaine** | 30. Communication & messagerie pro → patient |
...
Bibliothèque de messages pré-rédigés (T1, T2, gestationnel, pédiatrique) pour gain de temps consultations : conseils, encouragements, rappels, explications standards.
...
Quand il tente d'accéder à « Templates messages par pathologie »
...
```gherkin
Étant donné Une action sur « Templates messages par pathologie » est effectuée
...
```
GET    /api/patients/[patientId]/templates-messages-par-pathologie                    Lecture configuration courante
PUT    /api/patients/[patientId]/templates-messages-par-pathologie                    Création nouvelle version
POST   /api/patients/[patientId]/templates-messages-par-pathologie/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/templates-messages-par-pathologie/history            Historique versions
POST   /api/patients/[patientId]/templates-messages-par-pathologie/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/templates-messages-par-pathologie.service.ts`.
