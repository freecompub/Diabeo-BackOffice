# US-2264 — Diffusion message à cohorte

> 📌 **30. Communication & messagerie pro → patient** · Priorité **V3**
...
Envoi d'un message à un groupe de patients (ex: tous les T1 du cabinet, ou tous les patients en mode Ramadan). Avec validation médicale et consentements vérifiés.
...
Quand il tente d'accéder à « Diffusion message à cohorte »
...
```gherkin
Étant donné Une action sur « Diffusion message à cohorte » est effectuée
...
```
POST   /api/orchestrations/diffusion-message-a-cohorte     Déclenchement manuel
GET    /api/orchestrations/diffusion-message-a-cohorte/log Logs récents
PATCH  /api/orchestrations/diffusion-message-a-cohorte/preferences  Préférences cabinet
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/diffusion-message-a-cohorte.service.ts`.
