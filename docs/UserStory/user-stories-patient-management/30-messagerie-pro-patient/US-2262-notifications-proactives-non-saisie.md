# US-2262 — Notifications proactives non-saisie
...
Configuration : si un patient n'a pas saisi de données depuis X jours, envoyer un message d'encouragement automatique (paramétrable par le médecin).
...
Quand il tente d'accéder à « Notifications proactives non-saisie »
...
```gherkin
Étant donné Une action sur « Notifications proactives non-saisie » est effectuée
...
```
POST   /api/orchestrations/notifications-proactives-non-saisie     Déclenchement manuel
GET    /api/orchestrations/notifications-proactives-non-saisie/log Logs récents
PATCH  /api/orchestrations/notifications-proactives-non-saisie/preferences  Préférences cabinet
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/notifications-proactives-non-saisie.service.ts`.
