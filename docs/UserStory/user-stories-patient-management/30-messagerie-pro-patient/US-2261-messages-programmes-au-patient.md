# US-2261 — Messages programmés au patient
...
Programmation de messages futurs : rappel pré-consultation, encouragement post-objectif atteint, vérification suite à ajustement. Conditions déclenchement paramétrables.
...
Quand il tente d'accéder à « Messages programmés au patient »
...
```gherkin
Étant donné Une action sur « Messages programmés au patient » est effectuée
...
```
POST   /api/orchestrations/messages-programmes-au-patient     Déclenchement manuel
GET    /api/orchestrations/messages-programmes-au-patient/log Logs récents
PATCH  /api/orchestrations/messages-programmes-au-patient/preferences  Préférences cabinet
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/messages-programmes-au-patient.service.ts`.
