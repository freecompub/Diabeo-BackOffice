# US-2216 — Configuration seuil cétones
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3303 (détection cétones) |
...
Paramétrage du seuil d'alerte cétonémie (par défaut 0.6 mmol/L) et procédure de stratification du risque DKA validée par le médecin.
...
**US miroir patient** : US-3303 (détection cétones)
...
Quand il tente d'accéder à « Configuration seuil cétones »
...
```gherkin
Étant donné Une action sur « Configuration seuil cétones » est effectuée
...
```
GET    /api/patients/[patientId]/configuration-seuil-cetones                    Lecture configuration courante
PUT    /api/patients/[patientId]/configuration-seuil-cetones                    Création nouvelle version
POST   /api/patients/[patientId]/configuration-seuil-cetones/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/configuration-seuil-cetones/history            Historique versions
POST   /api/patients/[patientId]/configuration-seuil-cetones/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/configuration-seuil-cetones.service.ts`.
...
- **Miroir app patient** : US-3303 (détection cétones)
