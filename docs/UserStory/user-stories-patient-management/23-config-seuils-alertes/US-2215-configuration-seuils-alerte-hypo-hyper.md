# US-2215 — Configuration seuils alerte hypo/hyper
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3280, US-3281 (détection seuils hypo) |
...
Le médecin paramètre les seuils d'alerte hypoglycémie (niveau 1 : <70, niveau 2 : <54) et hyperglycémie selon profil patient. Possibilité de seuils différents jour/nuit.
...
**US miroir patient** : US-3280, US-3281 (détection seuils hypo)
...
Quand il tente d'accéder à « Configuration seuils alerte hypo/hyper »
...
```gherkin
Étant donné Une action sur « Configuration seuils alerte hypo/hyper » est effectuée
...
```
GET    /api/patients/[patientId]/configuration-seuils-alerte-hypo-hyper                    Lecture configuration courante
PUT    /api/patients/[patientId]/configuration-seuils-alerte-hypo-hyper                    Création nouvelle version
POST   /api/patients/[patientId]/configuration-seuils-alerte-hypo-hyper/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/configuration-seuils-alerte-hypo-hyper/history            Historique versions
POST   /api/patients/[patientId]/configuration-seuils-alerte-hypo-hyper/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/configuration-seuils-alerte-hypo-hyper.service.ts`.
...
- **Miroir app patient** : US-3280, US-3281 (détection seuils hypo)
