# US-2217 — Validation protocole resucrage personnalisé
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3286 (choix resucrage perso) |
...
Le médecin valide le protocole de resucrage du patient : choix d'aliments (jus, gel, comprimés), quantités selon glycémie, prise en compte allergies/préférences/Ramadan.
...
**US miroir patient** : US-3286 (choix resucrage perso)
...
Quand il tente d'accéder à « Validation protocole resucrage personnalisé »
...
```gherkin
Étant donné Une action sur « Validation protocole resucrage personnalisé » est effectuée
...
```
GET    /api/patients/[patientId]/validation-protocole-resucrage-personnalise                    Lecture configuration courante
PUT    /api/patients/[patientId]/validation-protocole-resucrage-personnalise                    Création nouvelle version
POST   /api/patients/[patientId]/validation-protocole-resucrage-personnalise/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/validation-protocole-resucrage-personnalise/history            Historique versions
POST   /api/patients/[patientId]/validation-protocole-resucrage-personnalise/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/validation-protocole-resucrage-personnalise.service.ts`.
...
- **Miroir app patient** : US-3286 (choix resucrage perso)
