# US-2220 — Templates de seuils par profil
...
Bibliothèque de templates de seuils selon profil patient (T1 adulte stable, T1 ado, T2 sous insuline, gestationnel, pédiatrique). Application en un clic puis ajustement fin.
...
Quand il tente d'accéder à « Templates de seuils par profil »
...
```gherkin
Étant donné Une action sur « Templates de seuils par profil » est effectuée
...
```
GET    /api/patients/[patientId]/templates-de-seuils-par-profil                    Lecture configuration courante
PUT    /api/patients/[patientId]/templates-de-seuils-par-profil                    Création nouvelle version
POST   /api/patients/[patientId]/templates-de-seuils-par-profil/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/templates-de-seuils-par-profil/history            Historique versions
POST   /api/patients/[patientId]/templates-de-seuils-par-profil/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/templates-de-seuils-par-profil.service.ts`.
