# US-2214 — Configuration cibles glycémiques par patient

> 📌 **23. Configuration seuils & alertes patient** · Priorité **MVP**
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3174 (cibles glycémiques perso) |
...
Le médecin définit les cibles glycémiques personnalisées (jeun, post-prandial 1h, post-prandial 2h, nocturne) pour chaque patient. Cibles validées médicalement, traçabilité complète.
...
**US miroir patient** : US-3174 (cibles glycémiques perso)
...
Quand il tente d'accéder à « Configuration cibles glycémiques par patient »
...
```gherkin
Étant donné Une action sur « Configuration cibles glycémiques par patient » est effectuée
...
```
GET    /api/patients/[patientId]/configuration-cibles-glycemiques-par-patient                    Lecture configuration courante
PUT    /api/patients/[patientId]/configuration-cibles-glycemiques-par-patient                    Création nouvelle version
POST   /api/patients/[patientId]/configuration-cibles-glycemiques-par-patient/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/configuration-cibles-glycemiques-par-patient/history            Historique versions
POST   /api/patients/[patientId]/configuration-cibles-glycemiques-par-patient/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/configuration-cibles-glycemiques-par-patient.service.ts`.
...
- **Miroir app patient** : US-3174 (cibles glycémiques perso)
