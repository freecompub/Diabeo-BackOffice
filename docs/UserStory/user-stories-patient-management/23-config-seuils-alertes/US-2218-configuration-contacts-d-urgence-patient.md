# US-2218 — Configuration contacts d'urgence patient

> 📌 **23. Configuration seuils & alertes patient** · Priorité **V1**
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3331 (perso contacts urgence) |
...
Le médecin peut consulter et co-configurer (avec le patient) les contacts d'urgence : jusqu'à 5 contacts hiérarchisés, avec règles d'escalade. Modification soumise au consentement patient.
...
**US miroir patient** : US-3331 (perso contacts urgence)
...
Quand il tente d'accéder à « Configuration contacts d'urgence patient »
...
```gherkin
Étant donné Une action sur « Configuration contacts d'urgence patient » est effectuée
...
```
GET    /api/patients/[patientId]/configuration-contacts-d-urgence-patient                    Lecture configuration courante
PUT    /api/patients/[patientId]/configuration-contacts-d-urgence-patient                    Création nouvelle version
POST   /api/patients/[patientId]/configuration-contacts-d-urgence-patient/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/configuration-contacts-d-urgence-patient/history            Historique versions
POST   /api/patients/[patientId]/configuration-contacts-d-urgence-patient/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/configuration-contacts-d-urgence-patient.service.ts`.
...
- **Miroir app patient** : US-3331 (perso contacts urgence)
