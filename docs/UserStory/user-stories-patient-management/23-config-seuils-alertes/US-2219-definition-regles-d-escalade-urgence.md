# US-2219 — Définition règles d'escalade urgence
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3291 (alerte auto contacts) |
...
Configuration de la chaîne d'escalade : patient → contact 1 → contact 2 → médecin référent → SAMU. Délais d'attente entre chaque niveau paramétrables.
...
**US miroir patient** : US-3291 (alerte auto contacts)
...
Quand il tente d'accéder à « Définition règles d'escalade urgence »
...
```gherkin
Étant donné Une action sur « Définition règles d'escalade urgence » est effectuée
...
```
GET    /api/patients/[patientId]/definition-regles-d-escalade-urgence                    Lecture configuration courante
PUT    /api/patients/[patientId]/definition-regles-d-escalade-urgence                    Création nouvelle version
POST   /api/patients/[patientId]/definition-regles-d-escalade-urgence/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/definition-regles-d-escalade-urgence/history            Historique versions
POST   /api/patients/[patientId]/definition-regles-d-escalade-urgence/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/definition-regles-d-escalade-urgence.service.ts`.
...
- **Miroir app patient** : US-3291 (alerte auto contacts)
