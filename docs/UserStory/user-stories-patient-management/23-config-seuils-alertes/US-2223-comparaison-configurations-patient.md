# US-2223 — Comparaison configurations patient
...
Vue side-by-side de la configuration actuelle vs templates standards ou vs cohorte similaire, pour repérer les outliers.
...
Quand il tente d'accéder à « Comparaison configurations patient »
...
```gherkin
Étant donné Une action sur « Comparaison configurations patient » est effectuée
...
```
GET    /api/cohort/comparaison-configurations-patient            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/comparaison-configurations-patient    Vue détaillée patient
POST   /api/patients/[patientId]/comparaison-configurations-patient/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/comparaison-configurations-patient.service.ts`.
