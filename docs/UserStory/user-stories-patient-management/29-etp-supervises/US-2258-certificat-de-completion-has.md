# US-2258 — Certificat de complétion HAS
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3245 (certificat patient) |
...
Génération automatique d'un certificat conforme aux exigences HAS, signé par le médecin coordinateur ETP. Téléchargeable patient + archivage cabinet.
...
**US miroir patient** : US-3245 (certificat patient)
...
Quand il tente d'accéder à « Certificat de complétion HAS »
...
```gherkin
Étant donné Une action sur « Certificat de complétion HAS » est effectuée
...
```
GET    /api/admin/audit/certificat-de-completion-has        Lecture (filtres)
POST   /api/admin/audit/certificat-de-completion-has/export Demande export
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/certificat-de-completion-has.service.ts`.
...
- **Miroir app patient** : US-3245 (certificat patient)
