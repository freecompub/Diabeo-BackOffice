# US-2254 — Bibliothèque programmes ETP
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3243 (programmes ETP patient) |
...
Catalogue de programmes d'éducation thérapeutique structurés selon cadre HAS : insulinothérapie initiation, diabète gestationnel, équilibre alimentaire, sport et diabète.
...
**US miroir patient** : US-3243 (programmes ETP patient)
...
Quand il tente d'accéder à « Bibliothèque programmes ETP »
...
```gherkin
Étant donné Une action sur « Bibliothèque programmes ETP » est effectuée
...
```
GET    /api/patients/[patientId]/bibliotheque-programmes-etp                    Lecture configuration courante
PUT    /api/patients/[patientId]/bibliotheque-programmes-etp                    Création nouvelle version
POST   /api/patients/[patientId]/bibliotheque-programmes-etp/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/bibliotheque-programmes-etp/history            Historique versions
POST   /api/patients/[patientId]/bibliotheque-programmes-etp/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/bibliotheque-programmes-etp.service.ts`.
...
- **Miroir app patient** : US-3243 (programmes ETP patient)
