# US-2256 — Suivi progression patient dans programme
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3244 (suivi progression) |
...
Tableau de bord temps réel : modules complétés, quiz validés, temps passé, blocages identifiés. Aide à la consultation de suivi ETP.
...
**US miroir patient** : US-3244 (suivi progression)
...
Quand il tente d'accéder à « Suivi progression patient dans programme »
...
```gherkin
Étant donné Une action sur « Suivi progression patient dans programme » est effectuée
...
```
GET    /api/cohort/suivi-progression-patient-dans-programme            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/suivi-progression-patient-dans-programme    Vue détaillée patient
POST   /api/patients/[patientId]/suivi-progression-patient-dans-programme/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/suivi-progression-patient-dans-programme.service.ts`.
...
- **Miroir app patient** : US-3244 (suivi progression)
