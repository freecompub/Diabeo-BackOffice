...
Vue croisée automatique : pour chaque repas, glycémie pré et post, bolus pris, comparaison à la prédiction. Aide à l'ajustement IC.
...
Quand il tente d'accéder à « Contextualisation glycémie ↔ repas »
...
```gherkin
Étant donné Une action sur « Contextualisation glycémie ↔ repas » est effectuée
...
```
GET    /api/cohort/contextualisation-glycemie-repas            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/contextualisation-glycemie-repas    Vue détaillée patient
POST   /api/patients/[patientId]/contextualisation-glycemie-repas/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/contextualisation-glycemie-repas.service.ts`.
