# US-2248 — Vue journal alimentaire patient
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3072 (saisie repas) |
...
Consultation du journal repas du patient : aliments, quantités, glucides estimés, photos. Avec corrélation glycémie post-prandiale pour analyse.
...
**US miroir patient** : US-3072 (saisie repas)
...
Quand il tente d'accéder à « Vue journal alimentaire patient »
...
```gherkin
Étant donné Une action sur « Vue journal alimentaire patient » est effectuée
...
```
GET    /api/cohort/vue-journal-alimentaire-patient            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/vue-journal-alimentaire-patient    Vue détaillée patient
POST   /api/patients/[patientId]/vue-journal-alimentaire-patient/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/vue-journal-alimentaire-patient.service.ts`.
...
- **Miroir app patient** : US-3072 (saisie repas)
