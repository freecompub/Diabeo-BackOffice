# US-2249 — Détection patterns alimentaires problématiques

> 📌 **28. Supervision repas & adhésion thérapeutique** · Priorité **V2**
...
Algorithme qui détecte les patterns à risque : repas sans bolus, glucides systématiquement sous-estimés, prises alimentaires nocturnes, etc.
...
Quand il tente d'accéder à « Détection patterns alimentaires problématiques »
...
```gherkin
Étant donné Une action sur « Détection patterns alimentaires problématiques » est effectuée
...
```
GET    /api/cohort/detection-patterns-alimentaires-problematiques            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/detection-patterns-alimentaires-problematiques    Vue détaillée patient
POST   /api/patients/[patientId]/detection-patterns-alimentaires-problematiques/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/detection-patterns-alimentaires-problematiques.service.ts`.
