# US-2227 — Bilan trimestriel urgences par patient

> 📌 **24. Supervision urgences déclenchées** · Priorité **V1**
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3336 (bilan trimestriel) |
...
Synthèse automatique trimestrielle : nombre d'hypos par niveau, DKA, durée moyenne, patterns horaires. Téléchargeable PDF, partageable avec le patient lors de consultation.
...
**US miroir patient** : US-3336 (bilan trimestriel)
...
Quand il tente d'accéder à « Bilan trimestriel urgences par patient »
...
```gherkin
Étant donné Une action sur « Bilan trimestriel urgences par patient » est effectuée
...
```
GET    /api/cohort/bilan-trimestriel-urgences-par-patient            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/bilan-trimestriel-urgences-par-patient    Vue détaillée patient
POST   /api/patients/[patientId]/bilan-trimestriel-urgences-par-patient/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/bilan-trimestriel-urgences-par-patient.service.ts`.
...
- **Miroir app patient** : US-3336 (bilan trimestriel)
