# US-2225 — Détail timeline d'une urgence
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3333 (journal urgences) |
...
Vue minute par minute d'une urgence : déclenchement, étapes suivies par le patient, glycémies pendant, fin (succès/SAMU/abandon). Avec contexte (repas, activité, autres événements).
...
**US miroir patient** : US-3333 (journal urgences)
...
Quand il tente d'accéder à « Détail timeline d'une urgence »
...
```gherkin
Étant donné Une action sur « Détail timeline d'une urgence » est effectuée
...
```
GET    /api/cohort/detail-timeline-d-une-urgence            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/detail-timeline-d-une-urgence    Vue détaillée patient
POST   /api/patients/[patientId]/detail-timeline-d-une-urgence/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/detail-timeline-d-une-urgence.service.ts`.
...
- **Miroir app patient** : US-3333 (journal urgences)
