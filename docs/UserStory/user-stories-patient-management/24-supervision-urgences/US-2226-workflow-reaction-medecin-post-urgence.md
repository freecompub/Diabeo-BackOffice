# US-2226 — Workflow réaction médecin post-urgence

> 📌 **24. Supervision urgences déclenchées** · Priorité **MVP**
...
Après une urgence, le médecin peut : appeler le patient, créer une proposition d'ajustement, planifier une consultation, classer comme résolue. Toutes actions auditées.
...
Quand il tente d'accéder à « Workflow réaction médecin post-urgence »
...
```gherkin
Étant donné Une action sur « Workflow réaction médecin post-urgence » est effectuée
...
```
POST   /api/patients/[patientId]/workflows/workflow-reaction-medecin-post-urgence     Initier workflow
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/workflow-reaction-medecin-post-urgence.service.ts`.
