# US-2228 — Statistiques cohorte urgences
...
Indicateurs agrégés à l'échelle du cabinet : taux d'hypos sévères pour 1000 patient-jours, fréquence DKA, comparaison à moyennes nationales (anonymisées).
...
Quand il tente d'accéder à « Statistiques cohorte urgences »
...
```gherkin
Étant donné Une action sur « Statistiques cohorte urgences » est effectuée
...
```
GET    /api/cohort/statistiques-cohorte-urgences            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/statistiques-cohorte-urgences    Vue détaillée patient
POST   /api/patients/[patientId]/statistiques-cohorte-urgences/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/statistiques-cohorte-urgences.service.ts`.
