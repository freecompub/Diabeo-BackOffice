# US-2230 — Notification temps réel urgence patient
...
Push notification au médecin référent quand un patient déclenche une urgence critique (DKA, hypo sévère). Permet réaction sous 10-15 minutes en heures ouvrées.
...
Quand il tente d'accéder à « Notification temps réel urgence patient »
...
```gherkin
Étant donné Une action sur « Notification temps réel urgence patient » est effectuée
...
```
POST   /api/orchestrations/notification-temps-reel-urgence-patient     Déclenchement manuel
GET    /api/orchestrations/notification-temps-reel-urgence-patient/log Logs récents
PATCH  /api/orchestrations/notification-temps-reel-urgence-patient/preferences  Préférences cabinet
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/notification-temps-reel-urgence-patient.service.ts`.
