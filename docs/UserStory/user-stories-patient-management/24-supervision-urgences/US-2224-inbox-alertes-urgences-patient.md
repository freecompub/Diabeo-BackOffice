# US-2224 — Inbox alertes urgences patient
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3334 (CR auto au médecin) |
...
Tableau de bord temps réel des urgences déclenchées par les patients du portefeuille du médecin : hypos sévères, DKA, alertes non-réponse, etc. Tri par criticité et fraîcheur.
...
**US miroir patient** : US-3334 (CR auto au médecin)
...
Quand il tente d'accéder à « Inbox alertes urgences patient »
...
```gherkin
Étant donné Une action sur « Inbox alertes urgences patient » est effectuée
...
```
GET    /api/cohort/inbox-alertes-urgences-patient            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/inbox-alertes-urgences-patient    Vue détaillée patient
POST   /api/patients/[patientId]/inbox-alertes-urgences-patient/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/inbox-alertes-urgences-patient.service.ts`.
...
- **Miroir app patient** : US-3334 (CR auto au médecin)
