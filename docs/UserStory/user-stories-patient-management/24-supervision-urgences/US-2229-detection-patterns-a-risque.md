# US-2229 — Détection patterns à risque
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3308, US-3309 (détection répétée) |
...
Algorithme qui détecte les patients à risque : hypos répétées, sous-déclaration suspecte, DKA antérieurs. Alerte proactive au médecin pour intervention préventive.
...
**US miroir patient** : US-3308, US-3309 (détection répétée)
...
Quand il tente d'accéder à « Détection patterns à risque »
...
```gherkin
Étant donné Une action sur « Détection patterns à risque » est effectuée
...
```
GET    /api/cohort/detection-patterns-a-risque            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/detection-patterns-a-risque    Vue détaillée patient
POST   /api/patients/[patientId]/detection-patterns-a-risque/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/detection-patterns-a-risque.service.ts`.
...
- **Miroir app patient** : US-3308, US-3309 (détection répétée)
