# US-2242 — Notifications partagées multi-aidants

> 📌 **26. Gestion aidants & partages** · Priorité **V1**
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3222 (notifs partagées) |
...
Configuration côté médecin : quel type d'alerte est envoyée à quel aidant. Permet d'éviter de submerger le contact secondaire avec des notifications mineures.
...
**US miroir patient** : US-3222 (notifs partagées)
...
Quand il tente d'accéder à « Notifications partagées multi-aidants »
...
```gherkin
Étant donné Une action sur « Notifications partagées multi-aidants » est effectuée
...
```
GET    /api/patients/[patientId]/notifications-partagees-multi-aidants                    Lecture configuration courante
PUT    /api/patients/[patientId]/notifications-partagees-multi-aidants                    Création nouvelle version
POST   /api/patients/[patientId]/notifications-partagees-multi-aidants/validate           Validation par DOCTOR si créée par NURSE
GET    /api/patients/[patientId]/notifications-partagees-multi-aidants/history            Historique versions
POST   /api/patients/[patientId]/notifications-partagees-multi-aidants/restore/[versionId]   Restauration d'une version antérieure
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/notifications-partagees-multi-aidants.service.ts`.
...
- **Miroir app patient** : US-3222 (notifs partagées)
