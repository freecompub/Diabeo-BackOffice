# US-2241 — Révocation forcée partage
...
Cas exceptionnels (suspicion violence, perte de confiance) : le médecin peut, en cas d'urgence, révoquer un partage avec justification écrite. Audit lourd, notification patient.
...
Quand il tente d'accéder à « Révocation forcée partage »
...
```gherkin
Étant donné Une action sur « Révocation forcée partage » est effectuée
...
```
POST   /api/patients/[patientId]/workflows/revocation-forcee-partage     Initier workflow
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/revocation-forcee-partage.service.ts`.
