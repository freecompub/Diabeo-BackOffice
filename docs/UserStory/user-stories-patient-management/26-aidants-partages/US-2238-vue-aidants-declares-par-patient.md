# US-2238 — Vue aidants déclarés par patient
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3220 (invitation aidant) |
...
Le médecin voit qui a accès aux données de son patient : aidants permanents (parents), partages temporaires (école), partages ponctuels (lien expirant).
...
**US miroir patient** : US-3220 (invitation aidant)
...
Quand il tente d'accéder à « Vue aidants déclarés par patient »
...
```gherkin
Étant donné Une action sur « Vue aidants déclarés par patient » est effectuée
...
```
GET    /api/cohort/vue-aidants-declares-par-patient            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/vue-aidants-declares-par-patient    Vue détaillée patient
POST   /api/patients/[patientId]/vue-aidants-declares-par-patient/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/vue-aidants-declares-par-patient.service.ts`.
...
- **Miroir app patient** : US-3220 (invitation aidant)
