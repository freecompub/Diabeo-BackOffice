# US-2243 — Vue dispositifs CGM/pompe/lecteur patient

> 📌 **27. Gestion dispositifs patient (vue pro)** · Priorité **V1**
...
|---|---|
| **ID** | `US-2243` |
| **Domaine** | 27. Gestion dispositifs patient (vue pro) |
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3110 (statut dispositif patient) |
...
Tableau des dispositifs déclarés par le patient avec statut : modèle, n° série, dernière sync, état pile, expiration capteur. Vue par patient et vue cohorte.
...
**US miroir patient** : US-3110 (statut dispositif patient)
...
Quand il tente d'accéder à « Vue dispositifs CGM/pompe/lecteur patient »
...
```gherkin
Étant donné Une action sur « Vue dispositifs CGM/pompe/lecteur patient » est effectuée
...
```
GET    /api/cohort/vue-dispositifs-cgm-pompe-lecteur-patient            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/vue-dispositifs-cgm-pompe-lecteur-patient    Vue détaillée patient
POST   /api/patients/[patientId]/vue-dispositifs-cgm-pompe-lecteur-patient/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/vue-dispositifs-cgm-pompe-lecteur-patient.service.ts`.
...
- **Miroir app patient** : US-3110 (statut dispositif patient)
