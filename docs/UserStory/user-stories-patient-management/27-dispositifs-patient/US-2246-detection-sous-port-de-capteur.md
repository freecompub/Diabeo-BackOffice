# US-2246 — Détection sous-port de capteur
...
Identification des patients qui ne portent pas leur CGM régulièrement (% temps sans capteur). Ressource clé pour identifier qui rappeler en consultation.
...
Quand il tente d'accéder à « Détection sous-port de capteur »
...
```gherkin
Étant donné Une action sur « Détection sous-port de capteur » est effectuée
...
```
GET    /api/cohort/detection-sous-port-de-capteur            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/detection-sous-port-de-capteur    Vue détaillée patient
POST   /api/patients/[patientId]/detection-sous-port-de-capteur/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/detection-sous-port-de-capteur.service.ts`.
