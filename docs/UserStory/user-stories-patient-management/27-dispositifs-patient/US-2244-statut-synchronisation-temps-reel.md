# US-2244 — Statut synchronisation temps réel
...
Indicateur live de la fraîcheur des données pour chaque patient : sync OK (< 5 min), retard (5-30 min), critique (> 30 min sans sync). Filtre cohorte.
...
Quand il tente d'accéder à « Statut synchronisation temps réel »
...
```gherkin
Étant donné Une action sur « Statut synchronisation temps réel » est effectuée
...
```
GET    /api/cohort/statut-synchronisation-temps-reel            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/statut-synchronisation-temps-reel    Vue détaillée patient
POST   /api/patients/[patientId]/statut-synchronisation-temps-reel/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/statut-synchronisation-temps-reel.service.ts`.
