# US-2245 — Détection dispositifs défaillants

> 📌 **27. Gestion dispositifs patient (vue pro)** · Priorité **V2**
...
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-2001 (Login JWT), US-2011 (Audit log immuable), US-2012 (RBAC 4 rôles), US-2018 (Fiche patient), Miroir app patient : US-3043 (alerte capteur défaillant) |
...
Algorithme qui repère un capteur en chute, une pompe avec occlusions répétées, un lecteur qui ne synchronise plus. Alerte le médecin pour intervention.
...
**US miroir patient** : US-3043 (alerte capteur défaillant)
...
Quand il tente d'accéder à « Détection dispositifs défaillants »
...
```gherkin
Étant donné Une action sur « Détection dispositifs défaillants » est effectuée
...
```
GET    /api/cohort/detection-dispositifs-defaillants            Vue cohorte (cabinet)
GET    /api/patients/[patientId]/detection-dispositifs-defaillants    Vue détaillée patient
POST   /api/patients/[patientId]/detection-dispositifs-defaillants/refresh    Force refresh
```
...
### Validation Zod
Schémas Zod stricts sur tous les inputs. Voir `src/lib/services/detection-dispositifs-defaillants.service.ts`.
...
- **Miroir app patient** : US-3043 (alerte capteur défaillant)
