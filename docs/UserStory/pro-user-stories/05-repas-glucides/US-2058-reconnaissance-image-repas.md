# US-2058 — Reconnaissance image repas

> 📌 **5. Repas & glucides** · Priorité **V4** · Pays **Universel**
> 
> 💬 **Note inventaire** : Brique IA optionnelle
...
|---|---|
| **ID** | `US-2058` |
| **Référence inventaire** | `FN-058` |
| **Domaine** | 5. Repas & glucides |
| **Priorité** | **V4** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Foodvisor / LogMeal / API IA maison |
...
### 🌐 Intégration externe — Foodvisor / LogMeal / API IA maison
...
Quand il accède à la fonctionnalité « Reconnaissance image repas »
...
Quand il tente d'accéder à « Reconnaissance image repas »
...
```gherkin
Étant donné Le service externe « Foodvisor / LogMeal / API IA maison » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Foodvisor / LogMeal / API IA maison » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name reconnaissance-image-repas`
...
export const reconnaissance_image_repasSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ReconnaissanceimagerepasInput = z.infer<typeof reconnaissance_image_repasSchema>
```
...
```bash
pnpm test src/lib/services/reconnaissance-image-repas.test.ts
```
...
- [ ] Un appel à « Foodvisor / LogMeal / API IA maison » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/reconnaissance-image-repas.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/reconnaissance-image-repas.spec.ts
```
...
- Documentation officielle de **Foodvisor / LogMeal / API IA maison** (à vérifier au démarrage)
