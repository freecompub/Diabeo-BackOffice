# US-2054 — Bibliothèque aliments France

> 📌 **5. Repas & glucides** · Priorité **V1** · Pays **FR**
> 
> 💬 **Note inventaire** : ANSES = source officielle
...
|---|---|
| **ID** | `US-2054` |
| **Référence inventaire** | `FN-054` |
| **Domaine** | 5. Repas & glucides |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | CIQUAL (ANSES) / Open Food Facts |
| **Modèle économique** | Gratuit (open data) |
| **Coût estimé** | Gratuit |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — CIQUAL (ANSES) / Open Food Facts
...
Quand il accède à la fonctionnalité « Bibliothèque aliments France »
...
Quand il tente d'accéder à « Bibliothèque aliments France »
...
```gherkin
Étant donné Le service externe « CIQUAL (ANSES) / Open Food Facts » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « CIQUAL (ANSES) / Open Food Facts » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name bibliotheque-aliments-france`
...
export const bibliotheque_aliments_franceSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type BibliothequealimentsfranceInput = z.infer<typeof bibliotheque_aliments_franceSchema>
```
...
```bash
pnpm test src/lib/services/bibliotheque-aliments-france.test.ts
```
...
- [ ] Un appel à « CIQUAL (ANSES) / Open Food Facts » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/bibliotheque-aliments-france.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/bibliotheque-aliments-france.spec.ts
```
...
- Documentation officielle de **CIQUAL (ANSES) / Open Food Facts** (à vérifier au démarrage)
