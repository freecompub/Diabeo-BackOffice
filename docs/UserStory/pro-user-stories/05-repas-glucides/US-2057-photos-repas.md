# US-2057 — Photos repas

> 📌 **5. Repas & glucides** · Priorité **V1** · Pays **Universel**
> 
> 💬 **Note inventaire** : MinIO auto-hébergé OK
...
### 🌐 Intégration externe — OVH Object Storage / MinIO
...
Quand il accède à la fonctionnalité « Photos repas »
...
Quand il tente d'accéder à « Photos repas »
...
```gherkin
Étant donné Le service externe « OVH Object Storage / MinIO » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « OVH Object Storage / MinIO » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name photos-repas`
...
export const photos_repasSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PhotosrepasInput = z.infer<typeof photos_repasSchema>
```
...
```bash
pnpm test src/lib/services/photos-repas.test.ts
```
...
- [ ] Un appel à « OVH Object Storage / MinIO » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/photos-repas.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/photos-repas.spec.ts
```
...
- Documentation officielle de **OVH Object Storage / MinIO** (à vérifier au démarrage)
