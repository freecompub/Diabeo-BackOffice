# US-2056 — Comptage glucides assisté

> 📌 **5. Repas & glucides** · Priorité **V2** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Comptage glucides assisté »
...
Quand il tente d'accéder à « Comptage glucides assisté »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name comptage-glucides-assiste`
...
export const comptage_glucides_assisteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ComptageglucidesassisteInput = z.infer<typeof comptage_glucides_assisteSchema>
```
...
```bash
pnpm test src/lib/services/comptage-glucides-assiste.test.ts
```
...
```bash
pnpm test:integration tests/integration/comptage-glucides-assiste.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/comptage-glucides-assiste.spec.ts
```
