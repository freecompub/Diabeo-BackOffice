# US-2062 — Impact glycémique effort

> 📌 **6. Activité physique** · Priorité **V2** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Impact glycémique effort »
...
Quand il tente d'accéder à « Impact glycémique effort »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name impact-glycemique-effort`
...
export const impact_glycemique_effortSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ImpactglycemiqueeffortInput = z.infer<typeof impact_glycemique_effortSchema>
```
...
```bash
pnpm test src/lib/services/impact-glycemique-effort.test.ts
```
...
```bash
pnpm test:integration tests/integration/impact-glycemique-effort.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/impact-glycemique-effort.spec.ts
```
