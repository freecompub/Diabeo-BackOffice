# US-2060 — Apple HealthKit

> 📌 **6. Activité physique** · Priorité **V1** · Pays **FR+DZ**
> 
> 💬 **Note inventaire** : Côté app iOS, pas backoffice
...
|---|---|
| **ID** | `US-2060` |
| **Référence inventaire** | `FN-060` |
| **Domaine** | 6. Activité physique |
| **Priorité** | **V1** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Oui |
| **Service / Standard** | Apple HealthKit SDK |
| **Modèle économique** | Gratuit |
| **Coût estimé** | Frais Apple Dev (~99$/an) |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Apple HealthKit SDK
...
Quand il accède à la fonctionnalité « Apple HealthKit »
...
Quand il tente d'accéder à « Apple HealthKit »
...
```gherkin
Étant donné Le service externe « Apple HealthKit SDK » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Apple HealthKit SDK » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name apple-healthkit`
...
export const apple_healthkitSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ApplehealthkitInput = z.infer<typeof apple_healthkitSchema>
```
...
```bash
pnpm test src/lib/services/apple-healthkit.test.ts
```
...
- [ ] Un appel à « Apple HealthKit SDK » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/apple-healthkit.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/apple-healthkit.spec.ts
```
...
- Documentation officielle de **Apple HealthKit SDK** (à vérifier au démarrage)
