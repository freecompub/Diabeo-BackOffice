# US-2059 — Journal activité

> 📌 **6. Activité physique** · Priorité **V1** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Journal activité »
...
Quand il tente d'accéder à « Journal activité »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name journal-activite`
...
export const journal_activiteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type JournalactiviteInput = z.infer<typeof journal_activiteSchema>
```
...
```bash
pnpm test src/lib/services/journal-activite.test.ts
```
...
```bash
pnpm test:integration tests/integration/journal-activite.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/journal-activite.spec.ts
```
