# US-2061 — Google Fit / Health Connect

> 📌 **6. Activité physique** · Priorité **V1** · Pays **FR+DZ**
> 
> 💬 **Note inventaire** : Côté app Android
...
|---|---|
| **ID** | `US-2061` |
| **Référence inventaire** | `FN-061` |
| **Domaine** | 6. Activité physique |
| **Priorité** | **V1** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Oui |
| **Service / Standard** | Google Fit API / Health Connect |
...
### 🌐 Intégration externe — Google Fit API / Health Connect
...
Quand il accède à la fonctionnalité « Google Fit / Health Connect »
...
Quand il tente d'accéder à « Google Fit / Health Connect »
...
```gherkin
Étant donné Le service externe « Google Fit API / Health Connect » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Google Fit API / Health Connect » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name google-fit-health-connect`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/patients/[id]/activity` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/patients/[id]/activity` | JWT | Selon AC-1 | Création |
| PUT     | `/api/patients/[id]/activity/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/patients/[id]/activity/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const google_fit_health_connectSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type GooglefithealthconnectInput = z.infer<typeof google_fit_health_connectSchema>
```
...
```bash
pnpm test src/lib/services/google-fit-health-connect.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/patients/[id]/activity` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/patients/[id]/activity` renvoie 401 sans JWT valide
- [ ] `/api/patients/[id]/activity` renvoie 403 avec un rôle insuffisant
- [ ] `/api/patients/[id]/activity` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Google Fit API / Health Connect » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/google-fit-health-connect.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/google-fit-health-connect.spec.ts
```
...
- Documentation officielle de **Google Fit API / Health Connect** (à vérifier au démarrage)
