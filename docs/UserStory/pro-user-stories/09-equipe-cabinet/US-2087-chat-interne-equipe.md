# US-2087 — Chat interne équipe

> 📌 **9. Équipe & cabinet** · Priorité **V2** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2087` |
| **Référence inventaire** | `FN-087` |
| **Domaine** | 9. Équipe & cabinet |
| **Priorité** | **V2** |
| **Pays cible** | Universel |
| **Intégration externe** | Partiel |
| **Service / Standard** | Interne ou Slack/Teams API |
| **Modèle économique** | Selon choix |
| **Coût estimé** | Selon outil |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Interne ou Slack/Teams API
...
Quand il accède à la fonctionnalité « Chat interne équipe »
...
Quand il tente d'accéder à « Chat interne équipe »
...
```gherkin
Étant donné Le service externe « Interne ou Slack/Teams API » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Interne ou Slack/Teams API » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name chat-interne-equipe`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/cabinets` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/cabinets` | JWT | Selon AC-1 | Création |
| PUT     | `/api/cabinets/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/cabinets/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const chat_interne_equipeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ChatinterneequipeInput = z.infer<typeof chat_interne_equipeSchema>
```
...
```bash
pnpm test src/lib/services/chat-interne-equipe.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/cabinets` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/cabinets` renvoie 401 sans JWT valide
- [ ] `/api/cabinets` renvoie 403 avec un rôle insuffisant
- [ ] `/api/cabinets` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Interne ou Slack/Teams API » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/chat-interne-equipe.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/chat-interne-equipe.spec.ts
```
...
- Documentation officielle de **Interne ou Slack/Teams API** (à vérifier au démarrage)
