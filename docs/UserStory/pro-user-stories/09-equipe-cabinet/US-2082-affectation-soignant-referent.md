# US-2082 — Affectation soignant référent

> 📌 **9. Équipe & cabinet** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Table existe
...
|---|---|
| **ID** | `US-2082` |
| **Référence inventaire** | `FN-082` |
| **Domaine** | 9. Équipe & cabinet |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (PatientReferent) |
...
Quand il accède à la fonctionnalité « Affectation soignant référent »
...
Quand il tente d'accéder à « Affectation soignant référent »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name affectation-soignant-referent`
...
export const affectation_soignant_referentSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AffectationsoignantreferentInput = z.infer<typeof affectation_soignant_referentSchema>
```
...
```bash
pnpm test src/lib/services/affectation-soignant-referent.test.ts
```
...
```bash
pnpm test:integration tests/integration/affectation-soignant-referent.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/affectation-soignant-referent.spec.ts
```
