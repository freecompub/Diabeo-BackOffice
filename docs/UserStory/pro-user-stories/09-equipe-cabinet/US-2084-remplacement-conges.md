...
Quand il accède à la fonctionnalité « Remplacement / congés »
...
Quand il tente d'accéder à « Remplacement / congés »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name remplacement-conges`
...
export const remplacement_congesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type RemplacementcongesInput = z.infer<typeof remplacement_congesSchema>
```
...
```bash
pnpm test src/lib/services/remplacement-conges.test.ts
```
...
```bash
pnpm test:integration tests/integration/remplacement-conges.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/remplacement-conges.spec.ts
```
