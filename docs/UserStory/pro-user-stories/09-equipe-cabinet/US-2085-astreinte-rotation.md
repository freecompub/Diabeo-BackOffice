...
Quand il accède à la fonctionnalité « Astreinte / rotation »
...
Quand il tente d'accéder à « Astreinte / rotation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name astreinte-rotation`
...
export const astreinte_rotationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AstreinterotationInput = z.infer<typeof astreinte_rotationSchema>
```
...
```bash
pnpm test src/lib/services/astreinte-rotation.test.ts
```
...
```bash
pnpm test:integration tests/integration/astreinte-rotation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/astreinte-rotation.spec.ts
```
