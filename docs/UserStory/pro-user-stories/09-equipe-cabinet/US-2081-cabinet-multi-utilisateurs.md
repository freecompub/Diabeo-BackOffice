# US-2081 — Cabinet multi-utilisateurs

> 📌 **9. Équipe & cabinet** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : HealthcareService existe
...
Plusieurs PS partagent patients
...
Quand il accède à la fonctionnalité « Cabinet multi-utilisateurs »
...
Quand il tente d'accéder à « Cabinet multi-utilisateurs »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name cabinet-multi-utilisateurs`
...
export const cabinet_multi_utilisateursSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CabinetmultiutilisateursInput = z.infer<typeof cabinet_multi_utilisateursSchema>
```
...
```bash
pnpm test src/lib/services/cabinet-multi-utilisateurs.test.ts
```
...
```bash
pnpm test:integration tests/integration/cabinet-multi-utilisateurs.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/cabinet-multi-utilisateurs.spec.ts
```
