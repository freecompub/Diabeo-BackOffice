# US-2083 — Délégation médecin → IDE
...
Quand il accède à la fonctionnalité « Délégation médecin → IDE »
...
Quand il tente d'accéder à « Délégation médecin → IDE »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name delegation-medecin-ide`
...
export const delegation_medecin_ideSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DelegationmedecinideInput = z.infer<typeof delegation_medecin_ideSchema>
```
...
```bash
pnpm test src/lib/services/delegation-medecin-ide.test.ts
```
...
```bash
pnpm test:integration tests/integration/delegation-medecin-ide.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/delegation-medecin-ide.spec.ts
```
