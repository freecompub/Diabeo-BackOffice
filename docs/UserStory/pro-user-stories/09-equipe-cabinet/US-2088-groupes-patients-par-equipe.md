# US-2088 — Groupes patients par équipe

> 📌 **9. Équipe & cabinet** · Priorité **V1** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Groupes patients par équipe »
...
Quand il tente d'accéder à « Groupes patients par équipe »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name groupes-patients-par-equipe`
...
export const groupes_patients_par_equipeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type GroupespatientsparequipeInput = z.infer<typeof groupes_patients_par_equipeSchema>
```
...
```bash
pnpm test src/lib/services/groupes-patients-par-equipe.test.ts
```
...
```bash
pnpm test:integration tests/integration/groupes-patients-par-equipe.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/groupes-patients-par-equipe.spec.ts
```
