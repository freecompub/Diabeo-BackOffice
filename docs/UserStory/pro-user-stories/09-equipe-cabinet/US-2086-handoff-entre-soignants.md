...
Quand il accède à la fonctionnalité « Handoff entre soignants »
...
Quand il tente d'accéder à « Handoff entre soignants »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name handoff-entre-soignants`
...
export const handoff_entre_soignantsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type HandoffentresoignantsInput = z.infer<typeof handoff_entre_soignantsSchema>
```
...
```bash
pnpm test src/lib/services/handoff-entre-soignants.test.ts
```
...
```bash
pnpm test:integration tests/integration/handoff-entre-soignants.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/handoff-entre-soignants.spec.ts
```
