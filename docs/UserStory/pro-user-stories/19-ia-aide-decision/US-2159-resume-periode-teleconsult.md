# US-2159 — Résumé période téléconsult

> 📌 **19. IA & aide décision** · Priorité **V2** · Pays **Universel**
> 
...
### 🌐 Intégration externe — Anthropic Claude API / OpenAI
...
Quand il accède à la fonctionnalité « Résumé période téléconsult »
...
Quand il tente d'accéder à « Résumé période téléconsult »
...
```gherkin
Étant donné Le service externe « Anthropic Claude API / OpenAI » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Anthropic Claude API / OpenAI » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name resume-periode-teleconsult`
...
export const resume_periode_teleconsultSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ResumeperiodeteleconsultInput = z.infer<typeof resume_periode_teleconsultSchema>
```
...
```bash
pnpm test src/lib/services/resume-periode-teleconsult.test.ts
```
...
- [ ] Un appel à « Anthropic Claude API / OpenAI » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/resume-periode-teleconsult.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/resume-periode-teleconsult.spec.ts
```
...
- Documentation officielle de **Anthropic Claude API / OpenAI** (à vérifier au démarrage)
