# US-2158 — Détection anomalies port capteur
...
Quand il accède à la fonctionnalité « Détection anomalies port capteur »
...
Quand il tente d'accéder à « Détection anomalies port capteur »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name detection-anomalies-port-capteur`
...
export const detection_anomalies_port_capteurSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DetectionanomaliesportcapteurInput = z.infer<typeof detection_anomalies_port_capteurSchema>
```
...
```bash
pnpm test src/lib/services/detection-anomalies-port-capteur.test.ts
```
...
```bash
pnpm test:integration tests/integration/detection-anomalies-port-capteur.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/detection-anomalies-port-capteur.spec.ts
```
