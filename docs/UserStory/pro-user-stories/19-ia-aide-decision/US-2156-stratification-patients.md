# US-2156 — Stratification patients
...
Quand il accède à la fonctionnalité « Stratification patients »
...
Quand il tente d'accéder à « Stratification patients »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name stratification-patients`
...
export const stratification_patientsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type StratificationpatientsInput = z.infer<typeof stratification_patientsSchema>
```
...
```bash
pnpm test src/lib/services/stratification-patients.test.ts
```
...
```bash
pnpm test:integration tests/integration/stratification-patients.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/stratification-patients.spec.ts
```
