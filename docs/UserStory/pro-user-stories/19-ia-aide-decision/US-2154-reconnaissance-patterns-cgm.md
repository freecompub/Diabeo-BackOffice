# US-2154 — Reconnaissance patterns CGM
...
Quand il accède à la fonctionnalité « Reconnaissance patterns CGM »
...
Quand il tente d'accéder à « Reconnaissance patterns CGM »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name reconnaissance-patterns-cgm`
...
export const reconnaissance_patterns_cgmSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ReconnaissancepatternscgmInput = z.infer<typeof reconnaissance_patterns_cgmSchema>
```
...
```bash
pnpm test src/lib/services/reconnaissance-patterns-cgm.test.ts
```
...
```bash
pnpm test:integration tests/integration/reconnaissance-patterns-cgm.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/reconnaissance-patterns-cgm.spec.ts
```
