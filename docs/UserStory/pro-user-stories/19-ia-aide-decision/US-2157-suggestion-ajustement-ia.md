# US-2157 — Suggestion ajustement IA

> 📌 **19. IA & aide décision** · Priorité **V3** · Pays **Universel**
> 
> 💬 **Note inventaire** : Marquage CE Dispositif Médical requis
...
|---|---|
| **ID** | `US-2157` |
| **Référence inventaire** | `FN-157` |
| **Domaine** | 19. IA & aide décision |
| **Priorité** | **V3** |
| **Pays cible** | Universel |
| **Intégration externe** | Partiel |
| **Service / Standard** | Anthropic / OpenAI ou modèle maison |
| **Modèle économique** | Payant à l'usage |
| **Coût estimé** | ~0.001-0.05€/requête |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Anthropic / OpenAI ou modèle maison
...
Quand il accède à la fonctionnalité « Suggestion ajustement IA »
...
Quand il tente d'accéder à « Suggestion ajustement IA »
...
```gherkin
Étant donné Le service externe « Anthropic / OpenAI ou modèle maison » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Anthropic / OpenAI ou modèle maison » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name suggestion-ajustement-ia`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/ai` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/ai` | JWT | Selon AC-1 | Création |
| PUT     | `/api/ai/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/ai/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const suggestion_ajustement_iaSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SuggestionajustementiaInput = z.infer<typeof suggestion_ajustement_iaSchema>
```
...
```bash
pnpm test src/lib/services/suggestion-ajustement-ia.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/ai` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/ai` renvoie 401 sans JWT valide
- [ ] `/api/ai` renvoie 403 avec un rôle insuffisant
- [ ] `/api/ai` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Anthropic / OpenAI ou modèle maison » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/suggestion-ajustement-ia.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/suggestion-ajustement-ia.spec.ts
```
...
- Documentation officielle de **Anthropic / OpenAI ou modèle maison** (à vérifier au démarrage)
