# US-2155 — Prédiction risque hypo

> 📌 **19. IA & aide décision** · Priorité **V3** · Pays **Universel**
> 
> 💬 **Note inventaire** : Risque réglementaire = DM
...
Quand il accède à la fonctionnalité « Prédiction risque hypo »
...
Quand il tente d'accéder à « Prédiction risque hypo »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prediction-risque-hypo`
...
export const prediction_risque_hypoSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PredictionrisquehypoInput = z.infer<typeof prediction_risque_hypoSchema>
```
...
```bash
pnpm test src/lib/services/prediction-risque-hypo.test.ts
```
...
```bash
pnpm test:integration tests/integration/prediction-risque-hypo.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prediction-risque-hypo.spec.ts
```
