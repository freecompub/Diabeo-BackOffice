...
|---|---|
| **ID** | `US-2129` |
| **Référence inventaire** | `FN-129` |
| **Domaine** | 15. Interopérabilité |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Standard HPRIM |
...
### 🌐 Intégration externe — Standard HPRIM
...
Quand il accède à la fonctionnalité « HPRIM / résultats labo »
...
Quand il tente d'accéder à « HPRIM / résultats labo »
...
```gherkin
Étant donné Le service externe « Standard HPRIM » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Standard HPRIM » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name hprim-resultats-labo`
...
export const hprim_resultats_laboSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type HprimresultatslaboInput = z.infer<typeof hprim_resultats_laboSchema>
```
...
```bash
pnpm test src/lib/services/hprim-resultats-labo.test.ts
```
...
- [ ] Un appel à « Standard HPRIM » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/hprim-resultats-labo.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/hprim-resultats-labo.spec.ts
```
...
- Documentation officielle de **Standard HPRIM** (à vérifier au démarrage)
