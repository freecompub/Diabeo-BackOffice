# US-2131 — API publique partenaires

> 📌 **15. Interopérabilité** · Priorité **V2** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « API publique partenaires »
...
Quand il tente d'accéder à « API publique partenaires »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name api-publique-partenaires`
...
export const api_publique_partenairesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ApipubliquepartenairesInput = z.infer<typeof api_publique_partenairesSchema>
```
...
```bash
pnpm test src/lib/services/api-publique-partenaires.test.ts
```
...
```bash
pnpm test:integration tests/integration/api-publique-partenaires.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/api-publique-partenaires.spec.ts
```
