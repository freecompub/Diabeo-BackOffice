...
> 📌 **15. Interopérabilité** · Priorité **V1** · Pays **Universel**
> 
> 💬 **Note inventaire** : Standard ouvert
...
### 🌐 Intégration externe — HAPI FHIR (open source)
...
Quand il accède à la fonctionnalité « HL7 FHIR R4 »
...
Quand il tente d'accéder à « HL7 FHIR R4 »
...
```gherkin
Étant donné Le service externe « HAPI FHIR (open source) » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « HAPI FHIR (open source) » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name hl7-fhir-r4`
...
export const hl7_fhir_r4Schema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Hl7Fhirr4Input = z.infer<typeof hl7_fhir_r4Schema>
```
...
```bash
pnpm test src/lib/services/hl7-fhir-r4.test.ts
```
...
- [ ] Un appel à « HAPI FHIR (open source) » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/hl7-fhir-r4.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/hl7-fhir-r4.spec.ts
```
...
- Documentation officielle de **HAPI FHIR (open source)** (à vérifier au démarrage)
