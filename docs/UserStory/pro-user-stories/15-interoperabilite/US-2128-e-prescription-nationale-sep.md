# US-2128 — e-prescription nationale (SeP)

> 📌 **15. Interopérabilité** · Priorité **V2** · Pays **FR**
> 
> 💬 **Note inventaire** : Obligatoire 2025
...
|---|---|
| **ID** | `US-2128` |
| **Référence inventaire** | `FN-128` |
| **Domaine** | 15. Interopérabilité |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | ANS — SeP |
| **Modèle économique** | Gratuit |
| **Coût estimé** | Pré-requis: LAP certifié |
| **Statut** | 🆕 À démarrer |
...
Quand il accède à la fonctionnalité « e-prescription nationale (SeP) »
...
Quand il tente d'accéder à « e-prescription nationale (SeP) »
...
```gherkin
Étant donné Le service externe « ANS — SeP » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS — SeP » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name e-prescription-nationale-sep`
...
export const e_prescription_nationale_sepSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type EprescriptionnationalesepInput = z.infer<typeof e_prescription_nationale_sepSchema>
```
...
```bash
pnpm test src/lib/services/e-prescription-nationale-sep.test.ts
```
...
- [ ] Un appel à « ANS — SeP » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/e-prescription-nationale-sep.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/e-prescription-nationale-sep.spec.ts
```
...
- Documentation officielle de **ANS — SeP** (à vérifier au démarrage)
