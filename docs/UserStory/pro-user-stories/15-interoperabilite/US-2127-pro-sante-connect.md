# US-2127 — Pro Santé Connect

> 📌 **15. Interopérabilité** · Priorité **V1** · Pays **FR**
> 
> 💬 **Note inventaire** : Voir aussi domaine Auth
...
|---|---|
| **ID** | `US-2127` |
| **Référence inventaire** | `FN-127` |
| **Domaine** | 15. Interopérabilité |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | ANS — PSC |
...
Quand il accède à la fonctionnalité « Pro Santé Connect »
...
Quand il tente d'accéder à « Pro Santé Connect »
...
```gherkin
Étant donné Le service externe « ANS — PSC » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS — PSC » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name pro-sante-connect`
...
export const pro_sante_connectSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ProsanteconnectInput = z.infer<typeof pro_sante_connectSchema>
```
...
```bash
pnpm test src/lib/services/pro-sante-connect.test.ts
```
...
- [ ] Un appel à « ANS — PSC » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/pro-sante-connect.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/pro-sante-connect.spec.ts
```
...
- Documentation officielle de **ANS — PSC** (à vérifier au démarrage)
