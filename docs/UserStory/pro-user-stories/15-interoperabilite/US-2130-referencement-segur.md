# US-2130 — Référencement Ségur

> 📌 **15. Interopérabilité** · Priorité **V2** · Pays **FR**
> 
> 💬 **Note inventaire** : Subventionné via SONS
...
|---|---|
| **ID** | `US-2130` |
| **Référence inventaire** | `FN-130` |
| **Domaine** | 15. Interopérabilité |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | ANS — Ségur Numérique Santé |
| **Modèle économique** | Gratuit + audit |
| **Coût estimé** | Audit ~30-80 k€ |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — ANS — Ségur Numérique Santé
...
Quand il accède à la fonctionnalité « Référencement Ségur »
...
Quand il tente d'accéder à « Référencement Ségur »
...
```gherkin
Étant donné Le service externe « ANS — Ségur Numérique Santé » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS — Ségur Numérique Santé » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name referencement-segur`
...
export const referencement_segurSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ReferencementsegurInput = z.infer<typeof referencement_segurSchema>
```
...
```bash
pnpm test src/lib/services/referencement-segur.test.ts
```
...
- [ ] Un appel à « ANS — Ségur Numérique Santé » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/referencement-segur.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/referencement-segur.spec.ts
```
...
- Documentation officielle de **ANS — Ségur Numérique Santé** (à vérifier au démarrage)
