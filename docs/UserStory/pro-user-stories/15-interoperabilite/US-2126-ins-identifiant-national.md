# US-2126 — INS — Identifiant National

> 📌 **15. Interopérabilité** · Priorité **V1** · Pays **FR**
> 
...
|---|---|
| **ID** | `US-2126` |
| **Référence inventaire** | `FN-126` |
| **Domaine** | 15. Interopérabilité |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | ANS Téléservice INSi |
| **Modèle économique** | Gratuit + habilitation |
| **Coût estimé** | Audit ~5-10 k€ |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — ANS Téléservice INSi
...
Quand il accède à la fonctionnalité « INS — Identifiant National »
...
Quand il tente d'accéder à « INS — Identifiant National »
...
```gherkin
Étant donné Le service externe « ANS Téléservice INSi » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS Téléservice INSi » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name ins-identifiant-national`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/interop` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/interop` | JWT | Selon AC-1 | Création |
| PUT     | `/api/interop/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/interop/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const ins_identifiant_nationalSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type InsidentifiantnationalInput = z.infer<typeof ins_identifiant_nationalSchema>
```
...
```bash
pnpm test src/lib/services/ins-identifiant-national.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/interop` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/interop` renvoie 401 sans JWT valide
- [ ] `/api/interop` renvoie 403 avec un rôle insuffisant
- [ ] `/api/interop` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « ANS Téléservice INSi » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/ins-identifiant-national.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/ins-identifiant-national.spec.ts
```
...
- [ANS — Annuaires & référentiels](https://esante.gouv.fr/)
- [HL7 FHIR R4](https://hl7.org/fhir/R4/)
- Documentation officielle de **ANS Téléservice INSi** (à vérifier au démarrage)
