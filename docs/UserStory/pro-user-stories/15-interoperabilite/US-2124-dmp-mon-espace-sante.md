# US-2124 — DMP / Mon Espace Santé

> 📌 **15. Interopérabilité** · Priorité **V1** · Pays **FR**
> 
> 💬 **Note inventaire** : Obligatoire Ségur
...
|---|---|
| **ID** | `US-2124` |
| **Référence inventaire** | `FN-124` |
| **Domaine** | 15. Interopérabilité |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | ANS — DMP API |
| **Modèle économique** | Gratuit + agrément CNDA |
| **Coût estimé** | Audit ~10-30 k€ |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — ANS — DMP API
...
Quand il accède à la fonctionnalité « DMP / Mon Espace Santé »
...
Quand il tente d'accéder à « DMP / Mon Espace Santé »
...
```gherkin
Étant donné Le service externe « ANS — DMP API » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS — DMP API » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name dmp-mon-espace-sante`
...
export const dmp_mon_espace_santeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DmpmonespacesanteInput = z.infer<typeof dmp_mon_espace_santeSchema>
```
...
```bash
pnpm test src/lib/services/dmp-mon-espace-sante.test.ts
```
...
- [ ] Un appel à « ANS — DMP API » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/dmp-mon-espace-sante.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/dmp-mon-espace-sante.spec.ts
```
...
- Documentation officielle de **ANS — DMP API** (à vérifier au démarrage)
