...
> 📌 **15. Interopérabilité** · Priorité **V4** · Pays **FR**
> 
...
|---|---|
| **ID** | `US-2125` |
| **Référence inventaire** | `FN-125` |
| **Domaine** | 15. Interopérabilité |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Mailiz (ANS) ou Apicrypt |
| **Modèle économique** | Gratuit (Mailiz) / Payant |
...
### 🌐 Intégration externe — Mailiz (ANS) ou Apicrypt
...
Quand il accède à la fonctionnalité « MSSanté »
...
Quand il tente d'accéder à « MSSanté »
...
```gherkin
Étant donné Le service externe « Mailiz (ANS) ou Apicrypt » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Mailiz (ANS) ou Apicrypt » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name mssante`
...
export const mssanteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type MssanteInput = z.infer<typeof mssanteSchema>
```
...
```bash
pnpm test src/lib/services/mssante.test.ts
```
...
- [ ] Un appel à « Mailiz (ANS) ou Apicrypt » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/mssante.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/mssante.spec.ts
```
...
- Documentation officielle de **Mailiz (ANS) ou Apicrypt** (à vérifier au démarrage)
