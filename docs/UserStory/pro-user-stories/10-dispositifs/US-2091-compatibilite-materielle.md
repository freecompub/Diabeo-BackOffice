# US-2091 — Compatibilité matérielle

> 📌 **10. Dispositifs** · Priorité **V1** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Compatibilité matérielle »
...
Quand il tente d'accéder à « Compatibilité matérielle »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name compatibilite-materielle`
...
export const compatibilite_materielleSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CompatibilitematerielleInput = z.infer<typeof compatibilite_materielleSchema>
```
...
```bash
pnpm test src/lib/services/compatibilite-materielle.test.ts
```
...
```bash
pnpm test:integration tests/integration/compatibilite-materielle.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/compatibilite-materielle.spec.ts
```
