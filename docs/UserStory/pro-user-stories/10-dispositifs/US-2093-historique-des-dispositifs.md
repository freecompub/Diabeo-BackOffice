# US-2093 — Historique des dispositifs
...
Quand il accède à la fonctionnalité « Historique des dispositifs »
...
Quand il tente d'accéder à « Historique des dispositifs »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name historique-des-dispositifs`
...
export const historique_des_dispositifsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type HistoriquedesdispositifsInput = z.infer<typeof historique_des_dispositifsSchema>
```
...
```bash
pnpm test src/lib/services/historique-des-dispositifs.test.ts
```
...
```bash
pnpm test:integration tests/integration/historique-des-dispositifs.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/historique-des-dispositifs.spec.ts
```
