# US-2092 — Désactivation / révocation
...
Quand il accède à la fonctionnalité « Désactivation / révocation »
...
Quand il tente d'accéder à « Désactivation / révocation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name desactivation-revocation`
...
export const desactivation_revocationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DesactivationrevocationInput = z.infer<typeof desactivation_revocationSchema>
```
...
```bash
pnpm test src/lib/services/desactivation-revocation.test.ts
```
...
```bash
pnpm test:integration tests/integration/desactivation-revocation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/desactivation-revocation.spec.ts
```
