# US-2089 — Pairing device

> 📌 **10. Dispositifs** · Priorité **MVP** · Pays **FR+DZ**
> 
> 💬 **Note inventaire** : PatientDevice table existe
...
|---|---|
| **ID** | `US-2089` |
| **Référence inventaire** | `FN-089` |
| **Domaine** | 10. Dispositifs |
| **Priorité** | **MVP** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Oui |
| **Service / Standard** | Voir Dexcom/Abbott/Medtronic |
| **Modèle économique** | Selon partenaire |
...
### 🌐 Intégration externe — Voir Dexcom/Abbott/Medtronic
...
Quand il accède à la fonctionnalité « Pairing device »
...
Quand il tente d'accéder à « Pairing device »
...
```gherkin
Étant donné Le service externe « Voir Dexcom/Abbott/Medtronic » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Voir Dexcom/Abbott/Medtronic » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name pairing-device`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/devices` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/devices` | JWT | Selon AC-1 | Création |
| PUT     | `/api/devices/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/devices/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const pairing_deviceSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PairingdeviceInput = z.infer<typeof pairing_deviceSchema>
```
...
```bash
pnpm test src/lib/services/pairing-device.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/devices` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/devices` renvoie 401 sans JWT valide
- [ ] `/api/devices` renvoie 403 avec un rôle insuffisant
- [ ] `/api/devices` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Voir Dexcom/Abbott/Medtronic » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/pairing-device.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/pairing-device.spec.ts
```
...
- Documentation officielle de **Voir Dexcom/Abbott/Medtronic** (à vérifier au démarrage)
