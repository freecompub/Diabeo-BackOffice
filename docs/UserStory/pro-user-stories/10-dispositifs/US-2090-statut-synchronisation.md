# US-2090 — Statut synchronisation

> 📌 **10. Dispositifs** · Priorité **MVP** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Statut synchronisation »
...
Quand il tente d'accéder à « Statut synchronisation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name statut-synchronisation`
...
export const statut_synchronisationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type StatutsynchronisationInput = z.infer<typeof statut_synchronisationSchema>
```
...
```bash
pnpm test src/lib/services/statut-synchronisation.test.ts
```
...
```bash
pnpm test:integration tests/integration/statut-synchronisation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/statut-synchronisation.spec.ts
```
