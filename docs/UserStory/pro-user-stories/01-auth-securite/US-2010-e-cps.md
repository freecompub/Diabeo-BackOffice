...
> 
> 💬 **Note inventaire** : Plus simple que carte CPS
...
Quand il accède à la fonctionnalité « e-CPS »
...
Quand il tente d'accéder à « e-CPS »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name e-cps`
...
```bash
pnpm test src/lib/services/e-cps.test.ts
```
...
```bash
pnpm test:integration tests/integration/e-cps.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/e-cps.spec.ts
```
