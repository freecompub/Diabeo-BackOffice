...
> 📌 **1. Auth & Sécurité** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : OVH recommandé pour HDS
...
|---|---|
| **ID** | `US-2003` |
| **Référence inventaire** | `FN-003` |
| **Domaine** | 1. Auth & Sécurité |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | OVH Email Pro / Mailjet / SendGrid |
...
### 🌐 Intégration externe — OVH Email Pro / Mailjet / SendGrid
...
Quand il accède à la fonctionnalité « Reset password email »
...
Quand il tente d'accéder à « Reset password email »
...
```gherkin
Étant donné Le service externe « OVH Email Pro / Mailjet / SendGrid » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « OVH Email Pro / Mailjet / SendGrid » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name reset-password-email`
...
export const reset_password_emailSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ResetpasswordemailInput = z.infer<typeof reset_password_emailSchema>
```
...
```bash
pnpm test src/lib/services/reset-password-email.test.ts
```
...
- [ ] Un appel à « OVH Email Pro / Mailjet / SendGrid » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/reset-password-email.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/reset-password-email.spec.ts
```
...
- Documentation officielle de **OVH Email Pro / Mailjet / SendGrid** (à vérifier au démarrage)
