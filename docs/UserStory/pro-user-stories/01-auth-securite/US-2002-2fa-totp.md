...
> 
> 💬 **Note inventaire** : Quasi-obligatoire HDS
...
Double facteur via app authenticator
...
Quand il accède à la fonctionnalité « 2FA TOTP »
...
Quand il tente d'accéder à « 2FA TOTP »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name 2fa-totp`
...
export const 2fa_totpSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type 2FatotpInput = z.infer<typeof 2fa_totpSchema>
```
...
```bash
pnpm test src/lib/services/2fa-totp.test.ts
```
...
```bash
pnpm test:integration tests/integration/2fa-totp.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/2fa-totp.spec.ts
```
