...
> 📌 **1. Auth & Sécurité** · Priorité **V2** · Pays **FR**
> 
> 💬 **Note inventaire** : Hors web — limité ambulatoire
...
|---|---|
| **ID** | `US-2009` |
| **Référence inventaire** | `FN-009` |
| **Domaine** | 1. Auth & Sécurité |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | ANS + middleware ICANopen/Cryptolib |
| **Modèle économique** | Gratuit (côté ANS) |
| **Coût estimé** | Lecteurs CPS côté PS |
| **Statut** | 🆕 À démarrer |
...
Connexion par carte de pro santé
...
### 🌐 Intégration externe — ANS + middleware ICANopen/Cryptolib
...
Quand il accède à la fonctionnalité « Carte CPS »
...
Quand il tente d'accéder à « Carte CPS »
...
```gherkin
Étant donné Le service externe « ANS + middleware ICANopen/Cryptolib » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS + middleware ICANopen/Cryptolib » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name carte-cps`
...
export const carte_cpsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CartecpsInput = z.infer<typeof carte_cpsSchema>
```
...
```bash
pnpm test src/lib/services/carte-cps.test.ts
```
...
- [ ] Un appel à « ANS + middleware ICANopen/Cryptolib » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/carte-cps.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/carte-cps.spec.ts
```
...
- Documentation officielle de **ANS + middleware ICANopen/Cryptolib** (à vérifier au démarrage)
