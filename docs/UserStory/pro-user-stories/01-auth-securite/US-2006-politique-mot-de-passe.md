# US-2006 — Politique mot de passe
...
Complexité, rotation, historique
...
Quand il accède à la fonctionnalité « Politique mot de passe »
...
Quand il tente d'accéder à « Politique mot de passe »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name politique-mot-de-passe`
...
export const politique_mot_de_passeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PolitiquemotdepasseInput = z.infer<typeof politique_mot_de_passeSchema>
```
...
```bash
pnpm test src/lib/services/politique-mot-de-passe.test.ts
```
...
```bash
pnpm test:integration tests/integration/politique-mot-de-passe.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/politique-mot-de-passe.spec.ts
```
