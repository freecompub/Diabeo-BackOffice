...
|---|---|
| **ID** | `US-2013` |
| **Référence inventaire** | `FN-013` |
| **Domaine** | 1. Auth & Sécurité |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (UserPrivacySettings) |
...
Recueil et gestion consentements
...
Quand il accède à la fonctionnalité « Consentement RGPD »
...
Quand il tente d'accéder à « Consentement RGPD »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name consentement-rgpd`
...
export const consentement_rgpdSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ConsentementrgpdInput = z.infer<typeof consentement_rgpdSchema>
```
...
```bash
pnpm test src/lib/services/consentement-rgpd.test.ts
```
...
```bash
pnpm test:integration tests/integration/consentement-rgpd.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/consentement-rgpd.spec.ts
```
