# US-2014 — Notification violation

> 📌 **1. Auth & Sécurité** · Priorité **V2** · Pays **FR**
> 
> 💬 **Note inventaire** : Délai 72h obligatoire RGPD
...
|---|---|
| **ID** | `US-2014` |
| **Référence inventaire** | `FN-014` |
| **Domaine** | 1. Auth & Sécurité |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (workflow + email) |
...
Quand il accède à la fonctionnalité « Notification violation »
...
Quand il tente d'accéder à « Notification violation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name notification-violation`
...
export const notification_violationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type NotificationviolationInput = z.infer<typeof notification_violationSchema>
```
...
```bash
pnpm test src/lib/services/notification-violation.test.ts
```
...
```bash
pnpm test:integration tests/integration/notification-violation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/notification-violation.spec.ts
```
