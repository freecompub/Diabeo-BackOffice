...
Authentification email/mdp avec JWT RS256
...
Quand il accède à la fonctionnalité « Login JWT »
...
Quand il tente d'accéder à « Login JWT »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name login-jwt`
...
export const login_jwtSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type LoginjwtInput = z.infer<typeof login_jwtSchema>
```
...
```bash
pnpm test src/lib/services/login-jwt.test.ts
```
...
```bash
pnpm test:integration tests/integration/login-jwt.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/login-jwt.spec.ts
```
