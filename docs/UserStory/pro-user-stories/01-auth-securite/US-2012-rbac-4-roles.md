...
Quand il accède à la fonctionnalité « RBAC 4 rôles »
...
Quand il tente d'accéder à « RBAC 4 rôles »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name rbac-4-roles`
...
export const rbac_4_rolesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Rbac4RolesInput = z.infer<typeof rbac_4_rolesSchema>
```
...
```bash
pnpm test src/lib/services/rbac-4-roles.test.ts
```
...
```bash
pnpm test:integration tests/integration/rbac-4-roles.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/rbac-4-roles.spec.ts
```
