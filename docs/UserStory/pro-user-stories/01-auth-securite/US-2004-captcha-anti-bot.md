# US-2004 — Captcha anti-bot

> 📌 **1. Auth & Sécurité** · Priorité **V1** · Pays **Universel**
> 
> 💬 **Note inventaire** : Turnstile = privacy-friendly
...
|---|---|
| **ID** | `US-2004` |
| **Référence inventaire** | `FN-004` |
| **Domaine** | 1. Auth & Sécurité |
| **Priorité** | **V1** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Cloudflare Turnstile / hCaptcha |
...
Protection contre bots login/inscription
...
### 🌐 Intégration externe — Cloudflare Turnstile / hCaptcha
...
Quand il accède à la fonctionnalité « Captcha anti-bot »
...
Quand il tente d'accéder à « Captcha anti-bot »
...
```gherkin
Étant donné Le service externe « Cloudflare Turnstile / hCaptcha » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Cloudflare Turnstile / hCaptcha » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name captcha-anti-bot`
...
export const captcha_anti_botSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CaptchaantibotInput = z.infer<typeof captcha_anti_botSchema>
```
...
```bash
pnpm test src/lib/services/captcha-anti-bot.test.ts
```
...
- [ ] Un appel à « Cloudflare Turnstile / hCaptcha » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/captcha-anti-bot.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/captcha-anti-bot.spec.ts
```
...
- Documentation officielle de **Cloudflare Turnstile / hCaptcha** (à vérifier au démarrage)
