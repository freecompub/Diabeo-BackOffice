...
> 
> 💬 **Note inventaire** : Table Session existe
...
Liste sessions actives + déconnexion à distance
...
Quand il accède à la fonctionnalité « Sessions multiples »
...
Quand il tente d'accéder à « Sessions multiples »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name sessions-multiples`
...
export const sessions_multiplesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SessionsmultiplesInput = z.infer<typeof sessions_multiplesSchema>
```
...
```bash
pnpm test src/lib/services/sessions-multiples.test.ts
```
...
```bash
pnpm test:integration tests/integration/sessions-multiples.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/sessions-multiples.spec.ts
```
