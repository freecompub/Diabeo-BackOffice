# US-2005 — Verrouillage tentatives

> 📌 **1. Auth & Sécurité** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : rate-limit.ts existe déjà
...
Quand il accède à la fonctionnalité « Verrouillage tentatives »
...
Quand il tente d'accéder à « Verrouillage tentatives »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name verrouillage-tentatives`
...
export const verrouillage_tentativesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type VerrouillagetentativesInput = z.infer<typeof verrouillage_tentativesSchema>
```
...
```bash
pnpm test src/lib/services/verrouillage-tentatives.test.ts
```
...
```bash
pnpm test:integration tests/integration/verrouillage-tentatives.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/verrouillage-tentatives.spec.ts
```
