...
Quand il accède à la fonctionnalité « Templates de messages »
...
Quand il tente d'accéder à « Templates de messages »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name templates-de-messages`
...
export const templates_de_messagesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TemplatesdemessagesInput = z.infer<typeof templates_de_messagesSchema>
```
...
```bash
pnpm test src/lib/services/templates-de-messages.test.ts
```
...
```bash
pnpm test:integration tests/integration/templates-de-messages.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/templates-de-messages.spec.ts
```
