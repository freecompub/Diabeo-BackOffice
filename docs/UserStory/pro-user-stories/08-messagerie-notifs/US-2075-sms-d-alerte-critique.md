# US-2075 — SMS d'alerte critique

> 📌 **8. Messagerie & notifs** · Priorité **V4** · Pays **FR+DZ**
> 
...
|---|---|
| **ID** | `US-2075` |
| **Référence inventaire** | `FN-075` |
| **Domaine** | 8. Messagerie & notifs |
| **Priorité** | **V4** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Oui |
| **Service / Standard** | Twilio / OVH SMS / Orange Contact Everyone |
| **Modèle économique** | Payant à l'unité |
| **Coût estimé** | 0.04-0.08€/SMS |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Twilio / OVH SMS / Orange Contact Everyone
...
Quand il accède à la fonctionnalité « SMS d'alerte critique »
...
Quand il tente d'accéder à « SMS d'alerte critique »
...
```gherkin
Étant donné Le service externe « Twilio / OVH SMS / Orange Contact Everyone » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Twilio / OVH SMS / Orange Contact Everyone » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name sms-d-alerte-critique`
...
export const sms_d_alerte_critiqueSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SmsdalertecritiqueInput = z.infer<typeof sms_d_alerte_critiqueSchema>
```
...
```bash
pnpm test src/lib/services/sms-d-alerte-critique.test.ts
```
...
- [ ] Un appel à « Twilio / OVH SMS / Orange Contact Everyone » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/sms-d-alerte-critique.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/sms-d-alerte-critique.spec.ts
```
...
- Documentation officielle de **Twilio / OVH SMS / Orange Contact Everyone** (à vérifier au démarrage)
