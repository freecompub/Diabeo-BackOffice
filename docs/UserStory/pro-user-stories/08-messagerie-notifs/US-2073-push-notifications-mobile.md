# US-2073 — Push notifications mobile

> 📌 **8. Messagerie & notifs** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Tables prêtes
...
|---|---|
| **ID** | `US-2073` |
| **Référence inventaire** | `FN-073` |
| **Domaine** | 8. Messagerie & notifs |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Firebase Cloud Messaging + APNs |
| **Modèle économique** | Freemium |
| **Coût estimé** | Gratuit jusqu'à volumes élevés |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Firebase Cloud Messaging + APNs
...
Quand il accède à la fonctionnalité « Push notifications mobile »
...
Quand il tente d'accéder à « Push notifications mobile »
...
```gherkin
Étant donné Le service externe « Firebase Cloud Messaging + APNs » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Firebase Cloud Messaging + APNs » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name push-notifications-mobile`
...
export const push_notifications_mobileSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PushnotificationsmobileInput = z.infer<typeof push_notifications_mobileSchema>
```
...
```bash
pnpm test src/lib/services/push-notifications-mobile.test.ts
```
...
- [ ] Un appel à « Firebase Cloud Messaging + APNs » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/push-notifications-mobile.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/push-notifications-mobile.spec.ts
```
...
- Documentation officielle de **Firebase Cloud Messaging + APNs** (à vérifier au démarrage)
