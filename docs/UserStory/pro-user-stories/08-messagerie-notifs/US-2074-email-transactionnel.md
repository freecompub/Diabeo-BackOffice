# US-2074 — Email transactionnel

> 📌 **8. Messagerie & notifs** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : OVH = option HDS-friendly
...
|---|---|
| **ID** | `US-2074` |
| **Référence inventaire** | `FN-074` |
| **Domaine** | 8. Messagerie & notifs |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | OVH MX Plan / Mailjet / SendGrid |
| **Modèle économique** | Freemium / Payant |
| **Coût estimé** | 0–100€/mois selon volume |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — OVH MX Plan / Mailjet / SendGrid
...
Quand il accède à la fonctionnalité « Email transactionnel »
...
Quand il tente d'accéder à « Email transactionnel »
...
```gherkin
Étant donné Le service externe « OVH MX Plan / Mailjet / SendGrid » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « OVH MX Plan / Mailjet / SendGrid » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name email-transactionnel`
...
export const email_transactionnelSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type EmailtransactionnelInput = z.infer<typeof email_transactionnelSchema>
```
...
```bash
pnpm test src/lib/services/email-transactionnel.test.ts
```
...
- [ ] Un appel à « OVH MX Plan / Mailjet / SendGrid » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/email-transactionnel.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/email-transactionnel.spec.ts
```
...
- Documentation officielle de **OVH MX Plan / Mailjet / SendGrid** (à vérifier au démarrage)
