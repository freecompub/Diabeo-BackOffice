# US-2079 — Préférences notification
...
|---|---|
| **ID** | `US-2079` |
| **Référence inventaire** | `FN-079` |
| **Domaine** | 8. Messagerie & notifs |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (UserNotifPreferences) |
...
Quand il accède à la fonctionnalité « Préférences notification »
...
Quand il tente d'accéder à « Préférences notification »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name preferences-notification`
...
export const preferences_notificationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PreferencesnotificationInput = z.infer<typeof preferences_notificationSchema>
```
...
```bash
pnpm test src/lib/services/preferences-notification.test.ts
```
...
```bash
pnpm test:integration tests/integration/preferences-notification.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/preferences-notification.spec.ts
```
