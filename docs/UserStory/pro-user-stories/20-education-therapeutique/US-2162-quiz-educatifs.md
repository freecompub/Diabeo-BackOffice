# US-2162 — Quiz éducatifs

> 📌 **20. Éducation thérapeutique** · Priorité **V3** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Quiz éducatifs »
...
Quand il tente d'accéder à « Quiz éducatifs »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name quiz-educatifs`
...
export const quiz_educatifsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type QuizeducatifsInput = z.infer<typeof quiz_educatifsSchema>
```
...
```bash
pnpm test src/lib/services/quiz-educatifs.test.ts
```
...
```bash
pnpm test:integration tests/integration/quiz-educatifs.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/quiz-educatifs.spec.ts
```
