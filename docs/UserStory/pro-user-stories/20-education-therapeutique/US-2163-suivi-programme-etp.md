# US-2163 — Suivi programme ETP

> 📌 **20. Éducation thérapeutique** · Priorité **V3** · Pays **FR**
> 
...
Quand il accède à la fonctionnalité « Suivi programme ETP »
...
Quand il tente d'accéder à « Suivi programme ETP »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name suivi-programme-etp`
...
export const suivi_programme_etpSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SuiviprogrammeetpInput = z.infer<typeof suivi_programme_etpSchema>
```
...
```bash
pnpm test src/lib/services/suivi-programme-etp.test.ts
```
...
```bash
pnpm test:integration tests/integration/suivi-programme-etp.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/suivi-programme-etp.spec.ts
```
