# US-2160 — Bibliothèque ressources patients

> 📌 **20. Éducation thérapeutique** · Priorité **V2** · Pays **FR+DZ**
> 
...
|---|---|
| **ID** | `US-2160` |
| **Référence inventaire** | `FN-160` |
| **Domaine** | 20. Éducation thérapeutique |
| **Priorité** | **V2** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (CMS) |
...
Quand il accède à la fonctionnalité « Bibliothèque ressources patients »
...
Quand il tente d'accéder à « Bibliothèque ressources patients »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name bibliotheque-ressources-patients`
...
export const bibliotheque_ressources_patientsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type BibliothequeressourcespatientsInput = z.infer<typeof bibliotheque_ressources_patientsSchema>
```
...
```bash
pnpm test src/lib/services/bibliotheque-ressources-patients.test.ts
```
...
```bash
pnpm test:integration tests/integration/bibliotheque-ressources-patients.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/bibliotheque-ressources-patients.spec.ts
```
