# US-2161 — Contenus personnalisés profil

> 📌 **20. Éducation thérapeutique** · Priorité **V2** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Contenus personnalisés profil »
...
Quand il tente d'accéder à « Contenus personnalisés profil »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name contenus-personnalises-profil`
...
export const contenus_personnalises_profilSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ContenuspersonnalisesprofilInput = z.infer<typeof contenus_personnalises_profilSchema>
```
...
```bash
pnpm test src/lib/services/contenus-personnalises-profil.test.ts
```
...
```bash
pnpm test:integration tests/integration/contenus-personnalises-profil.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/contenus-personnalises-profil.spec.ts
```
