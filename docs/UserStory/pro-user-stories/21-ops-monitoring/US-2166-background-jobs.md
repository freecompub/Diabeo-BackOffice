...
Quand il accède à la fonctionnalité « Background jobs »
...
Quand il tente d'accéder à « Background jobs »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name background-jobs`
...
export type BackgroundjobsInput = z.infer<typeof background_jobsSchema>
```
...
```bash
pnpm test src/lib/services/background-jobs.test.ts
```
...
```bash
pnpm test:integration tests/integration/background-jobs.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/background-jobs.spec.ts
```
