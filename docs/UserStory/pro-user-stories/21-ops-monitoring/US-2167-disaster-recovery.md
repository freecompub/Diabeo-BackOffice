# US-2167 — Disaster recovery

> 📌 **21. Ops & monitoring** · Priorité **V1** · Pays **FR**
> 
...
|---|---|
| **ID** | `US-2167` |
| **Référence inventaire** | `FN-167` |
| **Domaine** | 21. Ops & monitoring |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | OVHcloud Backup / Veeam |
| **Modèle économique** | Payant |
| **Coût estimé** | ~50-300€/mois |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — OVHcloud Backup / Veeam
...
Quand il accède à la fonctionnalité « Disaster recovery »
...
Quand il tente d'accéder à « Disaster recovery »
...
```gherkin
Étant donné Le service externe « OVHcloud Backup / Veeam » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « OVHcloud Backup / Veeam » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name disaster-recovery`
...
export const disaster_recoverySchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DisasterrecoveryInput = z.infer<typeof disaster_recoverySchema>
```
...
```bash
pnpm test src/lib/services/disaster-recovery.test.ts
```
...
- [ ] Un appel à « OVHcloud Backup / Veeam » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/disaster-recovery.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/disaster-recovery.spec.ts
```
...
- Documentation officielle de **OVHcloud Backup / Veeam** (à vérifier au démarrage)
