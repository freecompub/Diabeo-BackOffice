# US-2168 — Metrics business temps réel

> 📌 **21. Ops & monitoring** · Priorité **V1** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2168` |
| **Référence inventaire** | `FN-168` |
| **Domaine** | 21. Ops & monitoring |
| **Priorité** | **V1** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Grafana + Prometheus / Metabase |
| **Modèle économique** | Gratuit (OSS) |
| **Coût estimé** | — |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Grafana + Prometheus / Metabase
...
Quand il accède à la fonctionnalité « Metrics business temps réel »
...
Quand il tente d'accéder à « Metrics business temps réel »
...
```gherkin
Étant donné Le service externe « Grafana + Prometheus / Metabase » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Grafana + Prometheus / Metabase » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name metrics-business-temps-reel`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/ops` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/ops` | JWT | Selon AC-1 | Création |
| PUT     | `/api/ops/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/ops/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const metrics_business_temps_reelSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type MetricsbusinesstempsreelInput = z.infer<typeof metrics_business_temps_reelSchema>
```
...
```bash
pnpm test src/lib/services/metrics-business-temps-reel.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/ops` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/ops` renvoie 401 sans JWT valide
- [ ] `/api/ops` renvoie 403 avec un rôle insuffisant
- [ ] `/api/ops` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Grafana + Prometheus / Metabase » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/metrics-business-temps-reel.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/metrics-business-temps-reel.spec.ts
```
...
- Documentation officielle de **Grafana + Prometheus / Metabase** (à vérifier au démarrage)
