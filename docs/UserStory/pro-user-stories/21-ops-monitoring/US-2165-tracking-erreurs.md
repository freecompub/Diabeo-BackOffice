...
|---|---|
| **ID** | `US-2165` |
| **Référence inventaire** | `FN-165` |
| **Domaine** | 21. Ops & monitoring |
| **Priorité** | **V1** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Sentry (peut être self-hosted HDS) |
...
### 🌐 Intégration externe — Sentry (peut être self-hosted HDS)
...
Quand il accède à la fonctionnalité « Tracking erreurs »
...
Quand il tente d'accéder à « Tracking erreurs »
...
```gherkin
Étant donné Le service externe « Sentry (peut être self-hosted HDS) » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Sentry (peut être self-hosted HDS) » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name tracking-erreurs`
...
export const tracking_erreursSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TrackingerreursInput = z.infer<typeof tracking_erreursSchema>
```
...
```bash
pnpm test src/lib/services/tracking-erreurs.test.ts
```
...
- [ ] Un appel à « Sentry (peut être self-hosted HDS) » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/tracking-erreurs.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/tracking-erreurs.spec.ts
```
...
- Documentation officielle de **Sentry (peut être self-hosted HDS)** (à vérifier au démarrage)
