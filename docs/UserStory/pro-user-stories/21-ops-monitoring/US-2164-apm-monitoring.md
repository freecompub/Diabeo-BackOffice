# US-2164 — APM monitoring

> 📌 **21. Ops & monitoring** · Priorité **V1** · Pays **Universel**
> 
> 💬 **Note inventaire** : Sentry self-hosted possible
...
### 🌐 Intégration externe — Sentry / Datadog / Grafana+Tempo
...
Quand il accède à la fonctionnalité « APM monitoring »
...
Quand il tente d'accéder à « APM monitoring »
...
```gherkin
Étant donné Le service externe « Sentry / Datadog / Grafana+Tempo » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Sentry / Datadog / Grafana+Tempo » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name apm-monitoring`
...
export const apm_monitoringSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ApmmonitoringInput = z.infer<typeof apm_monitoringSchema>
```
...
```bash
pnpm test src/lib/services/apm-monitoring.test.ts
```
...
- [ ] Un appel à « Sentry / Datadog / Grafana+Tempo » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/apm-monitoring.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/apm-monitoring.spec.ts
```
...
- Documentation officielle de **Sentry / Datadog / Grafana+Tempo** (à vérifier au démarrage)
