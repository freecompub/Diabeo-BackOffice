# US-2174 — Templates prescription récurrents

> 📌 **22. Prescription & ordonnances** · Priorité **V1** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Templates prescription récurrents »
...
Quand il tente d'accéder à « Templates prescription récurrents »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name templates-prescription-recurrents`
...
export const templates_prescription_recurrentsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TemplatesprescriptionrecurrentsInput = z.infer<typeof templates_prescription_recurrentsSchema>
```
...
```bash
pnpm test src/lib/services/templates-prescription-recurrents.test.ts
```
...
```bash
pnpm test:integration tests/integration/templates-prescription-recurrents.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/templates-prescription-recurrents.spec.ts
```
