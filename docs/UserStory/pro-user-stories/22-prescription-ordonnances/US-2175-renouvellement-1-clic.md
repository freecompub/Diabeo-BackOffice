...
Quand il accède à la fonctionnalité « Renouvellement 1 clic »
...
Quand il tente d'accéder à « Renouvellement 1 clic »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name renouvellement-1-clic`
...
export const renouvellement_1_clicSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Renouvellement1ClicInput = z.infer<typeof renouvellement_1_clicSchema>
```
...
```bash
pnpm test src/lib/services/renouvellement-1-clic.test.ts
```
...
```bash
pnpm test:integration tests/integration/renouvellement-1-clic.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/renouvellement-1-clic.spec.ts
```
