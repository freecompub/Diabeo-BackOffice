# US-2205 — Tracking dispensation officine

> 📌 **22. Prescription & ordonnances** · Priorité **V2** · Pays **FR**
> 
...
|---|---|
| **ID** | `US-2205` |
| **Référence inventaire** | `FN-205` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Service ANS SeP retour |
...
### 🌐 Intégration externe — Service ANS SeP retour
...
Quand il accède à la fonctionnalité « Tracking dispensation officine »
...
Quand il tente d'accéder à « Tracking dispensation officine »
...
```gherkin
Étant donné Le service externe « Service ANS SeP retour » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Service ANS SeP retour » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name tracking-dispensation-officine`
...
export const tracking_dispensation_officineSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TrackingdispensationofficineInput = z.infer<typeof tracking_dispensation_officineSchema>
```
...
```bash
pnpm test src/lib/services/tracking-dispensation-officine.test.ts
```
...
- [ ] Un appel à « Service ANS SeP retour » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/tracking-dispensation-officine.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/tracking-dispensation-officine.spec.ts
```
...
- Documentation officielle de **Service ANS SeP retour** (à vérifier au démarrage)
