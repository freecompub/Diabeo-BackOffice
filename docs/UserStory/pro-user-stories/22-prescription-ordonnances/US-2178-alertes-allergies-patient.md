# US-2178 — Alertes allergies patient
...
Quand il accède à la fonctionnalité « Alertes allergies patient »
...
Quand il tente d'accéder à « Alertes allergies patient »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name alertes-allergies-patient`
...
export const alertes_allergies_patientSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AlertesallergiespatientInput = z.infer<typeof alertes_allergies_patientSchema>
```
...
```bash
pnpm test src/lib/services/alertes-allergies-patient.test.ts
```
...
```bash
pnpm test:integration tests/integration/alertes-allergies-patient.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/alertes-allergies-patient.spec.ts
```
