...
|---|---|
| **ID** | `US-2191` |
| **Référence inventaire** | `FN-191` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | ANS + middleware ICANopen |
...
### 🌐 Intégration externe — ANS + middleware ICANopen
...
Quand il accède à la fonctionnalité « Signature carte CPS »
...
Quand il tente d'accéder à « Signature carte CPS »
...
```gherkin
Étant donné Le service externe « ANS + middleware ICANopen » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS + middleware ICANopen » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name signature-carte-cps`
...
export const signature_carte_cpsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SignaturecartecpsInput = z.infer<typeof signature_carte_cpsSchema>
```
...
```bash
pnpm test src/lib/services/signature-carte-cps.test.ts
```
...
- [ ] Un appel à « ANS + middleware ICANopen » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/signature-carte-cps.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/signature-carte-cps.spec.ts
```
...
- Documentation officielle de **ANS + middleware ICANopen** (à vérifier au démarrage)
