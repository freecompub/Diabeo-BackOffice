# US-2206 — Renouvellement programmé
...
Quand il accède à la fonctionnalité « Renouvellement programmé »
...
Quand il tente d'accéder à « Renouvellement programmé »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name renouvellement-programme`
...
export const renouvellement_programmeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type RenouvellementprogrammeInput = z.infer<typeof renouvellement_programmeSchema>
```
...
```bash
pnpm test src/lib/services/renouvellement-programme.test.ts
```
...
```bash
pnpm test:integration tests/integration/renouvellement-programme.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/renouvellement-programme.spec.ts
```
