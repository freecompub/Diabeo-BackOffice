# US-2185 — Prescription auto-surveillance
...
Quand il accède à la fonctionnalité « Prescription auto-surveillance »
...
Quand il tente d'accéder à « Prescription auto-surveillance »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-auto-surveillance`
...
export const prescription_auto_surveillanceSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptionautosurveillanceInput = z.infer<typeof prescription_auto_surveillanceSchema>
```
...
```bash
pnpm test src/lib/services/prescription-auto-surveillance.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-auto-surveillance.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-auto-surveillance.spec.ts
```
