...
|---|---|
| **ID** | `US-2194` |
| **Référence inventaire** | `FN-194` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Universign / DigiCert / TimeLogic |
| **Modèle économique** | Payant |
| **Coût estimé** | ~0.05-0.10€/horodatage |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Universign / DigiCert / TimeLogic
...
Quand il accède à la fonctionnalité « Horodatage qualifié »
...
Quand il tente d'accéder à « Horodatage qualifié »
...
```gherkin
Étant donné Le service externe « Universign / DigiCert / TimeLogic » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Universign / DigiCert / TimeLogic » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name horodatage-qualifie`
...
export const horodatage_qualifieSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type HorodatagequalifieInput = z.infer<typeof horodatage_qualifieSchema>
```
...
```bash
pnpm test src/lib/services/horodatage-qualifie.test.ts
```
...
- [ ] Un appel à « Universign / DigiCert / TimeLogic » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/horodatage-qualifie.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/horodatage-qualifie.spec.ts
```
...
- Documentation officielle de **Universign / DigiCert / TimeLogic** (à vérifier au démarrage)
