# US-2181 — Prescription pompe + consommables
...
Quand il accède à la fonctionnalité « Prescription pompe + consommables »
...
Quand il tente d'accéder à « Prescription pompe + consommables »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-pompe-consommables`
...
export const prescription_pompe_consommablesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptionpompeconsommablesInput = z.infer<typeof prescription_pompe_consommablesSchema>
```
...
```bash
pnpm test src/lib/services/prescription-pompe-consommables.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-pompe-consommables.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-pompe-consommables.spec.ts
```
