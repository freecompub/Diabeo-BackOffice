# US-2176 — Alertes interactions médic.
...
|---|---|
| **ID** | `US-2176` |
| **Référence inventaire** | `FN-176` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Inclus dans BdM agréée |
| **Modèle économique** | Inclus licence BdM |
| **Coût estimé** | — |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Inclus dans BdM agréée
...
Quand il accède à la fonctionnalité « Alertes interactions médic. »
...
Quand il tente d'accéder à « Alertes interactions médic. »
...
```gherkin
Étant donné Le service externe « Inclus dans BdM agréée » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Inclus dans BdM agréée » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name alertes-interactions-medic`
...
export const alertes_interactions_medicSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AlertesinteractionsmedicInput = z.infer<typeof alertes_interactions_medicSchema>
```
...
```bash
pnpm test src/lib/services/alertes-interactions-medic.test.ts
```
...
- [ ] Un appel à « Inclus dans BdM agréée » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/alertes-interactions-medic.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/alertes-interactions-medic.spec.ts
```
...
- Documentation officielle de **Inclus dans BdM agréée** (à vérifier au démarrage)
