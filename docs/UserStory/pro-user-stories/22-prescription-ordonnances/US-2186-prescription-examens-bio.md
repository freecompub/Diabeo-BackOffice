# US-2186 — Prescription examens bio
...
Quand il accède à la fonctionnalité « Prescription examens bio »
...
Quand il tente d'accéder à « Prescription examens bio »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-examens-bio`
...
export const prescription_examens_bioSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptionexamensbioInput = z.infer<typeof prescription_examens_bioSchema>
```
...
```bash
pnpm test src/lib/services/prescription-examens-bio.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-examens-bio.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-examens-bio.spec.ts
```
