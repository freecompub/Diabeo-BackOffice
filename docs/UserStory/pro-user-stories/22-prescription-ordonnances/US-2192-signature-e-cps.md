# US-2192 — Signature e-CPS

> 📌 **22. Prescription & ordonnances** · Priorité **V1** · Pays **FR**
> 
> 💬 **Note inventaire** : Plus simple que carte
...
Quand il accède à la fonctionnalité « Signature e-CPS »
...
Quand il tente d'accéder à « Signature e-CPS »
...
```gherkin
Étant donné Le service externe « ANS — e-CPS » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS — e-CPS » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name signature-e-cps`
...
export const signature_e_cpsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SignatureecpsInput = z.infer<typeof signature_e_cpsSchema>
```
...
```bash
pnpm test src/lib/services/signature-e-cps.test.ts
```
...
- [ ] Un appel à « ANS — e-CPS » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/signature-e-cps.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/signature-e-cps.spec.ts
```
...
- Documentation officielle de **ANS — e-CPS** (à vérifier au démarrage)
