# US-2197 — Génération PDF/A-3 ordonnance
...
Quand il accède à la fonctionnalité « Génération PDF/A-3 ordonnance »
...
Quand il tente d'accéder à « Génération PDF/A-3 ordonnance »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name generation-pdf-a-3-ordonnance`
...
export const generation_pdf_a_3_ordonnanceSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Generationpdfa3OrdonnanceInput = z.infer<typeof generation_pdf_a_3_ordonnanceSchema>
```
...
```bash
pnpm test src/lib/services/generation-pdf-a-3-ordonnance.test.ts
```
...
```bash
pnpm test:integration tests/integration/generation-pdf-a-3-ordonnance.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/generation-pdf-a-3-ordonnance.spec.ts
```
