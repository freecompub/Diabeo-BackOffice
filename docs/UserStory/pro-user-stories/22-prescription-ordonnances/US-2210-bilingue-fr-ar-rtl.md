...
Quand il accède à la fonctionnalité « Bilingue FR/AR (RTL) »
...
Quand il tente d'accéder à « Bilingue FR/AR (RTL) »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name bilingue-fr-ar-rtl`
...
export const bilingue_fr_ar_rtlSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type BilinguefrarrtlInput = z.infer<typeof bilingue_fr_ar_rtlSchema>
```
...
```bash
pnpm test src/lib/services/bilingue-fr-ar-rtl.test.ts
```
...
```bash
pnpm test:integration tests/integration/bilingue-fr-ar-rtl.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/bilingue-fr-ar-rtl.spec.ts
```
