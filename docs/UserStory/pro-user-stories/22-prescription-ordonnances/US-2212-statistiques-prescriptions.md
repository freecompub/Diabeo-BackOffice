# US-2212 — Statistiques prescriptions

> 📌 **22. Prescription & ordonnances** · Priorité **V2** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Statistiques prescriptions »
...
Quand il tente d'accéder à « Statistiques prescriptions »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name statistiques-prescriptions`
...
export const statistiques_prescriptionsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type StatistiquesprescriptionsInput = z.infer<typeof statistiques_prescriptionsSchema>
```
...
```bash
pnpm test src/lib/services/statistiques-prescriptions.test.ts
```
...
```bash
pnpm test:integration tests/integration/statistiques-prescriptions.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/statistiques-prescriptions.spec.ts
```
