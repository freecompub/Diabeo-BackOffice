# US-2173 — Certification HAS LAP propre

> 📌 **22. Prescription & ordonnances** · Priorité **V4** · Pays **FR**
> 
> 💬 **Note inventaire** : Long et cher
...
|---|---|
| **ID** | `US-2173` |
| **Référence inventaire** | `FN-173` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | SGS / LNE / Bureau Veritas |
| **Modèle économique** | Payant audit |
| **Coût estimé** | ~50-200 k€ + maintenance |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — SGS / LNE / Bureau Veritas
...
Quand il accède à la fonctionnalité « Certification HAS LAP propre »
...
Quand il tente d'accéder à « Certification HAS LAP propre »
...
```gherkin
Étant donné Le service externe « SGS / LNE / Bureau Veritas » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « SGS / LNE / Bureau Veritas » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name certification-has-lap-propre`
...
export const certification_has_lap_propreSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CertificationhaslappropreInput = z.infer<typeof certification_has_lap_propreSchema>
```
...
```bash
pnpm test src/lib/services/certification-has-lap-propre.test.ts
```
...
- [ ] Un appel à « SGS / LNE / Bureau Veritas » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/certification-has-lap-propre.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/certification-has-lap-propre.spec.ts
```
...
- Documentation officielle de **SGS / LNE / Bureau Veritas** (à vérifier au démarrage)
