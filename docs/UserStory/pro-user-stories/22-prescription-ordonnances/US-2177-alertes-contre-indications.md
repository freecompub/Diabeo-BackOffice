# US-2177 — Alertes contre-indications
...
Quand il accède à la fonctionnalité « Alertes contre-indications »
...
Quand il tente d'accéder à « Alertes contre-indications »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name alertes-contre-indications`
...
export const alertes_contre_indicationsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AlertescontreindicationsInput = z.infer<typeof alertes_contre_indicationsSchema>
```
...
```bash
pnpm test src/lib/services/alertes-contre-indications.test.ts
```
...
```bash
pnpm test:integration tests/integration/alertes-contre-indications.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/alertes-contre-indications.spec.ts
```
