# US-2193 — Cachet électronique cabinet
...
|---|---|
| **ID** | `US-2193` |
| **Référence inventaire** | `FN-193` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Prestataire eIDAS |
| **Modèle économique** | Payant |
| **Coût estimé** | ~100-500€/an certif |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Prestataire eIDAS
...
Quand il accède à la fonctionnalité « Cachet électronique cabinet »
...
Quand il tente d'accéder à « Cachet électronique cabinet »
...
```gherkin
Étant donné Le service externe « Prestataire eIDAS » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Prestataire eIDAS » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name cachet-electronique-cabinet`
...
export const cachet_electronique_cabinetSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CachetelectroniquecabinetInput = z.infer<typeof cachet_electronique_cabinetSchema>
```
...
```bash
pnpm test src/lib/services/cachet-electronique-cabinet.test.ts
```
...
- [ ] Un appel à « Prestataire eIDAS » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/cachet-electronique-cabinet.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/cachet-electronique-cabinet.spec.ts
```
...
- Documentation officielle de **Prestataire eIDAS** (à vérifier au démarrage)
