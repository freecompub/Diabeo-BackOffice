# US-2188 — Prescription consult diét/podo
...
Quand il accède à la fonctionnalité « Prescription consult diét/podo »
...
Quand il tente d'accéder à « Prescription consult diét/podo »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-consult-diet-podo`
...
export const prescription_consult_diet_podoSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptionconsultdietpodoInput = z.infer<typeof prescription_consult_diet_podoSchema>
```
...
```bash
pnpm test src/lib/services/prescription-consult-diet-podo.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-consult-diet-podo.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-consult-diet-podo.spec.ts
```
