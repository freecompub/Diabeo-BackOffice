# US-2196 — DataMatrix 2D-Doc ANTS
...
|---|---|
| **ID** | `US-2196` |
| **Référence inventaire** | `FN-196` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Norme ANTS (libs gratuites) |
| **Modèle économique** | Gratuit (norme ouverte) |
| **Coût estimé** | Gratuit |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Norme ANTS (libs gratuites)
...
Quand il accède à la fonctionnalité « DataMatrix 2D-Doc ANTS »
...
Quand il tente d'accéder à « DataMatrix 2D-Doc ANTS »
...
```gherkin
Étant donné Le service externe « Norme ANTS (libs gratuites) » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Norme ANTS (libs gratuites) » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name datamatrix-2d-doc-ants`
...
export const datamatrix_2d_doc_antsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Datamatrix2DdocantsInput = z.infer<typeof datamatrix_2d_doc_antsSchema>
```
...
```bash
pnpm test src/lib/services/datamatrix-2d-doc-ants.test.ts
```
...
- [ ] Un appel à « Norme ANTS (libs gratuites) » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/datamatrix-2d-doc-ants.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/datamatrix-2d-doc-ants.spec.ts
```
...
- Documentation officielle de **Norme ANTS (libs gratuites)** (à vérifier au démarrage)
