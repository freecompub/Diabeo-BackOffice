# US-2199 — Format CERFA bizone ALD

> 📌 **22. Prescription & ordonnances** · Priorité **V1** · Pays **FR**
> 
> 💬 **Note inventaire** : Diabète = ALD 8
...
|---|---|
| **ID** | `US-2199` |
| **Référence inventaire** | `FN-199` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (template officiel) |
...
Quand il accède à la fonctionnalité « Format CERFA bizone ALD »
...
Quand il tente d'accéder à « Format CERFA bizone ALD »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name format-cerfa-bizone-ald`
...
export const format_cerfa_bizone_aldSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type FormatcerfabizonealdInput = z.infer<typeof format_cerfa_bizone_aldSchema>
```
...
```bash
pnpm test src/lib/services/format-cerfa-bizone-ald.test.ts
```
...
```bash
pnpm test:integration tests/integration/format-cerfa-bizone-ald.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/format-cerfa-bizone-ald.spec.ts
```
