# US-2187 — Prescription IDE à domicile
...
Quand il accède à la fonctionnalité « Prescription IDE à domicile »
...
Quand il tente d'accéder à « Prescription IDE à domicile »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-ide-a-domicile`
...
export const prescription_ide_a_domicileSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptionideadomicileInput = z.infer<typeof prescription_ide_a_domicileSchema>
```
...
```bash
pnpm test src/lib/services/prescription-ide-a-domicile.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-ide-a-domicile.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-ide-a-domicile.spec.ts
```
