# US-2169 — Éditeur ordonnance structurée

> 📌 **22. Prescription & ordonnances** · Priorité **V1** · Pays **FR+DZ**
> 
> 💬 **Note inventaire** : Briques métier
...
Quand il accède à la fonctionnalité « Éditeur ordonnance structurée »
...
Quand il tente d'accéder à « Éditeur ordonnance structurée »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name editeur-ordonnance-structuree`
...
export const editeur_ordonnance_structureeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type EditeurordonnancestructureeInput = z.infer<typeof editeur_ordonnance_structureeSchema>
```
...
```bash
pnpm test src/lib/services/editeur-ordonnance-structuree.test.ts
```
...
```bash
pnpm test:integration tests/integration/editeur-ordonnance-structuree.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/editeur-ordonnance-structuree.spec.ts
```
