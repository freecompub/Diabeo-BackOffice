# US-2209 — Format ordonnance Algérie

> 📌 **22. Prescription & ordonnances** · Priorité **V2** · Pays **DZ**
> 
> 💬 **Note inventaire** : Réglementation locale DZ
...
|---|---|
| **ID** | `US-2209` |
| **Référence inventaire** | `FN-209` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V2** |
| **Pays cible** | DZ |
| **Intégration externe** | Partiel |
| **Service / Standard** | Référentiel LNCPP (à vérifier) |
| **Modèle économique** | À étudier |
...
### 🌐 Intégration externe — Référentiel LNCPP (à vérifier)
...
Quand il accède à la fonctionnalité « Format ordonnance Algérie »
...
Quand il tente d'accéder à « Format ordonnance Algérie »
...
```gherkin
Étant donné Le service externe « Référentiel LNCPP (à vérifier) » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Référentiel LNCPP (à vérifier) » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name format-ordonnance-algerie`
...
export const format_ordonnance_algerieSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type FormatordonnancealgerieInput = z.infer<typeof format_ordonnance_algerieSchema>
```
...
```bash
pnpm test src/lib/services/format-ordonnance-algerie.test.ts
```
...
- [ ] Un appel à « Référentiel LNCPP (à vérifier) » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/format-ordonnance-algerie.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/format-ordonnance-algerie.spec.ts
```
...
- Documentation officielle de **Référentiel LNCPP (à vérifier)** (à vérifier au démarrage)
