# US-2183 — Prescription glucagon urgence
...
Quand il accède à la fonctionnalité « Prescription glucagon urgence »
...
Quand il tente d'accéder à « Prescription glucagon urgence »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-glucagon-urgence`
...
export const prescription_glucagon_urgenceSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptionglucagonurgenceInput = z.infer<typeof prescription_glucagon_urgenceSchema>
```
...
```bash
pnpm test src/lib/services/prescription-glucagon-urgence.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-glucagon-urgence.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-glucagon-urgence.spec.ts
```
