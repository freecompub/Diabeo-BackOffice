# US-2171 — Base médic. publique gratuite

> 📌 **22. Prescription & ordonnances** · Priorité **MVP/POC** · Pays **FR**
> 
> 💬 **Note inventaire** : NON certifiée pour LAP
...
### 🌐 Intégration externe — ANSM/HAS — BDPM
...
Quand il accède à la fonctionnalité « Base médic. publique gratuite »
...
Quand il tente d'accéder à « Base médic. publique gratuite »
...
```gherkin
Étant donné Le service externe « ANSM/HAS — BDPM » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANSM/HAS — BDPM » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name base-medic-publique-gratuite`
...
export const base_medic_publique_gratuiteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type BasemedicpubliquegratuiteInput = z.infer<typeof base_medic_publique_gratuiteSchema>
```
...
```bash
pnpm test src/lib/services/base-medic-publique-gratuite.test.ts
```
...
- [ ] Un appel à « ANSM/HAS — BDPM » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/base-medic-publique-gratuite.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/base-medic-publique-gratuite.spec.ts
```
...
- Documentation officielle de **ANSM/HAS — BDPM** (à vérifier au démarrage)
