...
|---|---|
| **ID** | `US-2204` |
| **Référence inventaire** | `FN-204` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Mailiz / Apicrypt |
...
### 🌐 Intégration externe — Mailiz / Apicrypt
...
Quand il accède à la fonctionnalité « Envoi via MSSanté »
...
Quand il tente d'accéder à « Envoi via MSSanté »
...
```gherkin
Étant donné Le service externe « Mailiz / Apicrypt » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Mailiz / Apicrypt » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name envoi-via-mssante`
...
export const envoi_via_mssanteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type EnvoiviamssanteInput = z.infer<typeof envoi_via_mssanteSchema>
```
...
```bash
pnpm test src/lib/services/envoi-via-mssante.test.ts
```
...
- [ ] Un appel à « Mailiz / Apicrypt » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/envoi-via-mssante.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/envoi-via-mssante.spec.ts
```
...
- Documentation officielle de **Mailiz / Apicrypt** (à vérifier au démarrage)
