...
Séances éducation thérapeutique
...
Quand il accède à la fonctionnalité « Prescription ETP »
...
Quand il tente d'accéder à « Prescription ETP »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-etp`
...
export const prescription_etpSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptionetpInput = z.infer<typeof prescription_etpSchema>
```
...
```bash
pnpm test src/lib/services/prescription-etp.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-etp.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-etp.spec.ts
```
