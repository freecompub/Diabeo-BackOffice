# US-2198 — Verrouillage post-signature
...
Quand il accède à la fonctionnalité « Verrouillage post-signature »
...
Quand il tente d'accéder à « Verrouillage post-signature »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name verrouillage-post-signature`
...
export const verrouillage_post_signatureSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type VerrouillagepostsignatureInput = z.infer<typeof verrouillage_post_signatureSchema>
```
...
```bash
pnpm test src/lib/services/verrouillage-post-signature.test.ts
```
...
```bash
pnpm test:integration tests/integration/verrouillage-post-signature.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/verrouillage-post-signature.spec.ts
```
