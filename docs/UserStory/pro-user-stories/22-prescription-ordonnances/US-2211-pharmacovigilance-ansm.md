# US-2211 — Pharmacovigilance ANSM

> 📌 **22. Prescription & ordonnances** · Priorité **V2** · Pays **FR**
> 
> 💬 **Note inventaire** : Manuel ou API
...
|---|---|
| **ID** | `US-2211` |
| **Référence inventaire** | `FN-211` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Portail ANSM eRefMed |
...
Déclaration effets indésirables
...
### 🌐 Intégration externe — Portail ANSM eRefMed
...
Quand il accède à la fonctionnalité « Pharmacovigilance ANSM »
...
Quand il tente d'accéder à « Pharmacovigilance ANSM »
...
```gherkin
Étant donné Le service externe « Portail ANSM eRefMed » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Portail ANSM eRefMed » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name pharmacovigilance-ansm`
...
export const pharmacovigilance_ansmSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PharmacovigilanceansmInput = z.infer<typeof pharmacovigilance_ansmSchema>
```
...
```bash
pnpm test src/lib/services/pharmacovigilance-ansm.test.ts
```
...
- [ ] Un appel à « Portail ANSM eRefMed » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/pharmacovigilance-ansm.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/pharmacovigilance-ansm.spec.ts
```
...
- Documentation officielle de **Portail ANSM eRefMed** (à vérifier au démarrage)
