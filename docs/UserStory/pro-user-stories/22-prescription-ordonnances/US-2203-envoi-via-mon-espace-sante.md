# US-2203 — Envoi via Mon Espace Santé
...
### 🌐 Intégration externe — ANS — DMP/MES
...
Quand il accède à la fonctionnalité « Envoi via Mon Espace Santé »
...
Quand il tente d'accéder à « Envoi via Mon Espace Santé »
...
```gherkin
Étant donné Le service externe « ANS — DMP/MES » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS — DMP/MES » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name envoi-via-mon-espace-sante`
...
export const envoi_via_mon_espace_santeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type EnvoiviamonespacesanteInput = z.infer<typeof envoi_via_mon_espace_santeSchema>
```
...
```bash
pnpm test src/lib/services/envoi-via-mon-espace-sante.test.ts
```
...
- [ ] Un appel à « ANS — DMP/MES » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/envoi-via-mon-espace-sante.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/envoi-via-mon-espace-sante.spec.ts
```
...
- Documentation officielle de **ANS — DMP/MES** (à vérifier au démarrage)
