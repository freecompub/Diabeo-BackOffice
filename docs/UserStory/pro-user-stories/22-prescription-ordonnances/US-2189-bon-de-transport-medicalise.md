# US-2189 — Bon de transport médicalisé
...
Quand il accède à la fonctionnalité « Bon de transport médicalisé »
...
Quand il tente d'accéder à « Bon de transport médicalisé »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name bon-de-transport-medicalise`
...
export const bon_de_transport_medicaliseSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type BondetransportmedicaliseInput = z.infer<typeof bon_de_transport_medicaliseSchema>
```
...
```bash
pnpm test src/lib/services/bon-de-transport-medicalise.test.ts
```
...
```bash
pnpm test:integration tests/integration/bon-de-transport-medicalise.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/bon-de-transport-medicalise.spec.ts
```
