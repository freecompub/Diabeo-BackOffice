# US-2202 — Transmission e-prescription nationale

> 📌 **22. Prescription & ordonnances** · Priorité **V1** · Pays **FR**
> 
> 💬 **Note inventaire** : Pré-requis: LAP certifié
...
|---|---|
| **ID** | `US-2202` |
| **Référence inventaire** | `FN-202` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | ANS — Service e-Prescription |
| **Modèle économique** | Gratuit (CNDA agrément requis) |
...
### 🌐 Intégration externe — ANS — Service e-Prescription
...
Quand il accède à la fonctionnalité « Transmission e-prescription nationale »
...
Quand il tente d'accéder à « Transmission e-prescription nationale »
...
```gherkin
Étant donné Le service externe « ANS — Service e-Prescription » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « ANS — Service e-Prescription » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name transmission-e-prescription-nationale`
...
export const transmission_e_prescription_nationaleSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TransmissioneprescriptionnationaleInput = z.infer<typeof transmission_e_prescription_nationaleSchema>
```
...
```bash
pnpm test src/lib/services/transmission-e-prescription-nationale.test.ts
```
...
- [ ] Un appel à « ANS — Service e-Prescription » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/transmission-e-prescription-nationale.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/transmission-e-prescription-nationale.spec.ts
```
...
- Documentation officielle de **ANS — Service e-Prescription** (à vérifier au démarrage)
