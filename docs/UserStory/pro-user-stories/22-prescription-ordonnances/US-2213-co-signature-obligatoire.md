# US-2213 — Co-signature obligatoire
...
Quand il accède à la fonctionnalité « Co-signature obligatoire »
...
Quand il tente d'accéder à « Co-signature obligatoire »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name co-signature-obligatoire`
...
export const co_signature_obligatoireSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CosignatureobligatoireInput = z.infer<typeof co_signature_obligatoireSchema>
```
...
```bash
pnpm test src/lib/services/co-signature-obligatoire.test.ts
```
...
```bash
pnpm test:integration tests/integration/co-signature-obligatoire.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/co-signature-obligatoire.spec.ts
```
