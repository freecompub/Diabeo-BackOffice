# US-2195 — QR code vérif authenticité
...
Quand il accède à la fonctionnalité « QR code vérif authenticité »
...
Quand il tente d'accéder à « QR code vérif authenticité »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name qr-code-verif-authenticite`
...
export const qr_code_verif_authenticiteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type QrcodeverifauthenticiteInput = z.infer<typeof qr_code_verif_authenticiteSchema>
```
...
```bash
pnpm test src/lib/services/qr-code-verif-authenticite.test.ts
```
...
```bash
pnpm test:integration tests/integration/qr-code-verif-authenticite.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/qr-code-verif-authenticite.spec.ts
```
