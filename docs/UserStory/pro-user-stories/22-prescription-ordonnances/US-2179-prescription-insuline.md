...
Quand il accède à la fonctionnalité « Prescription insuline »
...
Quand il tente d'accéder à « Prescription insuline »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-insuline`
...
export const prescription_insulineSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptioninsulineInput = z.infer<typeof prescription_insulineSchema>
```
...
```bash
pnpm test src/lib/services/prescription-insuline.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-insuline.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-insuline.spec.ts
```
