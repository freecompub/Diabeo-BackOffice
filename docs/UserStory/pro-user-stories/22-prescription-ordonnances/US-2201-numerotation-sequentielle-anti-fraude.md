# US-2201 — Numérotation séquentielle anti-fraude
...
Quand il accède à la fonctionnalité « Numérotation séquentielle anti-fraude »
...
Quand il tente d'accéder à « Numérotation séquentielle anti-fraude »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name numerotation-sequentielle-anti-fraude`
...
export const numerotation_sequentielle_anti_fraudeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type NumerotationsequentielleantifraudeInput = z.infer<typeof numerotation_sequentielle_anti_fraudeSchema>
```
...
```bash
pnpm test src/lib/services/numerotation-sequentielle-anti-fraude.test.ts
```
...
```bash
pnpm test:integration tests/integration/numerotation-sequentielle-anti-fraude.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/numerotation-sequentielle-anti-fraude.spec.ts
```
