# US-2172 — LAP certifié HAS turnkey

> 📌 **22. Prescription & ordonnances** · Priorité **V4** · Pays **FR**
> 
> 💬 **Note inventaire** : Évite certif HAS interne
...
|---|---|
| **ID** | `US-2172` |
| **Référence inventaire** | `FN-172` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Synapse Medicine / Ordoclic / Posos |
| **Modèle économique** | Payant SaaS |
| **Coût estimé** | ~19-50€/mois/PS |
| **Statut** | 🆕 À démarrer |
...
Module e-prescription clé en main
...
### 🌐 Intégration externe — Synapse Medicine / Ordoclic / Posos
...
Quand il accède à la fonctionnalité « LAP certifié HAS turnkey »
...
Quand il tente d'accéder à « LAP certifié HAS turnkey »
...
```gherkin
Étant donné Le service externe « Synapse Medicine / Ordoclic / Posos » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Synapse Medicine / Ordoclic / Posos » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name lap-certifie-has-turnkey`
...
export const lap_certifie_has_turnkeySchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type LapcertifiehasturnkeyInput = z.infer<typeof lap_certifie_has_turnkeySchema>
```
...
```bash
pnpm test src/lib/services/lap-certifie-has-turnkey.test.ts
```
...
- [ ] Un appel à « Synapse Medicine / Ordoclic / Posos » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/lap-certifie-has-turnkey.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/lap-certifie-has-turnkey.spec.ts
```
...
- Documentation officielle de **Synapse Medicine / Ordoclic / Posos** (à vérifier au démarrage)
