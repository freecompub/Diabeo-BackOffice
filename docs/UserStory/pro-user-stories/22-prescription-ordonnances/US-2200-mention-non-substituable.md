# US-2200 — Mention non substituable
...
Quand il accède à la fonctionnalité « Mention non substituable »
...
Quand il tente d'accéder à « Mention non substituable »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name mention-non-substituable`
...
export const mention_non_substituableSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type MentionnonsubstituableInput = z.infer<typeof mention_non_substituableSchema>
```
...
```bash
pnpm test src/lib/services/mention-non-substituable.test.ts
```
...
```bash
pnpm test:integration tests/integration/mention-non-substituable.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/mention-non-substituable.spec.ts
```
