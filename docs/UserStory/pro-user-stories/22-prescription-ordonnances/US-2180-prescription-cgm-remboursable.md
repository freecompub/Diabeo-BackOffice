# US-2180 — Prescription CGM remboursable
...
|---|---|
| **ID** | `US-2180` |
| **Référence inventaire** | `FN-180` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Partiel |
| **Service / Standard** | Interne + référentiel LPP CNAM |
| **Modèle économique** | Gratuit (LPP open data) |
| **Coût estimé** | — |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Interne + référentiel LPP CNAM
...
Quand il accède à la fonctionnalité « Prescription CGM remboursable »
...
Quand il tente d'accéder à « Prescription CGM remboursable »
...
```gherkin
Étant donné Le service externe « Interne + référentiel LPP CNAM » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Interne + référentiel LPP CNAM » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-cgm-remboursable`
...
export const prescription_cgm_remboursableSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptioncgmremboursableInput = z.infer<typeof prescription_cgm_remboursableSchema>
```
...
```bash
pnpm test src/lib/services/prescription-cgm-remboursable.test.ts
```
...
- [ ] Un appel à « Interne + référentiel LPP CNAM » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/prescription-cgm-remboursable.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-cgm-remboursable.spec.ts
```
...
- Documentation officielle de **Interne + référentiel LPP CNAM** (à vérifier au démarrage)
