# US-2170 — Base médicamenteuse certifiée HAS

> 📌 **22. Prescription & ordonnances** · Priorité **V4** · Pays **FR**
> 
> 💬 **Note inventaire** : Coût significatif
...
|---|---|
| **ID** | `US-2170` |
| **Référence inventaire** | `FN-170` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Vidal / BCB / Thériaque / Thésorimed / Posos |
| **Modèle économique** | Payant licence éditeur |
| **Coût estimé** | Variable, négocié |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Vidal / BCB / Thériaque / Thésorimed / Posos
...
Quand il accède à la fonctionnalité « Base médicamenteuse certifiée HAS »
...
Quand il tente d'accéder à « Base médicamenteuse certifiée HAS »
...
```gherkin
Étant donné Le service externe « Vidal / BCB / Thériaque / Thésorimed / Posos » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Vidal / BCB / Thériaque / Thésorimed / Posos » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name base-medicamenteuse-certifiee-has`
...
export const base_medicamenteuse_certifiee_hasSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type BasemedicamenteusecertifieehasInput = z.infer<typeof base_medicamenteuse_certifiee_hasSchema>
```
...
- [ ] Service métier lève l'exception attendue en cas de violation de règle (RM-1 à RM-8)
...
```bash
pnpm test src/lib/services/base-medicamenteuse-certifiee-has.test.ts
```
...
- [ ] Un appel à « Vidal / BCB / Thériaque / Thésorimed / Posos » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/base-medicamenteuse-certifiee-has.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/base-medicamenteuse-certifiee-has.spec.ts
```
...
- Documentation officielle de **Vidal / BCB / Thériaque / Thésorimed / Posos** (à vérifier au démarrage)
