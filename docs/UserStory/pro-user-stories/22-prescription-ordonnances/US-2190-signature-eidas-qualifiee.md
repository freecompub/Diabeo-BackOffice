# US-2190 — Signature eIDAS qualifiée

> 📌 **22. Prescription & ordonnances** · Priorité **V4** · Pays **FR**
> 
> 💬 **Note inventaire** : Voir domaine 17 aussi
...
|---|---|
| **ID** | `US-2190` |
| **Référence inventaire** | `FN-190` |
| **Domaine** | 22. Prescription & ordonnances |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | YouSign / Universign / Lex Persona |
| **Modèle économique** | Payant |
| **Coût estimé** | ~1-3€/signature ou abo |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — YouSign / Universign / Lex Persona
...
Quand il accède à la fonctionnalité « Signature eIDAS qualifiée »
...
Quand il tente d'accéder à « Signature eIDAS qualifiée »
...
```gherkin
Étant donné Le service externe « YouSign / Universign / Lex Persona » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « YouSign / Universign / Lex Persona » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name signature-eidas-qualifiee`
...
export const signature_eidas_qualifieeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SignatureeidasqualifieeInput = z.infer<typeof signature_eidas_qualifieeSchema>
```
...
```bash
pnpm test src/lib/services/signature-eidas-qualifiee.test.ts
```
...
- [ ] Un appel à « YouSign / Universign / Lex Persona » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/signature-eidas-qualifiee.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/signature-eidas-qualifiee.spec.ts
```
...
- Documentation officielle de **YouSign / Universign / Lex Persona** (à vérifier au démarrage)
