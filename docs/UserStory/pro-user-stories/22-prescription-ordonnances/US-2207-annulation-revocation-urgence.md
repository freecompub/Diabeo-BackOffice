# US-2207 — Annulation / révocation urgence
...
Quand il accède à la fonctionnalité « Annulation / révocation urgence »
...
Quand il tente d'accéder à « Annulation / révocation urgence »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name annulation-revocation-urgence`
...
export const annulation_revocation_urgenceSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AnnulationrevocationurgenceInput = z.infer<typeof annulation_revocation_urgenceSchema>
```
...
```bash
pnpm test src/lib/services/annulation-revocation-urgence.test.ts
```
...
```bash
pnpm test:integration tests/integration/annulation-revocation-urgence.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/annulation-revocation-urgence.spec.ts
```
