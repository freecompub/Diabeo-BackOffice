# US-2182 — Prescription bandelettes / aiguilles

> 📌 **22. Prescription & ordonnances** · Priorité **V1** · Pays **FR+DZ**
> 
...
Quand il accède à la fonctionnalité « Prescription bandelettes / aiguilles »
...
Quand il tente d'accéder à « Prescription bandelettes / aiguilles »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name prescription-bandelettes-aiguilles`
...
export const prescription_bandelettes_aiguillesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PrescriptionbandelettesaiguillesInput = z.infer<typeof prescription_bandelettes_aiguillesSchema>
```
...
```bash
pnpm test src/lib/services/prescription-bandelettes-aiguilles.test.ts
```
...
```bash
pnpm test:integration tests/integration/prescription-bandelettes-aiguilles.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/prescription-bandelettes-aiguilles.spec.ts
```
