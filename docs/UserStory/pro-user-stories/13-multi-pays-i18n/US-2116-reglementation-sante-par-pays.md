...
> 📌 **13. Multi-pays & i18n** · Priorité **V1** · Pays **FR+DZ**
> 
...
Quand il accède à la fonctionnalité « Réglementation santé par pays »
...
Quand il tente d'accéder à « Réglementation santé par pays »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name reglementation-sante-par-pays`
...
export const reglementation_sante_par_paysSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ReglementationsanteparpaysInput = z.infer<typeof reglementation_sante_par_paysSchema>
```
...
```bash
pnpm test src/lib/services/reglementation-sante-par-pays.test.ts
```
...
```bash
pnpm test:integration tests/integration/reglementation-sante-par-pays.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/reglementation-sante-par-pays.spec.ts
```
