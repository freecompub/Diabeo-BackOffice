# US-2115 — Formats date/nombre localisés
...
Quand il accède à la fonctionnalité « Formats date/nombre localisés »
...
Quand il tente d'accéder à « Formats date/nombre localisés »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name formats-date-nombre-localises`
...
export const formats_date_nombre_localisesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type FormatsdatenombrelocalisesInput = z.infer<typeof formats_date_nombre_localisesSchema>
```
...
```bash
pnpm test src/lib/services/formats-date-nombre-localises.test.ts
```
...
```bash
pnpm test:integration tests/integration/formats-date-nombre-localises.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/formats-date-nombre-localises.spec.ts
```
