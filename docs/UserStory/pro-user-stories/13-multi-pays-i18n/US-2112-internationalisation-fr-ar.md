# US-2112 — Internationalisation FR/AR

> 📌 **13. Multi-pays & i18n** · Priorité **MVP** · Pays **FR+DZ**
> 
> 💬 **Note inventaire** : Dossier messages/ existe
...
Quand il accède à la fonctionnalité « Internationalisation FR/AR »
...
Quand il tente d'accéder à « Internationalisation FR/AR »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name internationalisation-fr-ar`
...
export const internationalisation_fr_arSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type InternationalisationfrarInput = z.infer<typeof internationalisation_fr_arSchema>
```
...
```bash
pnpm test src/lib/services/internationalisation-fr-ar.test.ts
```
...
```bash
pnpm test:integration tests/integration/internationalisation-fr-ar.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/internationalisation-fr-ar.spec.ts
```
