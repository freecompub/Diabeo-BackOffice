...
Quand il tente d'accéder à « Devises EUR / DZD »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name devises-eur-dzd`
...
export const devises_eur_dzdSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DeviseseurdzdInput = z.infer<typeof devises_eur_dzdSchema>
```
...
```bash
pnpm test src/lib/services/devises-eur-dzd.test.ts
```
...
```bash
pnpm test:integration tests/integration/devises-eur-dzd.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/devises-eur-dzd.spec.ts
```
