...
Quand il accède à la fonctionnalité « Règles fiscales par pays »
...
Quand il tente d'accéder à « Règles fiscales par pays »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name regles-fiscales-par-pays`
...
export const regles_fiscales_par_paysSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ReglesfiscalesparpaysInput = z.infer<typeof regles_fiscales_par_paysSchema>
```
...
```bash
pnpm test src/lib/services/regles-fiscales-par-pays.test.ts
```
...
```bash
pnpm test:integration tests/integration/regles-fiscales-par-pays.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/regles-fiscales-par-pays.spec.ts
```
