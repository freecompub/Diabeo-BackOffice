# US-2069 — Génération ordonnance numérique

> 📌 **7. Téléconsult & ajustements** · Priorité **V4** · Pays **FR**
> 
> 💬 **Note inventaire** : Chantier majeur
...
|---|---|
| **ID** | `US-2069` |
| **Référence inventaire** | `FN-069` |
| **Domaine** | 7. Téléconsult & ajustements |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Voir domaine 22 (Prescription) |
| **Modèle économique** | Payant |
| **Coût estimé** | Voir détail |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Voir domaine 22 (Prescription)
...
Quand il accède à la fonctionnalité « Génération ordonnance numérique »
...
Quand il tente d'accéder à « Génération ordonnance numérique »
Alors la requête est rejetée avec HTTP 401/403, et la tentative est journalisée
```

### AC-3 — Workflow proposition 3 étapes

```gherkin
Étant donné Un médecin initie une proposition
Quand il franchit Analyse → Paramétrage → Confirmation
Alors chaque étape valide la précédente, l'audit log enregistre chaque transition
```

### AC-4 — Acceptation patient explicite requise

```gherkin
Étant donné Une proposition est créée
Quand le patient n'a pas encore répondu
Alors aucune modification automatique n'est appliquée à son dossier
```

### AC-5 — Tolérance aux pannes externes

```gherkin
Étant donné Le service externe « Voir domaine 22 (Prescription) » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Voir domaine 22 (Prescription) » n'est exposé côté client. Tous les appels passent par le backend.**
...
model AdjustmentProposal {
  ...
  patientId       String
  doctorId        String
  status          ProposalStatus  @default(PENDING) // PENDING | ACCEPTED | REJECTED | EXPIRED
  beforeSnapshot  Json     // Snapshot config avant
  afterProposal   Json     // Configuration proposée
  rationale       String   // Justification clinique
  patientResponseAt DateTime?
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name generation-ordonnance-numerique`
...
```
/api/patients/[id]/proposals
```

| Méthode | Endpoint | Auth | Rôles autorisés | Description |
|---------|----------|------|-----------------|-------------|
| GET     | `/api/patients/[id]/proposals` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/patients/[id]/proposals` | JWT | Selon AC-1 | Création |
| PUT     | `/api/patients/[id]/proposals/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/patients/[id]/proposals/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const generation_ordonnance_numeriqueSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type GenerationordonnancenumeriqueInput = z.infer<typeof generation_ordonnance_numeriqueSchema>
```
...
```bash
pnpm test src/lib/services/generation-ordonnance-numerique.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/patients/[id]/proposals` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/patients/[id]/proposals` renvoie 401 sans JWT valide
- [ ] `/api/patients/[id]/proposals` renvoie 403 avec un rôle insuffisant
- [ ] `/api/patients/[id]/proposals` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Voir domaine 22 (Prescription) » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/generation-ordonnance-numerique.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/generation-ordonnance-numerique.spec.ts
```
...
- Documentation officielle de **Voir domaine 22 (Prescription)** (à vérifier au démarrage)
