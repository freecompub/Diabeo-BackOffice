# US-2071 — Templates consultation
...
Quand il accède à la fonctionnalité « Templates consultation »
...
Quand il tente d'accéder à « Templates consultation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name templates-consultation`
...
export const templates_consultationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TemplatesconsultationInput = z.infer<typeof templates_consultationSchema>
```
...
```bash
pnpm test src/lib/services/templates-consultation.test.ts
```
...
```bash
pnpm test:integration tests/integration/templates-consultation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/templates-consultation.spec.ts
```
