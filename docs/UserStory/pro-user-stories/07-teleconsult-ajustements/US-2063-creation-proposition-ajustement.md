# US-2063 — Création proposition ajustement

> 📌 **7. Téléconsult & ajustements** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Table existante
...
Quand il accède à la fonctionnalité « Création proposition ajustement »
...
Quand il tente d'accéder à « Création proposition ajustement »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name creation-proposition-ajustement`
...
export const creation_proposition_ajustementSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CreationpropositionajustementInput = z.infer<typeof creation_proposition_ajustementSchema>
```
...
```bash
pnpm test src/lib/services/creation-proposition-ajustement.test.ts
```
...
```bash
pnpm test:integration tests/integration/creation-proposition-ajustement.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/creation-proposition-ajustement.spec.ts
```
