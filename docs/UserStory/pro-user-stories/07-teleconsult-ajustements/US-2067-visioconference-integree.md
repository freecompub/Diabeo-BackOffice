...
> 
> 💬 **Note inventaire** : Jitsi self-hosted = gratuit
...
|---|---|
| **ID** | `US-2067` |
| **Référence inventaire** | `FN-067` |
| **Domaine** | 7. Téléconsult & ajustements |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Daily.co / Twilio Video / Whereby / Jitsi |
| **Modèle économique** | Freemium / Payant |
| **Coût estimé** | 0–0.004€/min |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Daily.co / Twilio Video / Whereby / Jitsi
...
Quand il accède à la fonctionnalité « Visioconférence intégrée »
...
Quand il tente d'accéder à « Visioconférence intégrée »
...
```gherkin
Étant donné Le service externe « Daily.co / Twilio Video / Whereby / Jitsi » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Daily.co / Twilio Video / Whereby / Jitsi » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name visioconference-integree`
...
export const visioconference_integreeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type VisioconferenceintegreeInput = z.infer<typeof visioconference_integreeSchema>
```
...
```bash
pnpm test src/lib/services/visioconference-integree.test.ts
```
...
- [ ] Un appel à « Daily.co / Twilio Video / Whereby / Jitsi » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/visioconference-integree.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/visioconference-integree.spec.ts
```
...
- Documentation officielle de **Daily.co / Twilio Video / Whereby / Jitsi** (à vérifier au démarrage)
