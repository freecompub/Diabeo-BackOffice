# US-2072 — Facturation acte téléconsult

> 📌 **7. Téléconsult & ajustements** · Priorité **V1** · Pays **FR**
> 
> 💬 **Note inventaire** : Tarifs CPAM
...
Quand il accède à la fonctionnalité « Facturation acte téléconsult »
...
Quand il tente d'accéder à « Facturation acte téléconsult »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name facturation-acte-teleconsult`
...
export const facturation_acte_teleconsultSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type FacturationacteteleconsultInput = z.infer<typeof facturation_acte_teleconsultSchema>
```
...
```bash
pnpm test src/lib/services/facturation-acte-teleconsult.test.ts
```
...
```bash
pnpm test:integration tests/integration/facturation-acte-teleconsult.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/facturation-acte-teleconsult.spec.ts
```
