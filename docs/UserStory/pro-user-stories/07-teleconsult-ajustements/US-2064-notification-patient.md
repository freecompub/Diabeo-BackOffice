# US-2064 — Notification patient

> 📌 **7. Téléconsult & ajustements** · Priorité **MVP** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2064` |
| **Référence inventaire** | `FN-064` |
| **Domaine** | 7. Téléconsult & ajustements |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | FCM / SendGrid / Twilio |
| **Modèle économique** | Selon canal |
...
### 🌐 Intégration externe — FCM / SendGrid / Twilio
...
Quand il accède à la fonctionnalité « Notification patient »
...
Quand il tente d'accéder à « Notification patient »
...
```gherkin
Étant donné Le service externe « FCM / SendGrid / Twilio » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « FCM / SendGrid / Twilio » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name notification-patient`
...
export const notification_patientSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type NotificationpatientInput = z.infer<typeof notification_patientSchema>
```
...
```bash
pnpm test src/lib/services/notification-patient.test.ts
```
...
- [ ] Un appel à « FCM / SendGrid / Twilio » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/notification-patient.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/notification-patient.spec.ts
```
...
- Documentation officielle de **FCM / SendGrid / Twilio** (à vérifier au démarrage)
