# US-2065 — Accusé de réception patient
...
Quand il accède à la fonctionnalité « Accusé de réception patient »
...
Quand il tente d'accéder à « Accusé de réception patient »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name accuse-de-reception-patient`
...
export const accuse_de_reception_patientSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AccusedereceptionpatientInput = z.infer<typeof accuse_de_reception_patientSchema>
```
...
```bash
pnpm test src/lib/services/accuse-de-reception-patient.test.ts
```
...
```bash
pnpm test:integration tests/integration/accuse-de-reception-patient.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/accuse-de-reception-patient.spec.ts
```
