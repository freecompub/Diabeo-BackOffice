...
Quand il accède à la fonctionnalité « Notes consultation »
...
Quand il tente d'accéder à « Notes consultation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name notes-consultation`
...
export const notes_consultationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type NotesconsultationInput = z.infer<typeof notes_consultationSchema>
```
...
```bash
pnpm test src/lib/services/notes-consultation.test.ts
```
...
```bash
pnpm test:integration tests/integration/notes-consultation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/notes-consultation.spec.ts
```
