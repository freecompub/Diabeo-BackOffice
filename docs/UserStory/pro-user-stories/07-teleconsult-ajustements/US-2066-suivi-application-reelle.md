# US-2066 — Suivi application réelle

> 📌 **7. Téléconsult & ajustements** · Priorité **V1** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Suivi application réelle »
...
Quand il tente d'accéder à « Suivi application réelle »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name suivi-application-reelle`
...
export const suivi_application_reelleSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SuiviapplicationreelleInput = z.infer<typeof suivi_application_reelleSchema>
```
...
```bash
pnpm test src/lib/services/suivi-application-reelle.test.ts
```
...
```bash
pnpm test:integration tests/integration/suivi-application-reelle.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/suivi-application-reelle.spec.ts
```
