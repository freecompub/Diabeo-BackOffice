...
> 📌 **18. Admin système** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Existe partiellement
...
|---|---|
| **ID** | `US-2151` |
| **Référence inventaire** | `FN-151` |
| **Domaine** | 18. Admin système |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (deploy.sh) + OVH |
| **Modèle économique** | Variable |
| **Coût estimé** | OVH backup ~5-20€/mois |
| **Statut** | 🆕 À démarrer |
...
Quand il accède à la fonctionnalité « Gestion backups »
...
Quand il tente d'accéder à « Gestion backups »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name gestion-backups`
...
export const gestion_backupsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type GestionbackupsInput = z.infer<typeof gestion_backupsSchema>
```
...
```bash
pnpm test src/lib/services/gestion-backups.test.ts
```
...
```bash
pnpm test:integration tests/integration/gestion-backups.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/gestion-backups.spec.ts
```
