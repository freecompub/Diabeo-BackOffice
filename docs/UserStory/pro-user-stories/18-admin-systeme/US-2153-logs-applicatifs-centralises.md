# US-2153 — Logs applicatifs centralisés

> 📌 **18. Admin système** · Priorité **V1** · Pays **Universel**
> 
> 💬 **Note inventaire** : Loki self-hosted = gratuit
...
|---|---|
| **ID** | `US-2153` |
| **Référence inventaire** | `FN-153` |
| **Domaine** | 18. Admin système |
| **Priorité** | **V1** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Loki + Grafana / Datadog / OVH Logs |
| **Modèle économique** | Gratuit / Payant |
| **Coût estimé** | 0–100€/mois |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Loki + Grafana / Datadog / OVH Logs
...
Quand il accède à la fonctionnalité « Logs applicatifs centralisés »
...
Quand il tente d'accéder à « Logs applicatifs centralisés »
...
```gherkin
Étant donné Le service externe « Loki + Grafana / Datadog / OVH Logs » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Loki + Grafana / Datadog / OVH Logs » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name logs-applicatifs-centralises`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/admin` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/admin` | JWT | Selon AC-1 | Création |
| PUT     | `/api/admin/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/admin/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const logs_applicatifs_centralisesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type LogsapplicatifscentralisesInput = z.infer<typeof logs_applicatifs_centralisesSchema>
```
...
```bash
pnpm test src/lib/services/logs-applicatifs-centralises.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/admin` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/admin` renvoie 401 sans JWT valide
- [ ] `/api/admin` renvoie 403 avec un rôle insuffisant
- [ ] `/api/admin` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Loki + Grafana / Datadog / OVH Logs » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/logs-applicatifs-centralises.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/logs-applicatifs-centralises.spec.ts
```
...
- Documentation officielle de **Loki + Grafana / Datadog / OVH Logs** (à vérifier au démarrage)
