...
Quand il accède à la fonctionnalité « Paramètres cabinet »
...
Quand il tente d'accéder à « Paramètres cabinet »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name parametres-cabinet`
...
export const parametres_cabinetSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ParametrescabinetInput = z.infer<typeof parametres_cabinetSchema>
```
...
```bash
pnpm test src/lib/services/parametres-cabinet.test.ts
```
...
```bash
pnpm test:integration tests/integration/parametres-cabinet.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/parametres-cabinet.spec.ts
```
