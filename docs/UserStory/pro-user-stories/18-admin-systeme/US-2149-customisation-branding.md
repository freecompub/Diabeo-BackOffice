# US-2149 — Customisation branding
...
Quand il accède à la fonctionnalité « Customisation branding »
...
Quand il tente d'accéder à « Customisation branding »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name customisation-branding`
...
export const customisation_brandingSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CustomisationbrandingInput = z.infer<typeof customisation_brandingSchema>
```
...
```bash
pnpm test src/lib/services/customisation-branding.test.ts
```
...
```bash
pnpm test:integration tests/integration/customisation-branding.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/customisation-branding.spec.ts
```
