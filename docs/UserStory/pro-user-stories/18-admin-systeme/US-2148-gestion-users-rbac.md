# US-2148 — Gestion users RBAC

> 📌 **18. Admin système** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Routes existent
...
Quand il accède à la fonctionnalité « Gestion users RBAC »
...
Quand il tente d'accéder à « Gestion users RBAC »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name gestion-users-rbac`
...
export const gestion_users_rbacSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type GestionusersrbacInput = z.infer<typeof gestion_users_rbacSchema>
```
...
```bash
pnpm test src/lib/services/gestion-users-rbac.test.ts
```
...
```bash
pnpm test:integration tests/integration/gestion-users-rbac.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/gestion-users-rbac.spec.ts
```
