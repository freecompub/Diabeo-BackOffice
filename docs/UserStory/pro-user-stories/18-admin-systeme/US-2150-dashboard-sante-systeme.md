# US-2150 — Dashboard santé système
...
Quand il accède à la fonctionnalité « Dashboard santé système »
...
Quand il tente d'accéder à « Dashboard santé système »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name dashboard-sante-systeme`
...
export const dashboard_sante_systemeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DashboardsantesystemeInput = z.infer<typeof dashboard_sante_systemeSchema>
```
...
```bash
pnpm test src/lib/services/dashboard-sante-systeme.test.ts
```
...
```bash
pnpm test:integration tests/integration/dashboard-sante-systeme.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/dashboard-sante-systeme.spec.ts
```
