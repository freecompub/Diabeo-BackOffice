# US-2152 — Statut services public

> 📌 **18. Admin système** · Priorité **V2** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2152` |
| **Référence inventaire** | `FN-152` |
| **Domaine** | 18. Admin système |
| **Priorité** | **V2** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Cachet (OSS) / StatusPage / Instatus |
...
### 🌐 Intégration externe — Cachet (OSS) / StatusPage / Instatus
...
Quand il accède à la fonctionnalité « Statut services public »
...
Quand il tente d'accéder à « Statut services public »
...
```gherkin
Étant donné Le service externe « Cachet (OSS) / StatusPage / Instatus » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Cachet (OSS) / StatusPage / Instatus » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name statut-services-public`
...
export const statut_services_publicSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type StatutservicespublicInput = z.infer<typeof statut_services_publicSchema>
```
...
```bash
pnpm test src/lib/services/statut-services-public.test.ts
```
...
- [ ] Un appel à « Cachet (OSS) / StatusPage / Instatus » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/statut-services-public.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/statut-services-public.spec.ts
```
...
- Documentation officielle de **Cachet (OSS) / StatusPage / Instatus** (à vérifier au démarrage)
