# US-2050 — Templates ajustement

> 📌 **4. Insulinothérapie** · Priorité **V1** · Pays **Universel**
> 
...
Modèles réutilisables par profil
...
Quand il accède à la fonctionnalité « Templates ajustement »
...
Quand il tente d'accéder à « Templates ajustement »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name templates-ajustement`
...
export const templates_ajustementSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TemplatesajustementInput = z.infer<typeof templates_ajustementSchema>
```
...
```bash
pnpm test src/lib/services/templates-ajustement.test.ts
```
...
```bash
pnpm test:integration tests/integration/templates-ajustement.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/templates-ajustement.spec.ts
```
