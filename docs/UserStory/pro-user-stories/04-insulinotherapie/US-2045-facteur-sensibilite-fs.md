# US-2045 — Facteur sensibilité (FS)
...
Quand il accède à la fonctionnalité « Facteur sensibilité (FS) »
...
Quand il tente d'accéder à « Facteur sensibilité (FS) »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name facteur-sensibilite-fs`
...
export const facteur_sensibilite_fsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type FacteursensibilitefsInput = z.infer<typeof facteur_sensibilite_fsSchema>
```
...
```bash
pnpm test src/lib/services/facteur-sensibilite-fs.test.ts
```
...
```bash
pnpm test:integration tests/integration/facteur-sensibilite-fs.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/facteur-sensibilite-fs.spec.ts
```
