# US-2048 — Bornes sécurité validation
...
Quand il accède à la fonctionnalité « Bornes sécurité validation »
...
Quand il tente d'accéder à « Bornes sécurité validation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name bornes-securite-validation`
...
export const bornes_securite_validationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type BornessecuritevalidationInput = z.infer<typeof bornes_securite_validationSchema>
```
...
```bash
pnpm test src/lib/services/bornes-securite-validation.test.ts
```
...
```bash
pnpm test:integration tests/integration/bornes-securite-validation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/bornes-securite-validation.spec.ts
```
