# US-2052 — Comparaison MDI vs pompe

> 📌 **4. Insulinothérapie** · Priorité **V2** · Pays **Universel**
> 
...
Aide décision changement modalité
...
Quand il accède à la fonctionnalité « Comparaison MDI vs pompe »
...
Quand il tente d'accéder à « Comparaison MDI vs pompe »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name comparaison-mdi-vs-pompe`
...
export const comparaison_mdi_vs_pompeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ComparaisonmdivspompeInput = z.infer<typeof comparaison_mdi_vs_pompeSchema>
```
...
```bash
pnpm test src/lib/services/comparaison-mdi-vs-pompe.test.ts
```
...
```bash
pnpm test:integration tests/integration/comparaison-mdi-vs-pompe.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/comparaison-mdi-vs-pompe.spec.ts
```
