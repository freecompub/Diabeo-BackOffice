# US-2046 — Profils basaux pompe

> 📌 **4. Insulinothérapie** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Existant
...
Quand il accède à la fonctionnalité « Profils basaux pompe »
...
Quand il tente d'accéder à « Profils basaux pompe »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name profils-basaux-pompe`
...
export const profils_basaux_pompeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ProfilsbasauxpompeInput = z.infer<typeof profils_basaux_pompeSchema>
```
...
```bash
pnpm test src/lib/services/profils-basaux-pompe.test.ts
```
...
```bash
pnpm test:integration tests/integration/profils-basaux-pompe.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/profils-basaux-pompe.spec.ts
```
