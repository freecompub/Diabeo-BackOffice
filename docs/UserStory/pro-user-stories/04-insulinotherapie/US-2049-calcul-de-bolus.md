# US-2049 — Calcul de bolus

> 📌 **4. Insulinothérapie** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Logique implémentée
...
Quand il accède à la fonctionnalité « Calcul de bolus »
...
Quand il tente d'accéder à « Calcul de bolus »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name calcul-de-bolus`
...
export const calcul_de_bolusSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CalculdebolusInput = z.infer<typeof calcul_de_bolusSchema>
```
...
```bash
pnpm test src/lib/services/calcul-de-bolus.test.ts
```
...
```bash
pnpm test:integration tests/integration/calcul-de-bolus.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/calcul-de-bolus.spec.ts
```
