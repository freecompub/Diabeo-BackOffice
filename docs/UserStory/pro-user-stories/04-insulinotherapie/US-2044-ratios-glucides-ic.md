...
Quand il accède à la fonctionnalité « Ratios glucides (IC) »
...
Quand il tente d'accéder à « Ratios glucides (IC) »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name ratios-glucides-ic`
...
export const ratios_glucides_icSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type RatiosglucidesicInput = z.infer<typeof ratios_glucides_icSchema>
```
...
```bash
pnpm test src/lib/services/ratios-glucides-ic.test.ts
```
...
```bash
pnpm test:integration tests/integration/ratios-glucides-ic.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/ratios-glucides-ic.spec.ts
```
