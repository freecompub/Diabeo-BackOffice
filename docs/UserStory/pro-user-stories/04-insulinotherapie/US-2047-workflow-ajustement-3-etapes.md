# US-2047 — Workflow ajustement 3 étapes

> 📌 **4. Insulinothérapie** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Mockup dispo
...
Quand il accède à la fonctionnalité « Workflow ajustement 3 étapes »
...
Quand il tente d'accéder à « Workflow ajustement 3 étapes »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name workflow-ajustement-3-etapes`
...
export const workflow_ajustement_3_etapesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Workflowajustement3EtapesInput = z.infer<typeof workflow_ajustement_3_etapesSchema>
```
...
```bash
pnpm test src/lib/services/workflow-ajustement-3-etapes.test.ts
```
...
```bash
pnpm test:integration tests/integration/workflow-ajustement-3-etapes.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/workflow-ajustement-3-etapes.spec.ts
```
