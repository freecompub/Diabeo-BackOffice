# US-2043 — Données pompe à insuline

> 📌 **4. Insulinothérapie** · Priorité **V1** · Pays **FR+DZ**
> 
...
|---|---|
| **ID** | `US-2043` |
| **Référence inventaire** | `FN-043` |
| **Domaine** | 4. Insulinothérapie |
| **Priorité** | **V1** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Oui |
| **Service / Standard** | Medtronic / Tandem / Insulet API |
...
### 🌐 Intégration externe — Medtronic / Tandem / Insulet API
...
Quand il accède à la fonctionnalité « Données pompe à insuline »
...
Quand il tente d'accéder à « Données pompe à insuline »
...
```gherkin
Étant donné Le service externe « Medtronic / Tandem / Insulet API » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Medtronic / Tandem / Insulet API » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name donnees-pompe-a-insuline`
...
export const donnees_pompe_a_insulineSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DonneespompeainsulineInput = z.infer<typeof donnees_pompe_a_insulineSchema>
```
...
```bash
pnpm test src/lib/services/donnees-pompe-a-insuline.test.ts
```
...
- [ ] Un appel à « Medtronic / Tandem / Insulet API » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/donnees-pompe-a-insuline.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/donnees-pompe-a-insuline.spec.ts
```
...
- Documentation officielle de **Medtronic / Tandem / Insulet API** (à vérifier au démarrage)
