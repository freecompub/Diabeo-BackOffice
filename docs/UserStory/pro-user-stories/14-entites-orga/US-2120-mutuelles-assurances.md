# US-2120 — Mutuelles / assurances

> 📌 **14. Entités orga** · Priorité **V2** · Pays **FR+DZ**
> 
...
|---|---|
| **ID** | `US-2120` |
| **Référence inventaire** | `FN-120` |
| **Domaine** | 14. Entités orga |
| **Priorité** | **V2** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Non |
| **Service / Standard** | Interne + interop |
...
Quand il accède à la fonctionnalité « Mutuelles / assurances »
...
Quand il tente d'accéder à « Mutuelles / assurances »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name mutuelles-assurances`
...
export const mutuelles_assurancesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type MutuellesassurancesInput = z.infer<typeof mutuelles_assurancesSchema>
```
...
```bash
pnpm test src/lib/services/mutuelles-assurances.test.ts
```
...
```bash
pnpm test:integration tests/integration/mutuelles-assurances.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/mutuelles-assurances.spec.ts
```
