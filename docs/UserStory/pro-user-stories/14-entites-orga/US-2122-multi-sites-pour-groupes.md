# US-2122 — Multi-sites pour groupes

> 📌 **14. Entités orga** · Priorité **V2** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Multi-sites pour groupes »
...
Quand il tente d'accéder à « Multi-sites pour groupes »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name multi-sites-pour-groupes`
...
export const multi_sites_pour_groupesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type MultisitespourgroupesInput = z.infer<typeof multi_sites_pour_groupesSchema>
```
...
```bash
pnpm test src/lib/services/multi-sites-pour-groupes.test.ts
```
...
```bash
pnpm test:integration tests/integration/multi-sites-pour-groupes.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/multi-sites-pour-groupes.spec.ts
```
