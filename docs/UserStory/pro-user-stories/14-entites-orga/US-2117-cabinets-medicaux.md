...
Quand il accède à la fonctionnalité « Cabinets médicaux »
...
Quand il tente d'accéder à « Cabinets médicaux »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name cabinets-medicaux`
...
export const cabinets_medicauxSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CabinetsmedicauxInput = z.infer<typeof cabinets_medicauxSchema>
```
...
```bash
pnpm test src/lib/services/cabinets-medicaux.test.ts
```
...
```bash
pnpm test:integration tests/integration/cabinets-medicaux.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/cabinets-medicaux.spec.ts
```
