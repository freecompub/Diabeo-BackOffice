...
Quand il accède à la fonctionnalité « Hôpitaux / unités hospi »
...
Quand il tente d'accéder à « Hôpitaux / unités hospi »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name hopitaux-unites-hospi`
...
export const hopitaux_unites_hospiSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type HopitauxuniteshospiInput = z.infer<typeof hopitaux_unites_hospiSchema>
```
...
```bash
pnpm test src/lib/services/hopitaux-unites-hospi.test.ts
```
...
```bash
pnpm test:integration tests/integration/hopitaux-unites-hospi.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/hopitaux-unites-hospi.spec.ts
```
