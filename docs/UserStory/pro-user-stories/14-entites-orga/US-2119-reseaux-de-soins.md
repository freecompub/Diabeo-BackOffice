# US-2119 — Réseaux de soins

> 📌 **14. Entités orga** · Priorité **V1** · Pays **FR**
> 
...
Quand il accède à la fonctionnalité « Réseaux de soins »
...
Quand il tente d'accéder à « Réseaux de soins »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name reseaux-de-soins`
...
export const reseaux_de_soinsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ReseauxdesoinsInput = z.infer<typeof reseaux_de_soinsSchema>
```
...
```bash
pnpm test src/lib/services/reseaux-de-soins.test.ts
```
...
```bash
pnpm test:integration tests/integration/reseaux-de-soins.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/reseaux-de-soins.spec.ts
```
