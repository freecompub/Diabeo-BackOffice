...
Quand il accède à la fonctionnalité « Export CSV / Excel »
...
Quand il tente d'accéder à « Export CSV / Excel »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name export-csv-excel`
...
export const export_csv_excelSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ExportcsvexcelInput = z.infer<typeof export_csv_excelSchema>
```
...
```bash
pnpm test src/lib/services/export-csv-excel.test.ts
```
...
```bash
pnpm test:integration tests/integration/export-csv-excel.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/export-csv-excel.spec.ts
```
