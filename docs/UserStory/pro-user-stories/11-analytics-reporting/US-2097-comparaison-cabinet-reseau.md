# US-2097 — Comparaison cabinet/réseau
...
Quand il accède à la fonctionnalité « Comparaison cabinet/réseau »
...
Quand il tente d'accéder à « Comparaison cabinet/réseau »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name comparaison-cabinet-reseau`
...
export const comparaison_cabinet_reseauSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ComparaisoncabinetreseauInput = z.infer<typeof comparaison_cabinet_reseauSchema>
```
...
```bash
pnpm test src/lib/services/comparaison-cabinet-reseau.test.ts
```
...
```bash
pnpm test:integration tests/integration/comparaison-cabinet-reseau.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/comparaison-cabinet-reseau.spec.ts
```
