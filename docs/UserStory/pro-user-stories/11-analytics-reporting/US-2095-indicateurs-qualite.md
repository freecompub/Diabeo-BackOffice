...
Quand il accède à la fonctionnalité « Indicateurs qualité »
...
Quand il tente d'accéder à « Indicateurs qualité »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name indicateurs-qualite`
...
export const indicateurs_qualiteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type IndicateursqualiteInput = z.infer<typeof indicateurs_qualiteSchema>
```
...
```bash
pnpm test src/lib/services/indicateurs-qualite.test.ts
```
...
```bash
pnpm test:integration tests/integration/indicateurs-qualite.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/indicateurs-qualite.spec.ts
```
