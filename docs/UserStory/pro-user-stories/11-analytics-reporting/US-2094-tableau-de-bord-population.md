...
> 📌 **11. Analytics & reporting** · Priorité **V1** · Pays **Universel**
> 
> 💬 **Note inventaire** : DashboardConfiguration existe
...
Quand il accède à la fonctionnalité « Tableau de bord population »
...
Quand il tente d'accéder à « Tableau de bord population »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name tableau-de-bord-population`
...
export const tableau_de_bord_populationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TableaudebordpopulationInput = z.infer<typeof tableau_de_bord_populationSchema>
```
...
```bash
pnpm test src/lib/services/tableau-de-bord-population.test.ts
```
...
```bash
pnpm test:integration tests/integration/tableau-de-bord-population.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/tableau-de-bord-population.spec.ts
```
