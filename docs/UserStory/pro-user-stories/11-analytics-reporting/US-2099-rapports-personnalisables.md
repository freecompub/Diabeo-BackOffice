# US-2099 — Rapports personnalisables

> 📌 **11. Analytics & reporting** · Priorité **V2** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2099` |
| **Référence inventaire** | `FN-099` |
| **Domaine** | 11. Analytics & reporting |
| **Priorité** | **V2** |
| **Pays cible** | Universel |
| **Intégration externe** | Partiel |
| **Service / Standard** | Interne ou Metabase intégré |
| **Modèle économique** | Gratuit (Metabase OSS) |
| **Coût estimé** | — |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Interne ou Metabase intégré
...
Quand il accède à la fonctionnalité « Rapports personnalisables »
...
Quand il tente d'accéder à « Rapports personnalisables »
...
```gherkin
Étant donné Le service externe « Interne ou Metabase intégré » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Interne ou Metabase intégré » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name rapports-personnalisables`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/analytics` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/analytics` | JWT | Selon AC-1 | Création |
| PUT     | `/api/analytics/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/analytics/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const rapports_personnalisablesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type RapportspersonnalisablesInput = z.infer<typeof rapports_personnalisablesSchema>
```
...
```bash
pnpm test src/lib/services/rapports-personnalisables.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/analytics` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/analytics` renvoie 401 sans JWT valide
- [ ] `/api/analytics` renvoie 403 avec un rôle insuffisant
- [ ] `/api/analytics` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Interne ou Metabase intégré » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/rapports-personnalisables.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/rapports-personnalisables.spec.ts
```
...
- Documentation officielle de **Interne ou Metabase intégré** (à vérifier au démarrage)
