# US-2100 — Indicateurs charge soignant
...
Quand il accède à la fonctionnalité « Indicateurs charge soignant »
...
Quand il tente d'accéder à « Indicateurs charge soignant »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name indicateurs-charge-soignant`
...
export const indicateurs_charge_soignantSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type IndicateurschargesoignantInput = z.infer<typeof indicateurs_charge_soignantSchema>
```
...
```bash
pnpm test src/lib/services/indicateurs-charge-soignant.test.ts
```
...
```bash
pnpm test:integration tests/integration/indicateurs-charge-soignant.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/indicateurs-charge-soignant.spec.ts
```
