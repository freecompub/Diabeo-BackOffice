...
Quand il accède à la fonctionnalité « Cohorte par pathologie »
...
Quand il tente d'accéder à « Cohorte par pathologie »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name cohorte-par-pathologie`
...
export const cohorte_par_pathologieSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CohorteparpathologieInput = z.infer<typeof cohorte_par_pathologieSchema>
```
...
```bash
pnpm test src/lib/services/cohorte-par-pathologie.test.ts
```
...
```bash
pnpm test:integration tests/integration/cohorte-par-pathologie.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/cohorte-par-pathologie.spec.ts
```
