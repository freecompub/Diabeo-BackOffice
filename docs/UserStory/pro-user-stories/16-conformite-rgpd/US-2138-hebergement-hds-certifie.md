# US-2138 — Hébergement HDS certifié

> 📌 **16. Conformité & RGPD** · Priorité **MVP** · Pays **FR**
> 
> 💬 **Note inventaire** : Coût infra HDS
...
|---|---|
| **ID** | `US-2138` |
| **Référence inventaire** | `FN-138` |
| **Domaine** | 16. Conformité & RGPD |
| **Priorité** | **MVP** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | OVHcloud HDS |
| **Modèle économique** | Payant infrastructure |
| **Coût estimé** | ~200-1000€/mois |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — OVHcloud HDS
...
Quand il accède à la fonctionnalité « Hébergement HDS certifié »
...
Quand il tente d'accéder à « Hébergement HDS certifié »
...
```gherkin
Étant donné Le service externe « OVHcloud HDS » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « OVHcloud HDS » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name hebergement-hds-certifie`
...
export const hebergement_hds_certifieSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type HebergementhdscertifieInput = z.infer<typeof hebergement_hds_certifieSchema>
```
...
```bash
pnpm test src/lib/services/hebergement-hds-certifie.test.ts
```
...
- [ ] Un appel à « OVHcloud HDS » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/hebergement-hds-certifie.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/hebergement-hds-certifie.spec.ts
```
...
- Documentation officielle de **OVHcloud HDS** (à vérifier au démarrage)
