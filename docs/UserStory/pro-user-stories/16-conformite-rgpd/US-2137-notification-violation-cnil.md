# US-2137 — Notification violation CNIL

> 📌 **16. Conformité & RGPD** · Priorité **V1** · Pays **FR**
> 
...
|---|---|
| **ID** | `US-2137` |
| **Référence inventaire** | `FN-137` |
| **Domaine** | 16. Conformité & RGPD |
| **Priorité** | **V1** |
| **Pays cible** | FR |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (workflow) |
...
Quand il accède à la fonctionnalité « Notification violation CNIL »
...
Quand il tente d'accéder à « Notification violation CNIL »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name notification-violation-cnil`
...
export const notification_violation_cnilSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type NotificationviolationcnilInput = z.infer<typeof notification_violation_cnilSchema>
```
...
```bash
pnpm test src/lib/services/notification-violation-cnil.test.ts
```
...
```bash
pnpm test:integration tests/integration/notification-violation-cnil.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/notification-violation-cnil.spec.ts
```
