...
Quand il accède à la fonctionnalité « Pseudonymisation »
...
Quand il tente d'accéder à « Pseudonymisation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name pseudonymisation`
...
export const pseudonymisationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PseudonymisationInput = z.infer<typeof pseudonymisationSchema>
```
...
```bash
pnpm test src/lib/services/pseudonymisation.test.ts
```
...
```bash
pnpm test:integration tests/integration/pseudonymisation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/pseudonymisation.spec.ts
```
