# US-2135 — Effacement RGPD Article 17

> 📌 **16. Conformité & RGPD** · Priorité **MVP** · Pays **FR**
> 
> 💬 **Note inventaire** : Implémenté
...
|---|---|
| **ID** | `US-2135` |
| **Référence inventaire** | `FN-135` |
| **Domaine** | 16. Conformité & RGPD |
| **Priorité** | **MVP** |
| **Pays cible** | FR |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (deletion.service) |
...
Quand il accède à la fonctionnalité « Effacement RGPD Article 17 »
...
Quand il tente d'accéder à « Effacement RGPD Article 17 »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name effacement-rgpd-article-17`
...
export const effacement_rgpd_article_17Schema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Effacementrgpdarticle17Input = z.infer<typeof effacement_rgpd_article_17Schema>
```
...
- [ ] Service métier lève l'exception attendue en cas de violation de règle (RM-1 à RM-4)

```bash
pnpm test src/lib/services/effacement-rgpd-article-17.test.ts
```
...
```bash
pnpm test:integration tests/integration/effacement-rgpd-article-17.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/effacement-rgpd-article-17.spec.ts
```
