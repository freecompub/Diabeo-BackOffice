...
Quand il accède à la fonctionnalité « Audit log immuable »
...
Quand il tente d'accéder à « Audit log immuable »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name audit-log-immuable`
...
export const audit_log_immuableSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AuditlogimmuableInput = z.infer<typeof audit_log_immuableSchema>
```
...
```bash
pnpm test src/lib/services/audit-log-immuable.test.ts
```
...
```bash
pnpm test:integration tests/integration/audit-log-immuable.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/audit-log-immuable.spec.ts
```
