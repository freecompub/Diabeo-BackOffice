# US-2134 — Export RGPD Article 15
...
|---|---|
| **ID** | `US-2134` |
| **Référence inventaire** | `FN-134` |
| **Domaine** | 16. Conformité & RGPD |
| **Priorité** | **MVP** |
| **Pays cible** | FR |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (export.service) |
...
Quand il accède à la fonctionnalité « Export RGPD Article 15 »
...
Quand il tente d'accéder à « Export RGPD Article 15 »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name export-rgpd-article-15`
...
export const export_rgpd_article_15Schema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Exportrgpdarticle15Input = z.infer<typeof export_rgpd_article_15Schema>
```
...
```bash
pnpm test src/lib/services/export-rgpd-article-15.test.ts
```
...
```bash
pnpm test:integration tests/integration/export-rgpd-article-15.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/export-rgpd-article-15.spec.ts
```
