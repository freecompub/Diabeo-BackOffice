...
|---|---|
| **ID** | `US-2133` |
| **Référence inventaire** | `FN-133` |
| **Domaine** | 16. Conformité & RGPD |
| **Priorité** | **MVP** |
| **Pays cible** | FR |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (cron purge) |
...
Quand il accède à la fonctionnalité « Rétention 6 ans logs »
...
Quand il tente d'accéder à « Rétention 6 ans logs »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name retention-6-ans-logs`
...
export const retention_6_ans_logsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Retention6AnslogsInput = z.infer<typeof retention_6_ans_logsSchema>
```
...
```bash
pnpm test src/lib/services/retention-6-ans-logs.test.ts
```
...
```bash
pnpm test:integration tests/integration/retention-6-ans-logs.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/retention-6-ans-logs.spec.ts
```
