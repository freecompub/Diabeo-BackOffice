...
Quand il accède à la fonctionnalité « Relances automatiques »
...
Quand il tente d'accéder à « Relances automatiques »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name relances-automatiques`
...
export const relances_automatiquesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type RelancesautomatiquesInput = z.infer<typeof relances_automatiquesSchema>
```
...
```bash
pnpm test src/lib/services/relances-automatiques.test.ts
```
...
```bash
pnpm test:integration tests/integration/relances-automatiques.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/relances-automatiques.spec.ts
```
