# US-2105 — Numérotation séquentielle pays

> 📌 **12. Facturation** · Priorité **V1** · Pays **FR+DZ**
> 
> 💬 **Note inventaire** : US-900 documenté
...
|---|---|
| **ID** | `US-2105` |
| **Référence inventaire** | `FN-105` |
| **Domaine** | 12. Facturation |
| **Priorité** | **V1** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (transaction PostgreSQL) |
...
Quand il accède à la fonctionnalité « Numérotation séquentielle pays »
...
Quand il tente d'accéder à « Numérotation séquentielle pays »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name numerotation-sequentielle-pays`
...
export const numerotation_sequentielle_paysSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type NumerotationsequentiellepaysInput = z.infer<typeof numerotation_sequentielle_paysSchema>
```
...
```bash
pnpm test src/lib/services/numerotation-sequentielle-pays.test.ts
```
...
```bash
pnpm test:integration tests/integration/numerotation-sequentielle-pays.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/numerotation-sequentielle-pays.spec.ts
```
