...
> 📌 **12. Facturation** · Priorité **V1** · Pays **DZ**
> 
> 💬 **Note inventaire** : Modèle DZ documenté
...
Quand il accède à la fonctionnalité « Abonnement (DZ) »
...
Quand il tente d'accéder à « Abonnement (DZ) »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name abonnement-dz`
...
export const abonnement_dzSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type AbonnementdzInput = z.infer<typeof abonnement_dzSchema>
```
...
```bash
pnpm test src/lib/services/abonnement-dz.test.ts
```
...
```bash
pnpm test:integration tests/integration/abonnement-dz.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/abonnement-dz.spec.ts
```
