# US-2102 — Virement bancaire + facture PDF

> 📌 **12. Facturation** · Priorité **V1** · Pays **FR+DZ**
> 
...
|---|---|
| **ID** | `US-2102` |
| **Référence inventaire** | `FN-102` |
| **Domaine** | 12. Facturation |
| **Priorité** | **V1** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Partiel |
| **Service / Standard** | OVH / banque + génération PDF |
| **Modèle économique** | Variable |
| **Coût estimé** | Selon banque |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — OVH / banque + génération PDF
...
| **Modèle économique** | Variable |
| **Coût estimé** | Selon banque |
| **Type d'intégration** | Partiel (API REST / SDK / webhook selon fournisseur) |
...
Quand il accède à la fonctionnalité « Virement bancaire + facture PDF »
...
Quand il tente d'accéder à « Virement bancaire + facture PDF »
Alors la requête est rejetée avec HTTP 401/403, et la tentative est journalisée
```

### AC-3 — Numérotation séquentielle anti-fraude

```gherkin
Étant donné Une facture est émise en France
Quand le compteur génère le numéro suivant
Alors le numéro est unique par pays et sans trou (transaction PostgreSQL)
```

### AC-4 — Webhook idempotent

```gherkin
Étant donné Stripe envoie le même événement deux fois
Quand le système reçoit les deux notifications
Alors une seule action métier est exécutée (table StripeEvent en lookup)
```

### AC-5 — Tolérance aux pannes externes

```gherkin
Étant donné Le service externe « OVH / banque + génération PDF » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « OVH / banque + génération PDF » n'est exposé côté client. Tous les appels passent par le backend.**
...
model Invoice {
  ...
  number          String   @unique // Séquentiel par pays : FR-2026-000001
  country         Country  // FR | DZ
  cabinetId       String
  amountCents     Int
  currency        Currency // EUR | DZD
  taxCents        Int
  status          InvoiceStatus  // DRAFT | ISSUED | PAID | CANCELLED | REFUNDED
  paymentMethod   PaymentMethod? // STRIPE | BANK_TRANSFER
  stripePaymentIntentId String?
  pdfUrl          String?
  pdfHash         String?  // SHA-256 pour intégrité
  issuedAt        DateTime?
  paidAt          DateTime?
  createdAt       DateTime @default(now())
  updatedAt       DateTime @updatedAt

  @@index([cabinetId, country, status])
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name virement-bancaire-facture-pdf`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/billing` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/billing` | JWT | Selon AC-1 | Création |
| PUT     | `/api/billing/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/billing/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const virement_bancaire_facture_pdfSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type VirementbancairefacturepdfInput = z.infer<typeof virement_bancaire_facture_pdfSchema>
```
...
```bash
pnpm test src/lib/services/virement-bancaire-facture-pdf.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/billing` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/billing` renvoie 401 sans JWT valide
- [ ] `/api/billing` renvoie 403 avec un rôle insuffisant
- [ ] `/api/billing` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « OVH / banque + génération PDF » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/virement-bancaire-facture-pdf.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/virement-bancaire-facture-pdf.spec.ts
```
...
- Documentation officielle de **OVH / banque + génération PDF** (à vérifier au démarrage)
