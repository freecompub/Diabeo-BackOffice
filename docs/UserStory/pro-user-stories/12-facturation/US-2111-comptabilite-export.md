# US-2111 — Comptabilité export

> 📌 **12. Facturation** · Priorité **V2** · Pays **FR**
> 
> 💬 **Note inventaire** : Obligation fiscale FR
...
|---|---|
| **ID** | `US-2111` |
| **Référence inventaire** | `FN-111` |
| **Domaine** | 12. Facturation |
| **Priorité** | **V2** |
| **Pays cible** | FR |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (format CERFA) |
...
Quand il accède à la fonctionnalité « Comptabilité export »
...
Quand il tente d'accéder à « Comptabilité export »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name comptabilite-export`
...
export const comptabilite_exportSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ComptabiliteexportInput = z.infer<typeof comptabilite_exportSchema>
```
...
```bash
pnpm test src/lib/services/comptabilite-export.test.ts
```
...
```bash
pnpm test:integration tests/integration/comptabilite-export.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/comptabilite-export.spec.ts
```
