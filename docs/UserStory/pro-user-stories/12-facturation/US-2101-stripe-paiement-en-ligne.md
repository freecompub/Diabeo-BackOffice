# US-2101 — Stripe paiement en ligne

> 📌 **12. Facturation** · Priorité **V4** · Pays **FR+DZ**
> 
> 💬 **Note inventaire** : DZ = via partenaire bancaire
...
|---|---|
| **ID** | `US-2101` |
| **Référence inventaire** | `FN-101` |
| **Domaine** | 12. Facturation |
| **Priorité** | **V4** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Oui |
| **Service / Standard** | Stripe API |
| **Modèle économique** | Payant |
| **Coût estimé** | 1.4-2.9% + 0.25€/transac |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Stripe API
...
Quand il accède à la fonctionnalité « Stripe paiement en ligne »
...
Quand il tente d'accéder à « Stripe paiement en ligne »
...
```gherkin
Étant donné Le service externe « Stripe API » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Stripe API » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name stripe-paiement-en-ligne`
...
export const stripe_paiement_en_ligneSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type StripepaiementenligneInput = z.infer<typeof stripe_paiement_en_ligneSchema>
```
...
```bash
pnpm test src/lib/services/stripe-paiement-en-ligne.test.ts
```
...
- [ ] Un appel à « Stripe API » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/stripe-paiement-en-ligne.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/stripe-paiement-en-ligne.spec.ts
```
...
- Documentation officielle de **Stripe API** (à vérifier au démarrage)
