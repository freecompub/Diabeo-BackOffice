# US-2106 — Webhooks idempotents Stripe
...
Quand il accède à la fonctionnalité « Webhooks idempotents Stripe »
...
Quand il tente d'accéder à « Webhooks idempotents Stripe »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name webhooks-idempotents-stripe`
...
export const webhooks_idempotents_stripeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type WebhooksidempotentsstripeInput = z.infer<typeof webhooks_idempotents_stripeSchema>
```
...
```bash
pnpm test src/lib/services/webhooks-idempotents-stripe.test.ts
```
...
```bash
pnpm test:integration tests/integration/webhooks-idempotents-stripe.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/webhooks-idempotents-stripe.spec.ts
```
