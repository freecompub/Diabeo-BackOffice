...
> 📌 **12. Facturation** · Priorité **V1** · Pays **FR**
> 
> 💬 **Note inventaire** : Modèle FR documenté
...
Quand il accède à la fonctionnalité « Facturation au patient (FR) »
...
Quand il tente d'accéder à « Facturation au patient (FR) »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name facturation-au-patient-fr`
...
export const facturation_au_patient_frSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type FacturationaupatientfrInput = z.infer<typeof facturation_au_patient_frSchema>
```
...
```bash
pnpm test src/lib/services/facturation-au-patient-fr.test.ts
```
...
```bash
pnpm test:integration tests/integration/facturation-au-patient-fr.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/facturation-au-patient-fr.spec.ts
```
