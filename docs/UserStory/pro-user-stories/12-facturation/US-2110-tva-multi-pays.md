...
|---|---|
| **ID** | `US-2110` |
| **Référence inventaire** | `FN-110` |
| **Domaine** | 12. Facturation |
| **Priorité** | **V1** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Partiel |
| **Service / Standard** | Interne ou Stripe Tax |
| **Modèle économique** | Selon |
| **Coût estimé** | Stripe Tax = 0.5%/transac |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Interne ou Stripe Tax
...
Quand il accède à la fonctionnalité « TVA multi-pays »
...
Quand il tente d'accéder à « TVA multi-pays »
...
```gherkin
Étant donné Le service externe « Interne ou Stripe Tax » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Interne ou Stripe Tax » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name tva-multi-pays`
...
export const tva_multi_paysSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TvamultipaysInput = z.infer<typeof tva_multi_paysSchema>
```
...
```bash
pnpm test src/lib/services/tva-multi-pays.test.ts
```
...
- [ ] Un appel à « Interne ou Stripe Tax » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/tva-multi-pays.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/tva-multi-pays.spec.ts
```
...
- Documentation officielle de **Interne ou Stripe Tax** (à vérifier au démarrage)
