...
> 📌 **12. Facturation** · Priorité **V1** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2109` |
| **Référence inventaire** | `FN-109` |
| **Domaine** | 12. Facturation |
| **Priorité** | **V1** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Stripe Refund API |
| **Modèle économique** | Inclus Stripe |
| **Coût estimé** | — |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Stripe Refund API
...
Quand il accède à la fonctionnalité « Remboursements »
...
Quand il tente d'accéder à « Remboursements »
...
```gherkin
Étant donné Le service externe « Stripe Refund API » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Stripe Refund API » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name remboursements`
...
export const remboursementsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type RemboursementsInput = z.infer<typeof remboursementsSchema>
```
...
```bash
pnpm test src/lib/services/remboursements.test.ts
```
...
- [ ] Un appel à « Stripe Refund API » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/remboursements.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/remboursements.spec.ts
```
...
- Documentation officielle de **Stripe Refund API** (à vérifier au démarrage)
