# US-2140 — Upload via S3-compatible

> 📌 **17. Documents & fichiers** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : MinIO = gratuit auto-hébergé
...
|---|---|
| **ID** | `US-2140` |
| **Référence inventaire** | `FN-140` |
| **Domaine** | 17. Documents & fichiers |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | MinIO (auto-hébergé) ou OVH Object Storage |
| **Modèle économique** | Gratuit / payant |
| **Coût estimé** | 0–0.01€/Go/mois |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — MinIO (auto-hébergé) ou OVH Object Storage
...
Quand il accède à la fonctionnalité « Upload via S3-compatible »
...
Quand il tente d'accéder à « Upload via S3-compatible »
...
```gherkin
Étant donné Le service externe « MinIO (auto-hébergé) ou OVH Object Storage » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « MinIO (auto-hébergé) ou OVH Object Storage » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name upload-via-s3-compatible`
...
export const upload_via_s3_compatibleSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Uploadvias3CompatibleInput = z.infer<typeof upload_via_s3_compatibleSchema>
```
...
```bash
pnpm test src/lib/services/upload-via-s3-compatible.test.ts
```
...
- [ ] Un appel à « MinIO (auto-hébergé) ou OVH Object Storage » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/upload-via-s3-compatible.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/upload-via-s3-compatible.spec.ts
```
...
- Documentation officielle de **MinIO (auto-hébergé) ou OVH Object Storage** (à vérifier au démarrage)
