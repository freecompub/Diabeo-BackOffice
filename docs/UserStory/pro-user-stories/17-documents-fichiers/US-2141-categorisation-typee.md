...
Quand il accède à la fonctionnalité « Catégorisation typée »
...
Quand il tente d'accéder à « Catégorisation typée »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name categorisation-typee`
...
export const categorisation_typeeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CategorisationtypeeInput = z.infer<typeof categorisation_typeeSchema>
```
...
```bash
pnpm test src/lib/services/categorisation-typee.test.ts
```
...
```bash
pnpm test:integration tests/integration/categorisation-typee.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/categorisation-typee.spec.ts
```
