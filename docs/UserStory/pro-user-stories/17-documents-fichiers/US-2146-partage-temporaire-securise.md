# US-2146 — Partage temporaire sécurisé

> 📌 **17. Documents & fichiers** · Priorité **V1** · Pays **Universel**
> 
...
Quand il accède à la fonctionnalité « Partage temporaire sécurisé »
...
Quand il tente d'accéder à « Partage temporaire sécurisé »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name partage-temporaire-securise`
...
export const partage_temporaire_securiseSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type PartagetemporairesecuriseInput = z.infer<typeof partage_temporaire_securiseSchema>
```
...
```bash
pnpm test src/lib/services/partage-temporaire-securise.test.ts
```
...
```bash
pnpm test:integration tests/integration/partage-temporaire-securise.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/partage-temporaire-securise.spec.ts
```
