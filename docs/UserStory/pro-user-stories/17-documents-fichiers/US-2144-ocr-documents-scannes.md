# US-2144 — OCR documents scannés

> 📌 **17. Documents & fichiers** · Priorité **V2** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2144` |
| **Référence inventaire** | `FN-144` |
| **Domaine** | 17. Documents & fichiers |
| **Priorité** | **V2** |
| **Pays cible** | Universel |
| **Intégration externe** | Oui |
| **Service / Standard** | Tesseract (gratuit) / Google Vision / Mistral OCR |
| **Modèle économique** | Gratuit / Payant |
| **Coût estimé** | 0–0.0015€/page |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Tesseract (gratuit) / Google Vision / Mistral OCR
...
Quand il accède à la fonctionnalité « OCR documents scannés »
...
Quand il tente d'accéder à « OCR documents scannés »
...
```gherkin
Étant donné Le service externe « Tesseract (gratuit) / Google Vision / Mistral OCR » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Tesseract (gratuit) / Google Vision / Mistral OCR » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name ocr-documents-scannes`
...
export const ocr_documents_scannesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type OcrdocumentsscannesInput = z.infer<typeof ocr_documents_scannesSchema>
```
...
```bash
pnpm test src/lib/services/ocr-documents-scannes.test.ts
```
...
- [ ] Un appel à « Tesseract (gratuit) / Google Vision / Mistral OCR » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/ocr-documents-scannes.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/ocr-documents-scannes.spec.ts
```
...
- Documentation officielle de **Tesseract (gratuit) / Google Vision / Mistral OCR** (à vérifier au démarrage)
