# US-2143 — Signature électronique qualifiée eIDAS

> 📌 **17. Documents & fichiers** · Priorité **V4** · Pays **FR**
> 
> 💬 **Note inventaire** : Pour ordonnances notamment
...
|---|---|
| **ID** | `US-2143` |
| **Référence inventaire** | `FN-143` |
| **Domaine** | 17. Documents & fichiers |
| **Priorité** | **V4** |
| **Pays cible** | FR |
| **Intégration externe** | Oui |
| **Service / Standard** | Universign / YouSign / DocuSign / Lex Persona |
| **Modèle économique** | Payant |
| **Coût estimé** | 1-3€/signature ou 50-200€/mois |
| **Statut** | 🆕 À démarrer |
...
### 🌐 Intégration externe — Universign / YouSign / DocuSign / Lex Persona
...
Quand il accède à la fonctionnalité « Signature électronique qualifiée eIDAS »
...
Quand il tente d'accéder à « Signature électronique qualifiée eIDAS »
Alors la requête est rejetée avec HTTP 401/403, et la tentative est journalisée
```

### AC-3 — Tolérance aux pannes externes

```gherkin
Étant donné Le service externe « Universign / YouSign / DocuSign / Lex Persona » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Universign / YouSign / DocuSign / Lex Persona » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name signature-electronique-qualifiee-eidas`
...
|---------|----------|------|-----------------|-------------|
| GET     | `/api/documents` | JWT | Selon AC-1 | Liste / lecture |
| POST    | `/api/documents` | JWT | Selon AC-1 | Création |
| PUT     | `/api/documents/[id]` | JWT | Selon AC-1 | Modification |
| DELETE  | `/api/documents/[id]` | JWT | ADMIN/DOCTOR (selon RBAC) | Soft delete (RGPD) |
...
export const signature_electronique_qualifiee_eidasSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type SignatureelectroniquequalifieeeidasInput = z.infer<typeof signature_electronique_qualifiee_eidasSchema>
```
...
```bash
pnpm test src/lib/services/signature-electronique-qualifiee-eidas.test.ts
```

### Tests d'intégration (Vitest + Prisma + supertest)

- [ ] `/api/documents` renvoie 200 + payload conforme pour un cas nominal
- [ ] `/api/documents` renvoie 401 sans JWT valide
- [ ] `/api/documents` renvoie 403 avec un rôle insuffisant
- [ ] `/api/documents` renvoie 422 avec un payload invalide (schéma Zod)
...
- [ ] Un appel à « Universign / YouSign / DocuSign / Lex Persona » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/signature-electronique-qualifiee-eidas.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/signature-electronique-qualifiee-eidas.spec.ts
```
...
- Documentation officielle de **Universign / YouSign / DocuSign / Lex Persona** (à vérifier au démarrage)
