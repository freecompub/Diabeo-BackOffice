# US-2026 — INS — Identité Nationale Santé

> 📌 **2. Patients** · Priorité **V1** · Pays **FR**
> 
...
Récupération + vérif identitovigilance
...
Quand il accède à la fonctionnalité « INS — Identité Nationale Santé »
...
Quand il tente d'accéder à « INS — Identité Nationale Santé »
...
```gherkin
Étant donné Une opération « INS — Identité Nationale Santé » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name ins-identite-nationale-sante`
...
export const ins_identite_nationale_santeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type InsidentitenationalesanteInput = z.infer<typeof ins_identite_nationale_santeSchema>
```
...
```bash
pnpm test src/lib/services/ins-identite-nationale-sante.test.ts
```
...
```bash
pnpm test:integration tests/integration/ins-identite-nationale-sante.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/ins-identite-nationale-sante.spec.ts
```
