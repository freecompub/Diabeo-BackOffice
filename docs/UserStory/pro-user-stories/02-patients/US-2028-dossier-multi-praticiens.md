# US-2028 — Dossier multi-praticiens

> 📌 **2. Patients** · Priorité **V1** · Pays **Universel**
> 
> 💬 **Note inventaire** : Lié RBAC fin
...
Quand il accède à la fonctionnalité « Dossier multi-praticiens »
...
Quand il tente d'accéder à « Dossier multi-praticiens »
...
```gherkin
Étant donné Une opération « Dossier multi-praticiens » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name dossier-multi-praticiens`
...
export const dossier_multi_praticiensSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DossiermultipraticiensInput = z.infer<typeof dossier_multi_praticiensSchema>
```
...
```bash
pnpm test src/lib/services/dossier-multi-praticiens.test.ts
```
...
```bash
pnpm test:integration tests/integration/dossier-multi-praticiens.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/dossier-multi-praticiens.spec.ts
```
