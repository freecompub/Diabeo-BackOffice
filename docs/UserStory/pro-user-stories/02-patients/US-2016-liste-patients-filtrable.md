# US-2016 — Liste patients filtrable
...
Filtres pathologie/équilibre/alertes
...
Quand il accède à la fonctionnalité « Liste patients filtrable »
...
Quand il tente d'accéder à « Liste patients filtrable »
...
```gherkin
Étant donné Une opération « Liste patients filtrable » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name liste-patients-filtrable`
...
export const liste_patients_filtrableSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ListepatientsfiltrableInput = z.infer<typeof liste_patients_filtrableSchema>
```
...
```bash
pnpm test src/lib/services/liste-patients-filtrable.test.ts
```
...
```bash
pnpm test:integration tests/integration/liste-patients-filtrable.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/liste-patients-filtrable.spec.ts
```
