# US-2024 — Historique modifications

> 📌 **2. Patients** · Priorité **V1** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2024` |
| **Référence inventaire** | `FN-024` |
| **Domaine** | 2. Patients |
| **Priorité** | **V1** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (audit log + diff) |
...
Quand il accède à la fonctionnalité « Historique modifications »
...
Quand il tente d'accéder à « Historique modifications »
...
```gherkin
Étant donné Une opération « Historique modifications » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name historique-modifications`
...
export const historique_modificationsSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type HistoriquemodificationsInput = z.infer<typeof historique_modificationsSchema>
```
...
```bash
pnpm test src/lib/services/historique-modifications.test.ts
```
...
```bash
pnpm test:integration tests/integration/historique-modifications.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/historique-modifications.spec.ts
```
