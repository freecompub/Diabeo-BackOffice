...
Observations chronologiques chiffrées
...
Quand il accède à la fonctionnalité « Notes cliniques »
...
Quand il tente d'accéder à « Notes cliniques »
...
```gherkin
Étant donné Une opération « Notes cliniques » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name notes-cliniques`
...
export const notes_cliniquesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type NotescliniquesInput = z.infer<typeof notes_cliniquesSchema>
```
...
```bash
pnpm test src/lib/services/notes-cliniques.test.ts
```
...
```bash
pnpm test:integration tests/integration/notes-cliniques.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/notes-cliniques.spec.ts
```
