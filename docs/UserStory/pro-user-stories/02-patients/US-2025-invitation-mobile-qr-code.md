# US-2025 — Invitation mobile QR code

> 📌 **2. Patients** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Déjà mentionné dans repo
...
|---|---|
| **ID** | `US-2025` |
| **Référence inventaire** | `FN-025` |
| **Domaine** | 2. Patients |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (qrcode lib) |
...
Liaison patient avec app iOS Diabeo
...
Quand il accède à la fonctionnalité « Invitation mobile QR code »
...
Quand il tente d'accéder à « Invitation mobile QR code »
...
```gherkin
Étant donné Une opération « Invitation mobile QR code » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name invitation-mobile-qr-code`
...
export const invitation_mobile_qr_codeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type InvitationmobileqrcodeInput = z.infer<typeof invitation_mobile_qr_codeSchema>
```
...
```bash
pnpm test src/lib/services/invitation-mobile-qr-code.test.ts
```
...
```bash
pnpm test:integration tests/integration/invitation-mobile-qr-code.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/invitation-mobile-qr-code.spec.ts
```
