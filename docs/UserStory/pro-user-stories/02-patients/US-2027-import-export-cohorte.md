...
> 📌 **2. Patients** · Priorité **V2** · Pays **Universel**
> 
...
|---|---|
| **ID** | `US-2027` |
| **Référence inventaire** | `FN-027` |
| **Domaine** | 2. Patients |
| **Priorité** | **V2** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (xlsx, csv-parse) |
...
Quand il accède à la fonctionnalité « Import / export cohorte »
...
Quand il tente d'accéder à « Import / export cohorte »
...
```gherkin
Étant donné Une opération « Import / export cohorte » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name import-export-cohorte`
...
export const import_export_cohorteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ImportexportcohorteInput = z.infer<typeof import_export_cohorteSchema>
```
...
```bash
pnpm test src/lib/services/import-export-cohorte.test.ts
```
...
```bash
pnpm test:integration tests/integration/import-export-cohorte.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/import-export-cohorte.spec.ts
```
