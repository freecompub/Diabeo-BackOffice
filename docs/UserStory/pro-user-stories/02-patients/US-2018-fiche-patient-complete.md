# US-2018 — Fiche patient complète

> 📌 **2. Patients** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Mockups dispo
...
Démo, antécédents, traitements, objectifs
...
Quand il accède à la fonctionnalité « Fiche patient complète »
...
Quand il tente d'accéder à « Fiche patient complète »
...
```gherkin
Étant donné Une opération « Fiche patient complète » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name fiche-patient-complete`
...
export const fiche_patient_completeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type FichepatientcompleteInput = z.infer<typeof fiche_patient_completeSchema>
```
...
```bash
pnpm test src/lib/services/fiche-patient-complete.test.ts
```
...
```bash
pnpm test:integration tests/integration/fiche-patient-complete.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/fiche-patient-complete.spec.ts
```
