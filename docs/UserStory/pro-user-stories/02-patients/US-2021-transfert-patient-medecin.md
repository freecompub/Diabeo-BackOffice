# US-2021 — Transfert patient médecin
...
Réaffectation entre PS du même cabinet
...
Quand il accède à la fonctionnalité « Transfert patient médecin »
...
Quand il tente d'accéder à « Transfert patient médecin »
...
```gherkin
Étant donné Une opération « Transfert patient médecin » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name transfert-patient-medecin`
...
export const transfert_patient_medecinSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TransfertpatientmedecinInput = z.infer<typeof transfert_patient_medecinSchema>
```
...
```bash
pnpm test src/lib/services/transfert-patient-medecin.test.ts
```
...
```bash
pnpm test:integration tests/integration/transfert-patient-medecin.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/transfert-patient-medecin.spec.ts
```
