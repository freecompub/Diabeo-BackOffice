...
Quand il accède à la fonctionnalité « Tags & catégorisation »
...
Quand il tente d'accéder à « Tags & catégorisation »
...
```gherkin
Étant donné Une opération « Tags & catégorisation » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name tags-categorisation`
...
export const tags_categorisationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TagscategorisationInput = z.infer<typeof tags_categorisationSchema>
```
...
```bash
pnpm test src/lib/services/tags-categorisation.test.ts
```
...
```bash
pnpm test:integration tests/integration/tags-categorisation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/tags-categorisation.spec.ts
```
