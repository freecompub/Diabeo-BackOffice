...
> 
> 💬 **Note inventaire** : Complexe avec chiffrement
...
|---|---|
| **ID** | `US-2019` |
| **Référence inventaire** | `FN-019` |
| **Domaine** | 2. Patients |
| **Priorité** | **V1** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | PostgreSQL pg_trgm + HMAC index |
...
Recherche dans dossiers chiffrés
...
Quand il accède à la fonctionnalité « Recherche full-text »
...
Quand il tente d'accéder à « Recherche full-text »
...
```gherkin
Étant donné Une opération « Recherche full-text » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name recherche-full-text`
...
export const recherche_full_textSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type RecherchefulltextInput = z.infer<typeof recherche_full_textSchema>
```
...
```bash
pnpm test src/lib/services/recherche-full-text.test.ts
```
...
```bash
pnpm test:integration tests/integration/recherche-full-text.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/recherche-full-text.spec.ts
```
