# US-2020 — Archivage / soft delete

> 📌 **2. Patients** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Pattern documenté
...
|---|---|
| **ID** | `US-2020` |
| **Référence inventaire** | `FN-020` |
| **Domaine** | 2. Patients |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (soft delete pattern) |
...
Anonymisation conservation 6 ans
...
Quand il accède à la fonctionnalité « Archivage / soft delete »
...
Quand il tente d'accéder à « Archivage / soft delete »
...
```gherkin
Étant donné Une opération « Archivage / soft delete » est effectuée
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name archivage-soft-delete`
...
export const archivage_soft_deleteSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ArchivagesoftdeleteInput = z.infer<typeof archivage_soft_deleteSchema>
```
...
```bash
pnpm test src/lib/services/archivage-soft-delete.test.ts
```
...
```bash
pnpm test:integration tests/integration/archivage-soft-delete.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/archivage-soft-delete.spec.ts
```
