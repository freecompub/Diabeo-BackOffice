...
Quand il accède à la fonctionnalité « Heat-map glycémique »
...
Quand il tente d'accéder à « Heat-map glycémique »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name heat-map-glycemique`
...
export const heat_map_glycemiqueSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type HeatmapglycemiqueInput = z.infer<typeof heat_map_glycemiqueSchema>
```
...
```bash
pnpm test src/lib/services/heat-map-glycemique.test.ts
```
...
```bash
pnpm test:integration tests/integration/heat-map-glycemique.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/heat-map-glycemique.spec.ts
```
