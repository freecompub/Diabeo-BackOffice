...
> 
> 💬 **Note inventaire** : Standard international
...
Ambulatory Glucose Profile 14j
...
Quand il accède à la fonctionnalité « Profil AGP »
...
Quand il tente d'accéder à « Profil AGP »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name profil-agp`
...
export const profil_agpSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ProfilagpInput = z.infer<typeof profil_agpSchema>
```
...
```bash
pnpm test src/lib/services/profil-agp.test.ts
```
...
```bash
pnpm test:integration tests/integration/profil-agp.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/profil-agp.spec.ts
```
