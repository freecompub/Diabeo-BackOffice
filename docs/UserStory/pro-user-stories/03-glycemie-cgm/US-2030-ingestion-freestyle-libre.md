# US-2030 — Ingestion FreeStyle Libre

> 📌 **3. Glycémie & CGM** · Priorité **MVP** · Pays **FR+DZ**
> 
> 💬 **Note inventaire** : Le plus utilisé en FR
...
|---|---|
| **ID** | `US-2030` |
| **Référence inventaire** | `FN-030` |
| **Domaine** | 3. Glycémie & CGM |
| **Priorité** | **MVP** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Oui |
| **Service / Standard** | Abbott LibreLinkUp / LibreView API |
...
### 🌐 Intégration externe — Abbott LibreLinkUp / LibreView API
...
Quand il tente d'accéder à « Ingestion FreeStyle Libre »
...
```gherkin
Étant donné Le service externe « Abbott LibreLinkUp / LibreView API » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Abbott LibreLinkUp / LibreView API » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name ingestion-freestyle-libre`
...
export const ingestion_freestyle_libreSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type IngestionfreestylelibreInput = z.infer<typeof ingestion_freestyle_libreSchema>
```
...
```bash
pnpm test src/lib/services/ingestion-freestyle-libre.test.ts
```
...
- [ ] Un appel à « Abbott LibreLinkUp / LibreView API » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/ingestion-freestyle-libre.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/ingestion-freestyle-libre.spec.ts
```
...
- Documentation officielle de **Abbott LibreLinkUp / LibreView API** (à vérifier au démarrage)
