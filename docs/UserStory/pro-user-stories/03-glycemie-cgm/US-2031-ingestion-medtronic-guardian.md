# US-2031 — Ingestion Medtronic Guardian
...
### 🌐 Intégration externe — Medtronic CareLink API
...
Quand il tente d'accéder à « Ingestion Medtronic Guardian »
...
```gherkin
Étant donné Le service externe « Medtronic CareLink API » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Medtronic CareLink API » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name ingestion-medtronic-guardian`
...
export const ingestion_medtronic_guardianSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type IngestionmedtronicguardianInput = z.infer<typeof ingestion_medtronic_guardianSchema>
```
...
```bash
pnpm test src/lib/services/ingestion-medtronic-guardian.test.ts
```
...
- [ ] Un appel à « Medtronic CareLink API » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/ingestion-medtronic-guardian.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/ingestion-medtronic-guardian.spec.ts
```
...
- Documentation officielle de **Medtronic CareLink API** (à vérifier au démarrage)
