# US-2039 — Comparaison de périodes
...
Quand il tente d'accéder à « Comparaison de périodes »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name comparaison-de-periodes`
...
export const comparaison_de_periodesSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type ComparaisondeperiodesInput = z.infer<typeof comparaison_de_periodesSchema>
```
...
```bash
pnpm test src/lib/services/comparaison-de-periodes.test.ts
```
...
```bash
pnpm test:integration tests/integration/comparaison-de-periodes.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/comparaison-de-periodes.spec.ts
```
