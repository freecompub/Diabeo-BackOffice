# US-2035 — GMI / HbA1c estimée

> 📌 **3. Glycémie & CGM** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Formule Bergenstal 2018
...
Quand il tente d'accéder à « GMI / HbA1c estimée »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name gmi-hba1c-estimee`
...
export const gmi_hba1c_estimeeSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type Gmihba1CestimeeInput = z.infer<typeof gmi_hba1c_estimeeSchema>
```
...
```bash
pnpm test src/lib/services/gmi-hba1c-estimee.test.ts
```
...
```bash
pnpm test:integration tests/integration/gmi-hba1c-estimee.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/gmi-hba1c-estimee.spec.ts
```
