# US-2032 — Glycémies capillaires (BGM)

> 📌 **3. Glycémie & CGM** · Priorité **V1** · Pays **FR+DZ**
> 
...
|---|---|
| **ID** | `US-2032` |
| **Référence inventaire** | `FN-032` |
| **Domaine** | 3. Glycémie & CGM |
| **Priorité** | **V1** |
| **Pays cible** | FR+DZ |
| **Intégration externe** | Partiel |
| **Service / Standard** | Lecteurs Bluetooth (FORA, Roche) |
...
Saisie manuelle ou import lecteur
...
### 🌐 Intégration externe — Lecteurs Bluetooth (FORA, Roche)
...
Quand il accède à la fonctionnalité « Glycémies capillaires (BGM) »
...
Quand il tente d'accéder à « Glycémies capillaires (BGM) »
...
```gherkin
Étant donné Le service externe « Lecteurs Bluetooth (FORA, Roche) » est indisponible
...
- **RM-7 : Aucun secret du fournisseur « Lecteurs Bluetooth (FORA, Roche) » n'est exposé côté client. Tous les appels passent par le backend.**
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name glycemies-capillaires-bgm`
...
export const glycemies_capillaires_bgmSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type GlycemiescapillairesbgmInput = z.infer<typeof glycemies_capillaires_bgmSchema>
```
...
```bash
pnpm test src/lib/services/glycemies-capillaires-bgm.test.ts
```
...
- [ ] Un appel à « Lecteurs Bluetooth (FORA, Roche) » avec mock retourne le résultat attendu
...
```bash
pnpm test:integration tests/integration/glycemies-capillaires-bgm.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/glycemies-capillaires-bgm.spec.ts
```
...
- Documentation officielle de **Lecteurs Bluetooth (FORA, Roche)** (à vérifier au démarrage)
