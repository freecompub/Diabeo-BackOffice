...
Quand il accède à la fonctionnalité « Détection hypo/hyper »
...
Quand il tente d'accéder à « Détection hypo/hyper »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name detection-hypo-hyper`
...
export const detection_hypo_hyperSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type DetectionhypohyperInput = z.infer<typeof detection_hypo_hyperSchema>
```
...
```bash
pnpm test src/lib/services/detection-hypo-hyper.test.ts
```
...
```bash
pnpm test:integration tests/integration/detection-hypo-hyper.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/detection-hypo-hyper.spec.ts
```
