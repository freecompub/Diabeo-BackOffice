# US-2036 — Coefficient de variation
...
Quand il tente d'accéder à « Coefficient de variation »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name coefficient-de-variation`
...
export const coefficient_de_variationSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type CoefficientdevariationInput = z.infer<typeof coefficient_de_variationSchema>
```
...
```bash
pnpm test src/lib/services/coefficient-de-variation.test.ts
```
...
```bash
pnpm test:integration tests/integration/coefficient-de-variation.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/coefficient-de-variation.spec.ts
```
