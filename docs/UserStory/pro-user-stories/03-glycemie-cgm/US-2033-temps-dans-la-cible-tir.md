# US-2033 — Temps dans la cible (TIR)

> 📌 **3. Glycémie & CGM** · Priorité **MVP** · Pays **Universel**
> 
> 💬 **Note inventaire** : Algo standardisé
...
|---|---|
| **ID** | `US-2033` |
| **Référence inventaire** | `FN-033` |
| **Domaine** | 3. Glycémie & CGM |
| **Priorité** | **MVP** |
| **Pays cible** | Universel |
| **Intégration externe** | Non |
| **Service / Standard** | Interne (algo standard IDC) |
...
Quand il accède à la fonctionnalité « Temps dans la cible (TIR) »
...
Quand il tente d'accéder à « Temps dans la cible (TIR) »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name temps-dans-la-cible-tir`
...
export const temps_dans_la_cible_tirSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type TempsdanslacibletirInput = z.infer<typeof temps_dans_la_cible_tirSchema>
```
...
```bash
pnpm test src/lib/services/temps-dans-la-cible-tir.test.ts
```
...
```bash
pnpm test:integration tests/integration/temps-dans-la-cible-tir.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/temps-dans-la-cible-tir.spec.ts
```
