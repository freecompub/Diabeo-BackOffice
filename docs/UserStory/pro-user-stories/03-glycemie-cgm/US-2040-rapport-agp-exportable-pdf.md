# US-2040 — Rapport AGP exportable PDF

> 📌 **3. Glycémie & CGM** · Priorité **V1** · Pays **Universel**
> 
...
Génération PDF cliniquement validé
...
Quand il accède à la fonctionnalité « Rapport AGP exportable PDF »
...
Quand il tente d'accéder à « Rapport AGP exportable PDF »
...
- Créer une migration Prisma dédiée : `pnpm prisma migrate dev --name rapport-agp-exportable-pdf`
...
export const rapport_agp_exportable_pdfSchema = z.object({
  // À compléter selon la fonctionnalité
  // Exemples : id: z.string().cuid(), value: z.number().positive(), ...
})

export type RapportagpexportablepdfInput = z.infer<typeof rapport_agp_exportable_pdfSchema>
```
...
```bash
pnpm test src/lib/services/rapport-agp-exportable-pdf.test.ts
```
...
```bash
pnpm test:integration tests/integration/rapport-agp-exportable-pdf.test.ts
```
...
```bash
pnpm test:e2e tests/e2e/rapport-agp-exportable-pdf.spec.ts
```
