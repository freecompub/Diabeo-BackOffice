...
## US-2600 — Navigation globale (sidebar maigre + transitions)
...
- La liste de résultats n'expose **aucune PII de santé** (uniquement identité + drapeau) ; le détail n'est chargé qu'à l'ouverture du dossier (cf. US-2604).
- `AuditLog` sur **ouverture de patient**, pas sur la frappe de recherche.

### ⚠️ Points ouverts
- **Décidé** : raccourcis secondaires (« G puis J ») inclus en **V1**.

---

## US-2602 — « Ma journée » (worklist de tri du médecin)
...
### ⚠️ Points ouverts
- **Décidé** : ordre des sections **fixe en V1** (personnalisation non retenue pour l'instant).
...
- « Nouvelle consultation » ouvre/reprend un `Encounter` (cf. US-2605).
...
## US-2604 — Navigation interne du dossier patient (onglets-routes)
...
### ⚠️ Points ouverts
- **Onglet Historique** — **décidé : à livrer en V2.** Le seuil de « changement cliniquement significatif » (ce qui crée une entrée vs une simple mise à jour, historisation RGPD) reste à cadrer avec `medical-domain-validator` (V2).
...
### ⚠️ Points ouverts
1. **Frontière `Encounter`** — **décidé** : ouvrir le mode revue **reprend** la consultation ouverte du jour si elle existe, sinon **en crée une** ; un **brouillon est conservé** en cas d'interruption/timeout (compte rendu non finalisé).
2. **Revue `NURSE`** — **décidé** : `NURSE` autorisé pour les revues **éducation / suivi** uniquement (décision thérapeutique réservée `DOCTOR`).
3. **Gabarits de compte rendu** — **décidé** : compte rendu **pré-rempli (modèle générique)** **+** possibilité pour le médecin de **définir ses propres modèles**. Générique en **V1** ; modèles personnalisés du médecin en **V2**.
...
- **Ajout** : worklist « Ma journée » (US-2602) + palette `Ctrl-K` (US-2601) + barre de contexte/switcher (US-2603).
...
>
> ⚠️ **Dépendance bloquante** : ces 2 US présupposent le **modèle de capacités Q1/Q2 org-scopé** (à spécifier dans une US d'accès dédiée — `US-2610 (`gestion-personnel-droits-us.md`)`) : grant administratif, scope (cabinet / équipe), gating RPPS, non-auto-élévation, audit des octrois. Sans lui, la nav de gestion n'a rien à filtrer.
>
> **Roadmap : Variante A (US-2606) en V1 · Variante B (US-2607) en V3.**

---

## US-2606 — Bloc « Gestion cabinet » dans la sidebar (**Variante A — V1**)
...
- La sidebar se compose de **deux blocs indépendants**, chacun affiché selon sa capacité :
  - **Bloc clinique** (si **Q1**) : Ma journée, Patients, Agenda, Messagerie, Documents, Analytics (cf. US-2600).
...
- FR/AR + RTL corrects.
- Repli drawer (cf. US-2600) couvre aussi le bloc gestion.
...
### ⚠️ Points ouverts
- Sous-périmètre des grants en gestion (qui peut octroyer quoi, à qui) → relève de l'US d'accès `US-2610 (`gestion-personnel-droits-us.md`)`.

---

## US-2607 — Bascule « Mode soin ⇄ Mode gestion » (**Variante B — V3**)
...
- Le **Mode gestion n'expose aucune donnée de santé** ; le **Mode soin** garde le périmètre clinique (US-2600 à US-2605).
...
### ⚠️ Points ouverts
- **Décidé** : persistance du mode = **préférence serveur** (suit l'utilisateur d'un appareil à l'autre).
- Comportement d'un **deep-link Soin** reçu en mode Gestion (et inversement) : le deep-link **prime** et bascule le mode. *(à confirmer)*

### 🗺️ Roadmap
- **V1** : Variante A (US-2606) — coût minimal, blocs isolés.
- **V3** : Variante B (US-2607) — bascule + routing par mode, par-dessus A.

---

## US-2608 — Historique du dossier patient (**V2**)
...
### 🔗 Dépendances
US-2604 (onglet) · `Encounter` + addendum · `AdjustmentProposal` · `AuditLog`.

---

## US-2609 — Modèles de compte rendu personnalisés (**V2**)
...
### 📌 Description fonctionnelle
- Rappel V1 (US-2605) : **modèle générique pré-rempli**.
...
- Le compte rendu final reste un **addendum immuable** (US-2605).
...
### 🔗 Dépendances
US-2605 (compte rendu) · `Encounter` + addendum.
