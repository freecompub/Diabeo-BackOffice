...
## US-2615 — Découpler l'accès aux données de santé du rôle plateforme (**F1 — V4**)
### 👤 / 🎯
...
Tant que non livré, `ADMIN` garde l'accès PHI (cf. US-2610 §Phasage). Mesure transitoire : pas de rôle plateforme à un non-soignant.

---

## US-2616 — Modèle Tenant + politique de vérification fail-secure (**F2 — V1**)
### 👤 / 🎯
...
## US-2617 — Appartenance scopée avec capacités + enregistrement PS générique (**F4 — V1**)
### 👤 / 🎯
...
## US-2618 — Isolation patient en cabinet de groupe (**F6 — V1**)
### 👤 / 🎯
...
## US-2619 — Révocation immédiate de capacité (**F7 — V1**)
### 👤 / 🎯
...
## US-2620 — Couverture d'audit du socle (**F8 — V1**)
### 👤 / 🎯
...
## 🔗 Dépendances
`US-2610` · `US-2611` · `prisma/schema.prisma` (`Role`, `HealthcareService/Member`, `AuditLog`, `Session`) · `src/lib/auth/*` · `src/lib/access-control.ts` · `src/lib/org-access.ts`.

---

## US-2621 — Sécurité de session (mono-session + timeout + durées) (**V1**)
...
- **Mono-session** : un seul token valide à la fois pour **tout rôle backoffice** ; **exception `PATIENT`** (multi-appareils). *(consolidé depuis US-2610)*
...
- Durées documentées/ajustées ; **révocation immédiate** (lien US-2619 / F7).
...
### 🔗 Dépendances
`US-2610` (mono-session) · `US-2619` (révocation) · `src/lib/auth/*`.
