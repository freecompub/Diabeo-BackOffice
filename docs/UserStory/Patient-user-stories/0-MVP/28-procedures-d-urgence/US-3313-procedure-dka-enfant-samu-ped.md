# US-3313 — Procédure DKA enfant → SAMU péd
...
|---|---|
| **ID** | `US-3313` |
| **Référence inventaire** | `FNP-313` |
| **Domaine** | 28. Procédures d'urgence |
| **Priorité** | **MVP** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | Téléphonie + SMS + push parents |
| **Modèle économique** | Gratuit + SMS |
...
Alerte SAMU pédiatrique + parents
...
La fonctionnalité « Procédure DKA enfant → SAMU péd » est exposée selon les conventions iOS :
...
### Comportement utilisateur
La fonctionnalité « Procédure DKA enfant → SAMU péd » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Procédure DKA enfant → SAMU péd » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Procédure DKA enfant → SAMU péd) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Procédure DKA enfant → SAMU péd »
...
```gherkin
Étant donné Le service « Téléphonie + SMS + push parents » est indisponible
...
```gherkin
Étant donné La fonctionnalité « Procédure DKA enfant → SAMU péd » est déclenchée (auto ou manuel)
...
Quand un événement Procédure DKA enfant → SAMU péd se produit
...
Quand elle utilise « Procédure DKA enfant → SAMU péd »
...
Quand « Procédure DKA enfant → SAMU péd » s'exécute pendant la journée de jeûne
...
- **RM-6 : Aucun secret du fournisseur « Téléphonie + SMS + push parents » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **Téléphonie + SMS + push parents** (à valider en kick-off)
