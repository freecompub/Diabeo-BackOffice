# US-3293 — Procédure hypo nocturne (réveil)
...
|---|---|
| **ID** | `US-3293` |
| **Référence inventaire** | `FNP-293` |
| **Domaine** | 28. Procédures d'urgence |
| **Priorité** | **MVP** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | iOS Critical Alerts |
| **Modèle économique** | Gratuit (autorisation Apple) |
| **Coût estimé** | — |
...
La fonctionnalité « Procédure hypo nocturne (réveil) » est exposée selon les conventions iOS :
...
### Comportement utilisateur
La fonctionnalité « Procédure hypo nocturne (réveil) » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Procédure hypo nocturne (réveil) » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Procédure hypo nocturne (réveil)) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Procédure hypo nocturne (réveil) »
...
```gherkin
Étant donné Le service « iOS Critical Alerts » est indisponible
...
```gherkin
Étant donné La fonctionnalité « Procédure hypo nocturne (réveil) » est déclenchée (auto ou manuel)
...
Quand un événement Procédure hypo nocturne (réveil) se produit
...
Quand elle utilise « Procédure hypo nocturne (réveil) »
...
- **RM-6 : Aucun secret du fournisseur « iOS Critical Alerts » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **iOS Critical Alerts** (à valider en kick-off)
