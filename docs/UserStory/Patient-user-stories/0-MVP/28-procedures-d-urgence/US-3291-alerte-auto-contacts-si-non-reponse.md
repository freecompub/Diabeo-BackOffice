# US-3291 — Alerte auto contacts si non-réponse
...
|---|---|
| **ID** | `US-3291` |
| **Référence inventaire** | `FNP-291` |
| **Domaine** | 28. Procédures d'urgence |
| **Priorité** | **MVP** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | FCM / APNs + SMS Twilio |
...
La fonctionnalité « Alerte auto contacts si non-réponse » est exposée selon les conventions iOS :
...
### Comportement utilisateur
La fonctionnalité « Alerte auto contacts si non-réponse » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Alerte auto contacts si non-réponse » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Alerte auto contacts si non-réponse) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Alerte auto contacts si non-réponse »
...
```gherkin
Étant donné Le service « FCM / APNs + SMS Twilio » est indisponible
...
```gherkin
Étant donné La fonctionnalité « Alerte auto contacts si non-réponse » est déclenchée (auto ou manuel)
...
Quand un événement Alerte auto contacts si non-réponse se produit
...
Quand elle utilise « Alerte auto contacts si non-réponse »
...
Quand « Alerte auto contacts si non-réponse » s'exécute pendant la journée de jeûne
...
- **RM-6 : Aucun secret du fournisseur « FCM / APNs + SMS Twilio » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **FCM / APNs + SMS Twilio** (à valider en kick-off)
