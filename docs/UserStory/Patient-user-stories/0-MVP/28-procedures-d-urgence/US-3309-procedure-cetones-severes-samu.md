# US-3309 — Procédure cétones sévères → SAMU

> 📌 **28. Procédures d'urgence** · Priorité **MVP** · Plateforme **📱**

> 💬 **Note inventaire** : Vital — DZ: 14 SAMU
...
|---|---|
| **ID** | `US-3309` |
| **Référence inventaire** | `FNP-309` |
| **Domaine** | 28. Procédures d'urgence |
| **Priorité** | **MVP** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | Téléphonie OS + GPS + SMS |
| **Modèle économique** | Gratuit (OS) + SMS |
| **Coût estimé** | 0–0.10€/SMS |
| **Faisabilité France** | Direct |
| **Faisabilité Algérie** | Avec partenaire |
| **Story points (3 plateformes)** | iOS 5 · Android 5 · Web 4 |
| **Statut** | 🆕 À démarrer |
| **Dépendances** | US-3001 (Inscription / login), US-3026 (Profil patient — données démographiques), US-3290 (Détection seuil hypo) |
...
🤝 **Algérie : partenariat local nécessaire** pour l'implémentation complète. France : déploiement direct.
...
- **Critical Alerts** : entitlement `com.apple.developer.usernotifications.critical-alerts` — nécessite procédure de demande spécifique auprès d'Apple
- Téléphonie via `CallKit` ou `tel:` URL scheme pour appel SAMU (15)

### Permissions à déclarer (`Info.plist`)
- Notifications (gérées par `UNUserNotificationCenter`)
- `NSLocationWhenInUseUsageDescription` — partage géoloc en cas d'urgence

> ⚠️ **Critical Alerts iOS** : la procédure d'autorisation Apple peut prendre plusieurs semaines. À initier dès le début du sprint.

### Comportement utilisateur
La fonctionnalité « Procédure cétones sévères → SAMU » est exposée selon les conventions iOS :
...
### Effort estimé
**5 story points** (Fibonacci) sur la plateforme iOS
...
### APIs et capacités spécifiques
- Numérotation `Intent.ACTION_CALL` vers SAMU (15) ou 112

### Permissions à déclarer (`AndroidManifest.xml`)
- `android.permission.POST_NOTIFICATIONS` (API 33+)
- `android.permission.ACCESS_FINE_LOCATION`
- `android.permission.CALL_PHONE` (numérotation directe SAMU)

### Comportement utilisateur
La fonctionnalité « Procédure cétones sévères → SAMU » suit les Material Design 3 guidelines :
...
### Effort estimé
**5 story points** (Fibonacci) sur la plateforme Android
...
- `navigator.geolocation` pour partage de position avec permission utilisateur
...
- Pas d'appel téléphonique direct : seulement lien `tel:` (ouverture de l'app téléphone si dispo)

### Stratégies de fallback
- Aucun fallback nécessaire — fonctionnalité native web

### Comportement utilisateur
La version web de « Procédure cétones sévères → SAMU » est optimisée pour :
...
### Effort estimé
**4 story points** (Fibonacci) sur la plateforme Web
...
- **Notifications doublées** : toute alerte critique (Procédure cétones sévères → SAMU) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Procédure cétones sévères → SAMU »
...
```gherkin
Étant donné Le service « Téléphonie OS + GPS + SMS » est indisponible
...
### AC-4 — Procédure d'urgence guidée pas-à-pas

```gherkin
Étant donné La fonctionnalité « Procédure cétones sévères → SAMU » est déclenchée (auto ou manuel)
Quand le patient suit la procédure
Alors chaque étape est claire, minutée si pertinent, et validée avant de passer à la suivante
```

### AC-5 — Journal d'urgence immuable

```gherkin
Étant donné Une procédure d'urgence est exécutée
Quand elle se termine (succès ou interruption)
Alors un AuditLog d'urgence est créé, immuable, et envoyé au médecin référent dans la journée
```

### AC-6 — Mode pédiatrique : double notification

```gherkin
Étant donné Un patient mineur a deux parents administrateurs
Quand un événement Procédure cétones sévères → SAMU se produit
Alors les deux parents reçoivent la notification (push + email selon préférences) avec accusé de lecture
```

### AC-7 — Mode grossesse : cibles strictes appliquées

```gherkin
Étant donné Une patiente est en mode grossesse activé
Quand elle utilise « Procédure cétones sévères → SAMU »
Alors les seuils glycémiques utilisés sont les cibles strictes obstétriques (< 95 mg/dL à jeun, < 120 mg/dL post-prandial)
```

### AC-8 — Mode Ramadan : adaptation aux périodes de jeûne

```gherkin
Étant donné Le mode Ramadan est activé avec horaires Sahur/Iftar configurés
Quand « Procédure cétones sévères → SAMU » s'exécute pendant la journée de jeûne
...
- **RM-6 : Aucun secret du fournisseur « Téléphonie OS + GPS + SMS » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
- **RM-7 : Une procédure d'urgence ne peut JAMAIS être annulée silencieusement — toute interruption génère un AuditLog explicite.**
- **RM-8 : Les procédures d'urgence fonctionnent en hors-ligne complet (assets et logique embarqués).**
...
- Documentation officielle **Téléphonie OS + GPS + SMS** (à valider en kick-off)
