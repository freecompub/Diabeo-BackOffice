# US-3157 — Push alertes critiques (hypo)

> 📌 **11. Messagerie & notifs** · Priorité **MVP** · Plateforme **📱**

> 💬 **Note inventaire** : Critical Alerts iOS = procédure Apple
...
|---|---|
| **ID** | `US-3157` |
| **Référence inventaire** | `FNP-157` |
| **Domaine** | 11. Messagerie & notifs |
| **Priorité** | **MVP** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | FCM / APNs (Critical Alerts iOS) |
| **Modèle économique** | Gratuit (mais autorisation Apple) |
| **Coût estimé** | Demande Apple |
...
La fonctionnalité « Push alertes critiques (hypo) » est exposée selon les conventions iOS :
...
### Comportement utilisateur
La fonctionnalité « Push alertes critiques (hypo) » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Push alertes critiques (hypo) » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Push alertes critiques (hypo)) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Push alertes critiques (hypo) »
...
```gherkin
Étant donné Le service « FCM / APNs (Critical Alerts iOS) » est indisponible
...
Quand un événement Push alertes critiques (hypo) se produit
...
Quand « Push alertes critiques (hypo) » s'exécute pendant la journée de jeûne
...
- **RM-6 : Aucun secret du fournisseur « FCM / APNs (Critical Alerts iOS) » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **FCM / APNs (Critical Alerts iOS)** (à valider en kick-off)
