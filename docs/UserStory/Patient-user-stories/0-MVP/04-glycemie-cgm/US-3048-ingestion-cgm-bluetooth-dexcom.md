# US-3048 — Ingestion CGM Bluetooth Dexcom

> 📌 **4. Glycémie & CGM** · Priorité **MVP** · Plateforme **📱**

> 💬 **Note inventaire** : DZ: dispo mais distri limitée
...
|---|---|
| **ID** | `US-3048` |
| **Référence inventaire** | `FNP-048` |
| **Domaine** | 4. Glycémie & CGM |
| **Priorité** | **MVP** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | Dexcom Developer API |
| **Modèle économique** | Partenariat (gratuit) |
...
Sync via Dexcom Share/Clarity API
...
- **Langage** : Swift 5.9+
- **Frameworks** : `SwiftUI`, `Combine`, `CoreBluetooth`, `CoreData (cache local chiffré)`
...
La fonctionnalité « Ingestion CGM Bluetooth Dexcom » est exposée selon les conventions iOS :
...
- **Langage** : Kotlin 1.9+
- **Dépendances** : `Jetpack Compose`, `Coroutines + Flow`, `Nordic BLE Library (recommandé) ou Android BluetoothLE`, `Room (cache chiffré via SQLCipher) + WorkManager pour sync différée`
...
### Comportement utilisateur
La fonctionnalité « Ingestion CGM Bluetooth Dexcom » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Ingestion CGM Bluetooth Dexcom » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Ingestion CGM Bluetooth Dexcom) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Ingestion CGM Bluetooth Dexcom »
...
```gherkin
Étant donné Le service « Dexcom Developer API » est indisponible
...
Quand un événement Ingestion CGM Bluetooth Dexcom se produit
...
- **RM-6 : Aucun secret du fournisseur « Dexcom Developer API » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **Dexcom Developer API** (à valider en kick-off)
