# US-3049 — Ingestion FreeStyle Libre

> 📌 **4. Glycémie & CGM** · Priorité **MVP** · Plateforme **📱**

> 💬 **Note inventaire** : Le plus utilisé
...
|---|---|
| **ID** | `US-3049` |
| **Référence inventaire** | `FNP-049` |
| **Domaine** | 4. Glycémie & CGM |
| **Priorité** | **MVP** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | Abbott LibreLinkUp API |
| **Modèle économique** | Partenariat (gratuit) |
| **Coût estimé** | Négocié Abbott |
...
La fonctionnalité « Ingestion FreeStyle Libre » est exposée selon les conventions iOS :
...
### Comportement utilisateur
La fonctionnalité « Ingestion FreeStyle Libre » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Ingestion FreeStyle Libre » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Ingestion FreeStyle Libre) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Ingestion FreeStyle Libre »
...
```gherkin
Étant donné Le service « Abbott LibreLinkUp API » est indisponible
...
Quand un événement Ingestion FreeStyle Libre se produit
...
- **RM-6 : Aucun secret du fournisseur « Abbott LibreLinkUp API » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **Abbott LibreLinkUp API** (à valider en kick-off)
