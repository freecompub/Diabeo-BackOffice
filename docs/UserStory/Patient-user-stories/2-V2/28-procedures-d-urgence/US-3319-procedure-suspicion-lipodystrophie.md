# US-3319 — Procédure suspicion lipodystrophie
...
- `NSCameraUsageDescription` — scan de codes-barres / photos repas

### Comportement utilisateur
La fonctionnalité « Procédure suspicion lipodystrophie » est exposée selon les conventions iOS :
...
- **Langage** : Kotlin 1.9+
- **Dépendances** : `Jetpack Compose`, `Coroutines + Flow`, `CameraX + Google ML Kit Barcode Scanning (gratuit, on-device)`
...
### Comportement utilisateur
La fonctionnalité « Procédure suspicion lipodystrophie » suit les Material Design 3 guidelines :
...
### APIs web utilisées
- `navigator.mediaDevices.getUserMedia()` pour capture photo (avec permission utilisateur)
...
### Comportement utilisateur
La version web de « Procédure suspicion lipodystrophie » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Procédure suspicion lipodystrophie) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Procédure suspicion lipodystrophie »
...
```gherkin
Étant donné La fonctionnalité « Procédure suspicion lipodystrophie » est déclenchée (auto ou manuel)
...
Quand un événement Procédure suspicion lipodystrophie se produit
