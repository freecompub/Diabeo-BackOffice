# US-3270 — Calendrier hégirien optionnel DZ
...
|---|---|
| **ID** | `US-3270` |
| **Référence inventaire** | `FNP-270` |
| **Domaine** | 25. Multi-pays |
| **Priorité** | **V2** |
| **Plateforme cible** | 📱🖥️ |
| **Intégration externe** | Oui |
| **Service / Standard** | Algorithmes Umm al-Qura (libres) |
...
La fonctionnalité « Calendrier hégirien optionnel DZ » est exposée selon les conventions iOS :
...
### Comportement utilisateur
La fonctionnalité « Calendrier hégirien optionnel DZ » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Calendrier hégirien optionnel DZ » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Calendrier hégirien optionnel DZ) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Calendrier hégirien optionnel DZ »
...
```gherkin
Étant donné Le service « Algorithmes Umm al-Qura (libres) » est indisponible
...
Quand un événement Calendrier hégirien optionnel DZ se produit
...
- **RM-6 : Aucun secret du fournisseur « Algorithmes Umm al-Qura (libres) » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **Algorithmes Umm al-Qura (libres)** (à valider en kick-off)
