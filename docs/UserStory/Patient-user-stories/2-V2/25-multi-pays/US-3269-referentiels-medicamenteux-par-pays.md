# US-3269 — Référentiels médicamenteux par pays

> 📌 **25. Multi-pays** · Priorité **V2** · Plateforme **📱🖥️**
...
|---|---|
| **ID** | `US-3269` |
| **Référence inventaire** | `FNP-269` |
| **Domaine** | 25. Multi-pays |
| **Priorité** | **V2** |
| **Plateforme cible** | 📱🖥️ |
| **Intégration externe** | Oui |
| **Service / Standard** | Voir Prescription (BdM agréées) |
...
La fonctionnalité « Référentiels médicamenteux par pays » est exposée selon les conventions iOS :
...
### Comportement utilisateur
La fonctionnalité « Référentiels médicamenteux par pays » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Référentiels médicamenteux par pays » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Référentiels médicamenteux par pays) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Référentiels médicamenteux par pays »
...
```gherkin
Étant donné Le service « Voir Prescription (BdM agréées) » est indisponible
...
Quand un événement Référentiels médicamenteux par pays se produit
...
- **RM-6 : Aucun secret du fournisseur « Voir Prescription (BdM agréées) » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **Voir Prescription (BdM agréées)** (à valider en kick-off)
