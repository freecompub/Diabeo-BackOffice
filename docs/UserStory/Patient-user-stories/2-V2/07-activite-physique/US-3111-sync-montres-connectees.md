# US-3111 — Sync montres connectées

> 📌 **7. Activité physique** · Priorité **V2** · Plateforme **📱**
...
|---|---|
| **ID** | `US-3111` |
| **Référence inventaire** | `FNP-111` |
| **Domaine** | 7. Activité physique |
| **Priorité** | **V2** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | HealthKit / Connect IQ / Fitbit Web API |
| **Modèle économique** | Gratuit (SDK) |
| **Coût estimé** | Frais publication app |
| **Faisabilité France** | Direct |
| **Faisabilité Algérie** | Direct |
| **Story points (3 plateformes)** | iOS 8 · Android 8 · Web 6 |
| **Statut** | 🆕 À démarrer |
...
- **Langage** : Swift 5.9+
- **Frameworks** : `SwiftUI`, `Combine`, `CoreBluetooth`, `HealthKit`, `WatchKit + watchOS app companion`, `CoreData (cache local chiffré)`
- **Architecture** : MVVM + Coordinator, dépendances injectées via Swinject ou similaire

### APIs et capacités spécifiques
- Complication WatchOS pour accès rapide depuis le poignet
...
La fonctionnalité « Sync montres connectées » est exposée selon les conventions iOS :
...
### Effort estimé
**8 story points** (Fibonacci) sur la plateforme iOS
...
- **Langage** : Kotlin 1.9+
- **Dépendances** : `Jetpack Compose`, `Coroutines + Flow`, `Nordic BLE Library (recommandé) ou Android BluetoothLE`, `Health Connect (Android 14+) + Google Fit fallback`, `Wear OS Compose`, `Room (cache chiffré via SQLCipher) + WorkManager pour sync différée`
- **Architecture** : MVVM + Hilt (DI) + Repository pattern

### APIs et capacités spécifiques
- Tile Wear OS pour accès rapide
...
### Comportement utilisateur
La fonctionnalité « Sync montres connectées » suit les Material Design 3 guidelines :
...
### Effort estimé
**8 story points** (Fibonacci) sur la plateforme Android
...
- **Pas de wearable companion** : N/A en web
...
### Comportement utilisateur
La version web de « Sync montres connectées » est optimisée pour :
...
### Effort estimé
**6 story points** (Fibonacci) sur la plateforme Web
...
Quand il accède à la fonctionnalité « Sync montres connectées »
...
```gherkin
Étant donné Le service « HealthKit / Connect IQ / Fitbit Web API » est indisponible
...
- **RM-6 : Aucun secret du fournisseur « HealthKit / Connect IQ / Fitbit Web API » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **HealthKit / Connect IQ / Fitbit Web API** (à valider en kick-off)
