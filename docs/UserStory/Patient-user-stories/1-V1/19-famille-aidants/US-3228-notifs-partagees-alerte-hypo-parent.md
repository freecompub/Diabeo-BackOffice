# US-3228 — Notifs partagées (alerte hypo → parent)

> 📌 **19. Famille & aidants** · Priorité **V1** · Plateforme **📱**

> 💬 **Note inventaire** : Critique pédiatrie
...
|---|---|
| **ID** | `US-3228` |
| **Référence inventaire** | `FNP-228` |
| **Domaine** | 19. Famille & aidants |
| **Priorité** | **V1** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | FCM / APNs |
...
La fonctionnalité « Notifs partagées (alerte hypo → parent) » est exposée selon les conventions iOS :
...
### Comportement utilisateur
La fonctionnalité « Notifs partagées (alerte hypo → parent) » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Notifs partagées (alerte hypo → parent) » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Notifs partagées (alerte hypo → parent)) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Notifs partagées (alerte hypo → parent) »
...
```gherkin
Étant donné Le service « FCM / APNs » est indisponible
...
Quand un événement Notifs partagées (alerte hypo → parent) se produit
...
Quand elle utilise « Notifs partagées (alerte hypo → parent) »
...
- **RM-6 : Aucun secret du fournisseur « FCM / APNs » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **FCM / APNs** (à valider en kick-off)
