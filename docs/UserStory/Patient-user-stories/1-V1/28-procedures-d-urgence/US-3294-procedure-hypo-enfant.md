# US-3294 — Procédure hypo enfant

> 📌 **28. Procédures d'urgence** · Priorité **V1** · Plateforme **📱**
...
|---|---|
| **ID** | `US-3294` |
| **Référence inventaire** | `FNP-294` |
| **Domaine** | 28. Procédures d'urgence |
| **Priorité** | **V1** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | FCM / APNs multi-recipient |
...
La fonctionnalité « Procédure hypo enfant » est exposée selon les conventions iOS :
...
### APIs et capacités spécifiques
- **Notifications haute priorité** : `NotificationManager.IMPORTANCE_HIGH` + `setCategory(CATEGORY_ALARM)` + `FullScreenIntent` pour bypass DND

### Permissions à déclarer (`AndroidManifest.xml`)
- `android.permission.POST_NOTIFICATIONS` (API 33+)

### Comportement utilisateur
La fonctionnalité « Procédure hypo enfant » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Procédure hypo enfant » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Procédure hypo enfant) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Procédure hypo enfant »
...
```gherkin
Étant donné Le service « FCM / APNs multi-recipient » est indisponible
...
```gherkin
Étant donné La fonctionnalité « Procédure hypo enfant » est déclenchée (auto ou manuel)
...
Quand un événement Procédure hypo enfant se produit
...
Quand elle utilise « Procédure hypo enfant »
...
Quand « Procédure hypo enfant » s'exécute pendant la journée de jeûne
...
- **RM-6 : Aucun secret du fournisseur « FCM / APNs multi-recipient » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **FCM / APNs multi-recipient** (à valider en kick-off)
