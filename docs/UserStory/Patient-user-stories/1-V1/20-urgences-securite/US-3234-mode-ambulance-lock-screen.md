# US-3234 — Mode ambulance (lock screen)

> 📌 **20. Urgences & sécurité** · Priorité **V1** · Plateforme **📱**
...
|---|---|
| **ID** | `US-3234` |
| **Référence inventaire** | `FNP-234` |
| **Domaine** | 20. Urgences & sécurité |
| **Priorité** | **V1** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | iOS Medical ID / Android Emergency Info |
| **Modèle économique** | Gratuit (OS) |
| **Coût estimé** | — |
...
- **Langage** : Swift 5.9+
- **Frameworks** : `SwiftUI`, `Combine`, `WidgetKit`, `CoreLocation`
...
La fonctionnalité « Mode ambulance (lock screen) » est exposée selon les conventions iOS :
...
- **Langage** : Kotlin 1.9+
- **Dépendances** : `Jetpack Compose`, `Coroutines + Flow`, `Jetpack Glance (composables widgets)`, `FusedLocationProviderClient`
- **Architecture** : MVVM + Hilt (DI) + Repository pattern

### APIs et capacités spécifiques
- Widget Home Screen via Jetpack Glance — disponible Android 12+
...
### Comportement utilisateur
La fonctionnalité « Mode ambulance (lock screen) » suit les Material Design 3 guidelines :
...
- **Pas de widget** : concept N/A en web (le browser est déjà la "home")
...
### Comportement utilisateur
La version web de « Mode ambulance (lock screen) » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Mode ambulance (lock screen)) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Mode ambulance (lock screen) »
...
```gherkin
Étant donné Le service « iOS Medical ID / Android Emergency Info » est indisponible
...
- **RM-6 : Aucun secret du fournisseur « iOS Medical ID / Android Emergency Info » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **iOS Medical ID / Android Emergency Info** (à valider en kick-off)
