# US-3254 — Lecteur d'écran (VoiceOver/TalkBack)

> 📌 **23. Préférences & perso** · Priorité **V1** · Plateforme **📱**
...
|---|---|
| **ID** | `US-3254` |
| **Référence inventaire** | `FNP-254` |
| **Domaine** | 23. Préférences & perso |
| **Priorité** | **V1** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | Apple VoiceOver / Android TalkBack |
...
- **Langage** : Swift 5.9+
- **Frameworks** : `SwiftUI`, `Combine`, `CoreBluetooth`, `Speech + AVAudioEngine`
- **Architecture** : MVVM + Coordinator, dépendances injectées via Swinject ou similaire

### APIs et capacités spécifiques
- Intégration **App Intents** pour Siri ("Hey Siri, ...")
- Conformité **VoiceOver**, Dynamic Type, Smart Invert, Reduce Motion
...
La fonctionnalité « Lecteur d'écran (VoiceOver/TalkBack) » est exposée selon les conventions iOS :
...
- **Langage** : Kotlin 1.9+
- **Dépendances** : `Jetpack Compose`, `Coroutines + Flow`, `Nordic BLE Library (recommandé) ou Android BluetoothLE`, `SpeechRecognizer + App Shortcuts pour Google Assistant`
- **Architecture** : MVVM + Hilt (DI) + Repository pattern

### APIs et capacités spécifiques
- Conformité **TalkBack**, support du zoom système, haut contraste
...
### Comportement utilisateur
La fonctionnalité « Lecteur d'écran (VoiceOver/TalkBack) » suit les Material Design 3 guidelines :
...
### APIs web utilisées
- **Web Speech API** (SpeechRecognition + SpeechSynthesis) — support très partiel sur iOS Safari
...
- Pour les fonctions de saisie vocale critique : déléguer au mobile

### Comportement utilisateur
La version web de « Lecteur d'écran (VoiceOver/TalkBack) » est optimisée pour :
...
Quand il accède à la fonctionnalité « Lecteur d'écran (VoiceOver/TalkBack) »
...
```gherkin
Étant donné Le service « Apple VoiceOver / Android TalkBack » est indisponible
...
- **RM-6 : Aucun secret du fournisseur « Apple VoiceOver / Android TalkBack » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **Apple VoiceOver / Android TalkBack** (à valider en kick-off)
