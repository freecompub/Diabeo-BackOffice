# US-3109 — Sync Apple HealthKit

> 📌 **7. Activité physique** · Priorité **V1** · Plateforme **📱**
...
|---|---|
| **ID** | `US-3109` |
| **Référence inventaire** | `FNP-109` |
| **Domaine** | 7. Activité physique |
| **Priorité** | **V1** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | Apple HealthKit |
| **Modèle économique** | Gratuit (SDK natif) |
| **Coût estimé** | Apple Dev ~99€/an |
...
- **Langage** : Swift 5.9+
- **Frameworks** : `SwiftUI`, `Combine`, `HealthKit`, `CoreData (cache local chiffré)`
- **Architecture** : MVVM + Coordinator, dépendances injectées via Swinject ou similaire

### APIs et capacités spécifiques
- `BGTaskScheduler` pour synchronisation différée en arrière-plan

### Permissions à déclarer (`Info.plist`)
- `NSHealthShareUsageDescription` / `NSHealthUpdateUsageDescription`

### Comportement utilisateur
La fonctionnalité « Sync Apple HealthKit » est exposée selon les conventions iOS :
...
- **Langage** : Kotlin 1.9+
- **Dépendances** : `Jetpack Compose`, `Coroutines + Flow`, `Health Connect (Android 14+) + Google Fit fallback`, `Room (cache chiffré via SQLCipher) + WorkManager pour sync différée`
- **Architecture** : MVVM + Hilt (DI) + Repository pattern

### APIs et capacités spécifiques
- Synchronisation différée via `WorkManager` (backoff exponentiel)
...
### Comportement utilisateur
La fonctionnalité « Sync Apple HealthKit » suit les Material Design 3 guidelines :
...
### APIs web utilisées
- **Service Worker** + IndexedDB (chiffré via lib `crypto-js` ou subtle crypto)
- Background Sync API (Chrome/Edge uniquement)
...
### Comportement utilisateur
La version web de « Sync Apple HealthKit » est optimisée pour :
...
Quand il accède à la fonctionnalité « Sync Apple HealthKit »
...
```gherkin
Étant donné Le service « Apple HealthKit » est indisponible
...
- **RM-6 : Aucun secret du fournisseur « Apple HealthKit » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **Apple HealthKit** (à valider en kick-off)
