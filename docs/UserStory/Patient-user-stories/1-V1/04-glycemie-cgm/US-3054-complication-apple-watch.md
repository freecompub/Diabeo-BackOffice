# US-3054 — Complication Apple Watch
...
|---|---|
| **ID** | `US-3054` |
| **Référence inventaire** | `FNP-054` |
| **Domaine** | 4. Glycémie & CGM |
| **Priorité** | **V1** |
| **Plateforme cible** | 📱 |
| **Intégration externe** | Oui |
| **Service / Standard** | Apple WatchKit / WidgetKit |
...
- **Langage** : Swift 5.9+
- **Frameworks** : `SwiftUI`, `Combine`, `CoreBluetooth`, `WidgetKit`, `WatchKit + watchOS app companion`
...
La fonctionnalité « Complication Apple Watch » est exposée selon les conventions iOS :
...
- **Langage** : Kotlin 1.9+
- **Dépendances** : `Jetpack Compose`, `Coroutines + Flow`, `Nordic BLE Library (recommandé) ou Android BluetoothLE`, `Jetpack Glance (composables widgets)`, `Wear OS Compose`
...
### Comportement utilisateur
La fonctionnalité « Complication Apple Watch » suit les Material Design 3 guidelines :
...
### Comportement utilisateur
La version web de « Complication Apple Watch » est optimisée pour :
...
- **Notifications doublées** : toute alerte critique (Complication Apple Watch) est envoyée simultanément au parent administrateur ET au parent secondaire
...
Quand il accède à la fonctionnalité « Complication Apple Watch »
...
```gherkin
Étant donné Le service « Apple WatchKit / WidgetKit » est indisponible
...
Quand « Complication Apple Watch » s'exécute pendant la journée de jeûne
Alors le comportement est adapté (seuils prudents, pas de bolus repas, conseils Iftar/Sahur contextuels)
```

### AC-7 — Données chiffrées en local et en transit
...
- **RM-6 : Aucun secret du fournisseur « Apple WatchKit / WidgetKit » n'est embarqué dans les binaires mobiles ni exposé côté client web. Tous les appels passent par le backend Diabeo.**
...
- Documentation officielle **Apple WatchKit / WidgetKit** (à valider en kick-off)
