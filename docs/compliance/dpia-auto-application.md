...
| **Dérive de base : acceptation d'une proposition d'ensemble périmée (C3d)** — *risque résiduel temporaire, mitigation UI* | Une `SlotSetProposal` pending est un **cliché figé du jeu complet**. Tant que les **routes d'écriture par-créneau DOCTOR** subsistent (voir cadrage C3b §« grouped-only non appliqué »), un médecin peut baisser *un* créneau (ex. post-hypo) **sans** superséder la proposition pending ; l'accepter ensuite **écrase le jeu** et peut ré-introduire l'insuline coupée. La re-validation à l'acceptation (bornes/couverture, frontière MDR `nonInsulin`) est faite mais ne détecte PAS la **péremption sémantique**. **Mitigation actuelle** : écran `/patients/[id]/review` (écart actuel-vs-proposé visible au médecin) + acceptation atomique fail-closed. **Fermeture prévue** : slice dédiée post-C3d retirant la surface HTTP par-créneau (le `PUT` groupé `replaceSlotSet` supersède déjà). À traiter avant activation production. |
