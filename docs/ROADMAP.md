...
|----|-------|-----------|-----------|--------|--------|
| US-2645 | **EPIC** — Édition insulinothérapie multi-mode (fiche + self-service, propose→valide) | front/back/migration | — | XL | ✅ QUASI BOUCLÉ — 2646→2653 livrés ; **auto-application retirée** (ADR #28, #714 — toute édition patient = proposition médecin) ; **dé-escalade active multi-levier livrée** (#717). Reliquats non bloquants : flag mode-c `observance`, activation cron générateur prod (+ DPO) |
...
| US-2651 | Algorithme d'ajustement multi-mode (basal-bolus / doses fixes / non insuliné) | back | 2646, 2647 | L | ✅ DONE — **générateur multi-levier de bout en bout** : ICR + basal + ISF + fixedDose + dé-escalade + **cron**, doctor-gated (#666→#687) ; reste le flag mode-c `observance` |
