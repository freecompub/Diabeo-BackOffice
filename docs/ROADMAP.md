...
| [US-2663](../UserStory/insulinotherapie-edition/US-2663-EPIC-proposition-groupee-integrale.md) | **ÉPIC** — proposition GROUPÉE intégrale (grouped-only pour tous, algorithme compris ; disposition entière + surlignage des lignes modifiées ; retrait par-valeur) | back/front/iOS | 2657, 2645→2662 | XL | 🟡 **DESIGN VALIDÉ** (architect + medical + swift, 2026-07-12). Constats : voie groupée actuelle **moins sûre** (pas de CAS) ; anti-cliquet couplé à `AdjustmentProposal.accepted`. Slices S0 modèle+baseline → S1 CAS par-créneau + re-source cooldown → S2 revue unifiée → S3 moteur groupé → S4 voie manuelle → S5 retrait par-valeur + iOS. **Non démarré** (on avance par étapes). |
