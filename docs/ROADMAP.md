...
> **Mise à jour 2026-07-07 — 🆕 EPIC US-2654 Édition des créneaux horaires & autonomie graduée (spécification, V1)** : nouvel épic issu du constat qu'aujourd'hui **personne** (patient/infirmier/médecin) ne peut changer les **heures** d'un créneau ISF/ICR (seulement la valeur), + 2 bugs latents (IDOR `deleteIsf/deleteIcr`, UI menteuse de la page autonome). Modèle retenu : **fenêtre « tous les créneaux » → enregistrement de groupe transactionnel** (cohérence stricte re-validée serveur : chevauchement bloqué, trou ISF/ICR bloqué au save, trou basal = avertissement), **une seule proposition à la fois** (alerte, tue la collision P2002), **autonomie graduée par maturité** (junior=valeurs / intermédiaire=+créneaux / expert=+refuser/contre-proposer/**auto-application gouvernée** OFF par défaut, enveloppe de sécurité + DPIA), et **génération à la demande** [2 j–14 j] par médecin/infirmier. **5 US (V1)** : [US-2654 EPIC](../UserStory/insulinotherapie-edition/US-2654-EPIC-edition-creneaux-autonomie-graduee.md) · [US-2655](../UserStory/insulinotherapie-edition/US-2655-socle-serveur-groupe.md) socle serveur groupe · [US-2656](../UserStory/insulinotherapie-edition/US-2656-fenetre-edition-creneaux.md) fenêtre d'édition · [US-2657](../UserStory/insulinotherapie-edition/US-2657-maturite-autonomie-graduee.md) maturité + auto-application gouvernée · [US-2658](../UserStory/insulinotherapie-edition/US-2658-generation-a-la-demande.md) génération à la demande. Chaque US : **design validé medical + architecture avant code**. **Report V3** : *check d'adhérence & profilage patient* (comparer doses réellement utilisées vs constantes ; profiler → informer la maturité) — **prérequis captation de données** (aujourd'hui on capte `recommendedDose` + `wasDelivered` booléen, pas le montant délivré) + **DPIA profilage RGPD Art. 22** (annexe US-2658). *Spécification design-stage — SP à estimer.*
...
|----|-------|-----------|-----------|--------|--------|
| US-2645 | **EPIC** — Édition insulinothérapie multi-mode (fiche + self-service, propose→valide) | front/back/migration | — | XL | 🟡 EN COURS — majorité livrée (2646→2651) ; restent durcissement transverse (2652) + dé-escalade multi-levier (2653) |
...
| US-2652 | Garde-fous cliniques + i18n/acronymes + design-system + tests + docs | transverse | 2646→2651 | M | 🟡 EN COURS — durcissement fait **incrémentalement à chaque PR** (tests, design-system, i18n/glossaire, catalogue clinique, JSDoc) + persistance/assemblage/générateur **dose fixe** (#685→#687) ; gate finale formelle à acter |
| US-2653 | Propositions de **dé-escalade sur hypos récurrentes** (nadir-trigger, moins d'insuline) transverse aux 4 analyseurs, indépendant du deadband sur la moyenne | back | 2651 | M/L | 🟡 EN COURS — **dé-escalade ICR livrée** (prédicat/builder #673, matrice + flag `highVariabilityPostMeal` #674) ; basal/ISF/fixedDose ont la **garde hypo** (suppression de hausse) mais pas encore la dé-escalade active |
