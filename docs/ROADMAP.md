...
> **Mise à jour 2026-07-11 — US-2659 LIVRÉ (titration basale STYLO/MDI + baisse patient gatée, S0–S3)** : comble le trou « basale stylo non titrée » (l'algorithme titrait uniquement la pompe) et lève l'asymétrie « baisse patient interdite ». **S0** socle schéma (`AdjustmentProposal.basalDoseKind`, index unique partiel, CHECK d'exclusivité) → **S1** `single_injection` (treat-to-target à jeun, **hold zone asymétrique** obligatoire pour un pas fixe, dé-escalade `−min(20 %, 4 U)`, garde BGM/AC-4, Somogyi) → **S2** `split_injection` (dose soir/à jeun + matin/pré-dîner, garde de jour D9 « jamais croisé », confondeur bolus-midi flag-only, **une dose/run** priorité sécurité, verrou « 1 basale stylo pending ») → **S3** **baisse basale patient gatée** (relâche `patientDecreaseForbidden` : pompe dès INTERMEDIATE / stylo CONFIRME + **accusé DKA** immuable ; mode dérivé serveur anti-tamper ; l'écran de revue médecin surface les flags Somogyi ouverts avant décision). Jamais auto-appliqué (ADR #13/#29) ; application groupée `dailyDose` **différée** (accept stylo `styloBasalApplyNotSupported`). Chaque slice **validate-first** + revues renforcées (medical/HDS/code/prisma/swift) montrées avant correction. PRs #723→#726. **Follow-ups ticketés** : [US-2660](../UserStory/insulinotherapie-edition/US-2660-application-groupee-dose-basale-stylo.md) écriture groupée `dailyDose` à l'acceptation (HAUTE), [US-2661](../UserStory/insulinotherapie-edition/US-2661-flag-predinner-dedie-lien-proposition.md) flag pré-dîner dédié + lien Somogyi, [US-2662](../UserStory/insulinotherapie-edition/US-2662-affinements-titration-basale-mdi.md) affinements (câblage `MDI_BASAL_WARN_U`, dégludec 96 h V2, refacto DRY).

> **Mise à jour 2026-07-10 — ÉPIC US-2645 CLÔTURÉ (code complet)** : vérification de complétude — le flag mode-c `observance` était en réalité **déjà livré + testé** (note de reste périmée) ; le générateur couvre les 3 modes (basalBolus/fixedDose/nonInsulin) avec dé-escalade active des 4 leviers, sans stub ni TODO. Seul reste **hors périmètre code** : l'activation du cron générateur en production = gate **ops/DPO** (signature DPIA + flag `PROPOSAL_CRON_ENABLED`, runbook `cron-proposal-generator.md` §0). US-2645 passe **✅ DONE**.

> **Mise à jour 2026-07-10 — US-2653 dé-escalade active LIVRÉE (#717), épic US-2645 quasi bouclé** : dé-escalade des hypos récurrentes désormais active sur les **4 leviers** (ICR + ISF + basal + fixedDose) — matrice deadband × nadir récurrent par levier, **cooldown anti-cliquet 72 h**, **Somogyi → flag de revue** (baisse basale directe rejetée, décision medical : le phénomène de l'aube domine), 3 flags contextuels + surfaçage sévère isolé / non-actionnable (jamais de silence sur une hypo récurrente). Épic US-2645 : 2646→2653 livrés ; reliquats non bloquants = flag mode-c `observance`, activation cron générateur prod (+ DPO). **[périmé — épic clôturé, cf. bannière du haut ; `observance` était déjà livré, seule l'activation cron prod reste = gate ops/DPO]**

> **Mise à jour 2026-07-10 — Rafraîchissement statuts épic US-2645** : **US-2652** (garde-fous cliniques + i18n/acronymes + design-system + tests + docs) passe **✅ DONE** — gate transverse finale actée (cap patient min(%,abs) + rate-limit + audit `accessDenied`, #713). **Auto-application retirée** (#714, ADR #28). Reste sur l'épic : **US-2653** dé-escalade active basal/ISF/fixedDose + reliquats (flag mode-c `observance`, activation cron prod). **[périmé — US-2653 livré #717, `observance` déjà livré ; épic clôturé, cf. bannière du haut]**

> **Mise à jour 2026-07-10 — US-2654 EPIC Édition des créneaux horaires & autonomie graduée (partiellement livrée)** : fenêtre d'édition groupée + socle serveur + niveaux de maturité (JUNIOR/INTERMEDIATE/CONFIRME) + propositions **actifs**. **L'auto-application gouvernée (enveloppe C1–C8, slice C, DPIA) a été retirée** (2026-07-10) : toute édition patient génère désormais une **proposition groupée** `SlotSetProposal` validée par un médecin. **Reste livré** : [US-2655](../UserStory/insulinotherapie-edition/US-2655-socle-serveur-groupe.md) socle serveur groupe ✅ · [US-2656](../UserStory/insulinotherapie-edition/US-2656-fenetre-edition-creneaux.md) fenêtre d'édition ✅ · [US-2657](../UserStory/insulinotherapie-edition/US-2657-maturite-autonomie-graduee.md) maturité ✅ · [US-2658](../UserStory/insulinotherapie-edition/US-2658-generation-a-la-demande.md) génération à la demande ✅. Hors périmètre : auto-application (retirée), check adhérence & profilage patient (V3).

> **Mise à jour 2026-07-07 — ✅ Générateur de propositions MULTI-LEVIER complet (épic US-2645)** : le moteur de titration couvre désormais **les 4 leviers de bout en bout** — **ICR** (par créneau + dé-escalade hypos récurrentes), **basal** (pompe, créneau nocturne, garde Somogyi, snapping 0,05 U/h), **ISF** (corrections propres appariées, garde nadir tardif) et **fixedDose** (« doses simples » par moment, shift Option B, discriminateur `moment`) — plus le **cron** `generateForAllPatients` (advisory lock, audit run-level) et le mode **nonInsulin** (flags d'orientation, frontière MDR). Aucune proposition n'est auto-appliquée (`pending`, doctor-gated, ADR #13) ; jeux d'événements **disjoints** (pas de double-titration). Chaque tranche : **validate-first** (medical valide le DESIGN avant code) + **revue multi-agents** (code + medical + HDS/prisma selon le risque) montrée avant correction + merge sur GO explicite. Statuts US-2648→2651 corrigés ci-dessous (livrés). Reste : mode-c `observance`, activation cron prod (+ DPO), dé-escalade active basal/ISF/fixedDose (US-2653). **[périmé — `observance` + US-2653 livrés ; épic clôturé, cf. bannière du haut ; seule l'activation cron prod reste = gate ops/DPO]**
...
| [US-2018b](../UserStory/pro-user-stories/02-patients/US-2018b-consultation-patient-overlay-ephemere.md) | Consultation patient en overlay éphémère (sélection patient → vues patient-centrées, jeton serveur non partageable, aucun id dans l'URL) | 🆕 À démarrer (V1, 8 SP) | Cible : `consultation/open|close` + `PatientConsultationDrawer` + `/analytics` via jeton |
...
## Série Navigation & Accès Backoffice (cadrage — design-stage, à renuméroter)

> Source : branche `docs/nav-mockups` · maquette `docs/mockups/navigation.html` · US dans `docs/UserStory/Navigation/`.
> **Numérotation provisoire** (US-NAV-BO / US-ACCESS / US-SYSADMIN / US-TECH-SEC / US-BILLING) — à intégrer au corpus US-2xxx + estimer en SP avant planification. **Non comptées dans le tableau de stats global.**
...
| US-2624 | Pont **Dossier ↔ Consultation** (lanceur de revue) — 🚧 rétablit la distinction du mockup : bouton « ▶ Nouvelle consultation » dans `PatientContextBar` du dossier → `/patients/[id]/review` (la route `/review` était orpheline) ; retour « ‹ Au dossier » depuis la consultation. Tout soignant ayant accès au patient (décision étape 5 = DOCTOR-only inchangée), a11y + i18n FR/EN/AR + RTL. |
| US-2606 | Bloc « Gestion cabinet » dans la sidebar (Variante A) — ✅ DONE (PR5 : bloc « — GESTION — » gated Q2 [`canManageOrg` résolu serveur, absent du DOM sinon], 4 items [Équipe & droits · Facturation · Paiements · Paramètres] vers routes cabinet-agnostiques résolvant le scope managé [1 → direct, N → sélecteur], pages billing/payments [lecture `invoiceService.listByCabinet`, sans PHI] + settings [réutilise `CabinetDetailClient`], FR/EN/AR + RTL). ⚠️ V1 : mutations settings encore gated « manager légal `managerId` / ADMIN » (réconciliation Q2 = follow-up). |
...
| US-2613 | Admin plateforme Diabeo (établissements, personnel, bootstrap, politique vérif) ⚠️ garantie « sans PHI » effective en V4 (dép. F1) — 🚧 PR6a backend : `tenant.service` (CRUD + rattachement établissement), `verification-policy.service` (set/list fail-secure : tenant XOR pays, provisional borné, interdit prod sans flag), `ps-registration.service` (validation manuelle minimale verified/rejected ; enum `rejected` ajouté), `platform-admin.service` (bootstrap 1ᵉʳ org-admin + vue personnel cross-tenant + révocation incident), routes `/api/admin/{tenants,verification-policies,ps-registrations,platform/*}` gardées ADMIN, audit canonique, DPIA. **PR6b-1 UI « Structure »** : hub `/admin/platform` + entrée depuis le dashboard admin, organisations (`/admin/tenants` liste/création + détail édition/rattachement établissement), bootstrap (`/admin/platform/bootstrap` : sélection établissement → invitation 1ᵉʳ admin), FR/EN/AR + RTL. **PR6b-2 UI « Gouvernance »** : politique de vérification (`/admin/platform/verification-policies` : liste + pose tenant/pays × requis/provisoire borné), validation des preuves PS (`/admin/platform/ps-registrations` : liste en attente → valider/refuser), personnel cross-tenant (`/admin/platform/personnel` : recherche nom exact → appartenances/capacités → révocation incident), FR/EN/AR + RTL. ✅ **US-2613 DONE — clôt le bloc V1 « Navigation & Accès »**. |
...
| US-2625 | Home médecin « Ma journée » — alignement maquette Home v3 (triage-first) + **TIR par patient** sur les alertes (pathology-aware GD/adulte via `getCgmDefaults`, plancher de suffisance `cgmCaptureRate ≥ 30 %`, bi-palier <50 % « TIR bas » / 50–70 % « sous-cible »). 3 primitives partagées (DashboardCardHeader/Row/Pill), pills `feedback-*`/`pathology-*` (tokens `-fg` WCAG AA), sous-titre triage (`triageSummaryQuery` count-only, 1 audit + contexte réseau). Affine US-2602. Revue multi-agents (code/HDS/médical/a11y) = GO. 🚧 PR (suivi : US-2626/2627/2628). |
| US-2626 | **[BUG]** RDV « du jour » : `Appointment.hour` (`@db.Time`) traité comme instant ancré 1970 → badge « imminent » mort + heure affichée décalée de +1/+2 h. Préexistant (révélé par la revue US-2625, non aggravé). Recomposer l'instant `date`+`hour` ; corriger le test format-hour qui fige le bug. |
...
| US-2627 | **[TECH-DEBT]** Unifier le seuil de capture TIR : `population-analytics` importe `DASHBOARD_TIR.MIN_CAPTURE_RATE` (clinical-bounds) au lieu d'un `const` local dupliqué (valeur 30 % identique — anti-dérive). |
| US-2628 | **[TECH-DEBT]** Borner le dénominateur du TIR par patient (et KPI cabinet) à la plage CGM physiologique `CGM_AGGREGATE_RANGE_GL` (0,20–6,00 g/L), comme `analytics`/`population-analytics`. Impact faible (CHECK base déjà en place), cohérence inter-services. |
...
>
> Suites post-epic : `2642` (unification ouverture, liste → page) → suivis `2643` (décommission serveur `cTok`) + `2644` (durcissement a11y/sécurité `/patients/[id]`).
>
...
| US-2649 | Flux proposition → validation (provenance, notif, revue médecin, compare-and-swap) | front/back | 2646 | L | ✅ DONE (US-2649a/b, #652→#656) |
| US-2650 | Self-service patient : route `(patient)` + nav + lecture + proposer (cooldown anti-churn) | front/back | 2648, 2649 | L | ✅ DONE (#657→#660) |
| US-2651 | Algorithme d'ajustement multi-mode (basal-bolus / doses fixes / non insuliné) | back | 2646, 2647 | L | ✅ DONE — **générateur multi-levier de bout en bout** : ICR + basal + ISF + fixedDose + dé-escalade + **cron**, doctor-gated (#666→#687). Flag mode-c `observance` **livré** (garde enrollment + either/or CGM-capture/BGM, testé) |
| US-2652 | Garde-fous cliniques + i18n/acronymes + design-system + tests + docs | transverse | 2646→2651 | M | ✅ DONE — durcissement transverse fait **incrémentalement à chaque PR** (tests, design-system, i18n/glossaire, catalogue clinique, JSDoc) ; **gate finale formelle actée** (cap patient min(%,abs) + rate-limit + audit `accessDenied`, #713) |
| US-2653 | Propositions de **dé-escalade sur hypos récurrentes** (nadir-trigger, moins d'insuline) transverse aux 4 analyseurs, indépendant du deadband sur la moyenne | back | 2651 | M/L | ✅ DONE — dé-escalade active sur les **4 leviers** (ICR #673/#674, puis ISF/basal/fixedDose #717). Matrice deadband × nadir récurrent par levier, **cooldown anti-cliquet 72 h**, **Somogyi → flag** (baisse basale directe rejetée, medical), 3 flags contextuels + surfaçage sévère isolé / non-actionnable |
| [US-2659](../UserStory/insulinotherapie-edition/US-2659-titration-basale-mdi-et-baisse-patient.md) | Titration basale **stylo (MDI)** (single/split, treat-to-target à jeun) + **baisse basale proposable par le patient** selon le mode de délivrance | back/migration/iOS | 2651, 2650, 2657 | L | ✅ **LIVRÉ** (S0–S3, PRs #723→#726). **S0** socle (`basalDoseKind`, index, CHECK exclusivité) · **S1** single_injection (hold zone asymétrique, dé-escalade `−min`, garde BGM/AC-4, Somogyi) · **S2** split_injection (soir=à jeun/matin=pré-dîner, garde jour D9, confondeur bolus-midi, une-dose/run, verrou 1-pending) · **S3** baisse patient gatée (pompe INTERMEDIATE / stylo CONFIRME + accusé DKA immuable, mode serveur anti-tamper, flags Somogyi à la revue). Constantes `MDI_BASAL_*`. Validate-first + revues renforcées. Application groupée `dailyDose` différée (accept stylo `styloBasalApplyNotSupported`). **Contrat iOS** (inputs + codes) → `swift-expert`. |
| [US-2660](../UserStory/insulinotherapie-edition/US-2660-application-groupee-dose-basale-stylo.md) | **Application groupée** de la dose basale **stylo** (MDI) à l'acceptation médecin (écriture `dailyDose`/`morningDose`/`eveningDose`, compare-and-swap + fail-closed) | back/iOS | 2659 | M | ✅ **LIVRÉ** (ADR #30) — `accept(applyImmediately)` écrit la dose ciblée par `basalDoseKind` sur l'unique `BasalConfiguration` du patient (scopé patient). Gardes fail-closed `baselineMoved` (dérive) + `styloBasalNotFound` (dose effacée, `WHERE not-null`). `styloBasalApplyNotSupported` supprimé. **Contrat iOS** (accept stylo → `applied:true`) → `swift-expert`. |
| [US-2661](../UserStory/insulinotherapie-edition/US-2661-flag-predinner-dedie-lien-proposition.md) | Flag de revue **dédié « pré-dîner »** (basale stylo matin, enum+i18n FR/EN/AR) + lien flag Somogyi ↔ proposition à la revue | back/i18n/front | 2659 | S | ✅ **LIVRÉ** — flag `daytimeHypoHighPreDinner` (migration `20260723100000`) : la dose du MATIN du split (signal pré-dîner/diurne) lève un flag distinct ; soir/single/pompe gardent `nocturnalHypoHighFasting`. Libellé **non affirmatif** (medical : confondeur bolus-midi ≠ hypo garantie). Design **validé medical avant code**. i18n×3, `Record` exhaustif, doc catalogue, tests découplage. AC-4 lien = surface `listOpen` existante (MVP). |
| [US-2662](../UserStory/insulinotherapie-edition/US-2662-affinements-titration-basale-mdi.md) | Affinements titration basale MDI (groupé) : câblage `MDI_BASAL_WARN_U`, cooldown **dégludec 96 h** (V2, dépend molécule), refacto DRY générateur | back | 2659 | S | ✅ **LIVRÉ** — (1) badge WARN « dose élevée > 80 U » non bloquant à la revue (dérivé serveur, i18n×3) ; (2) **cooldown molécule** (`resolveMdiCooldownHours` : durée ≥ 30 h → 96 h dégludec/U300, sinon 72 h ; **fail-closed inconnu → 96 h**, design validé medical) sur les 3 cibles ; (3) DRY du cooldown (triplé → 1 resolver), refacto plus profonde différée (code critique-sécurité, gain interne). Suivi tracé : reset titration au changement de molécule. |
| [US-2663](../UserStory/insulinotherapie-edition/US-2663-EPIC-proposition-groupee-integrale.md) | **ÉPIC** — proposition GROUPÉE intégrale (grouped-only pour tous, algorithme compris ; disposition entière + surlignage des lignes modifiées ; retrait par-valeur) | back/front/iOS | 2657, 2645→2662 | XL | 🟡 **DESIGN VALIDÉ** (architect + medical + swift, 2026-07-12). Constats : voie groupée actuelle **moins sûre** (pas de CAS) ; anti-cliquet couplé à `AdjustmentProposal.accepted`. Slices S0 modèle+baseline → S1 CAS par-créneau + re-source cooldown → S2 revue unifiée → S3 moteur groupé → S4 voie manuelle → S5 retrait par-valeur + iOS. **S0 ✅ livré** (colonnes `source`+`baseline_slots`, module de typage, snapshot baseline). **S1 ✅ livré** (CAS d'ensemble fail-closed `assertBaselineUnchanged` sous verrou dans `replaceSlotSet` : `baselineMoved`/`baselineMissing` → 409 ; couvre la surface ISF/ICR existante ; stylo/dose-fixe + re-source anti-cliquet reportés en S3, diff-merge en S1bis). **S2 ✅ livré** (`SlotSetProposal` pending surfacée sur `/patients/[id]/review` : composant `GroupedProposalReview` — diff live→proposé surligné par créneau, bandeau `baselineDrifted`, badge structure, provenance, accept/reject ; helpers purs `slot-diff.ts` + `isBaselineUnchanged` ; `listPendingForReview` avec baseline ; i18n×3). **S2b ✅** (exclusion mutuelle grouped⇄par-valeur, raffinée par classe d'origine en S3b-0a). **S3a ✅** (re-source anti-cliquet : `lastAcceptedChangeAt` lit aussi les acceptations groupées, garde-fou #4). **S3b-0a ✅** (fondation bascule : coexistence humain⇄algorithme D2 — index relâché `..._origin`, 1 humain + 1 algo pending/param ; colonne `rationale` requise si `source=algorithm`, R3). **S3b-0b ✅** (front : `GroupedProposalReview` affiche la rationale moteur — motif `AdjustmentReason`/confiance/volume + direction de risque — et un bandeau de coexistence). **Reste S3b-1** (le moteur émet enfin le groupé ISF/ICR derrière flag, CAS persist par-créneau), S3c/d (basal/stylo/fixe), S4/S5. |
| [US-2664](../UserStory/insulinotherapie-edition/US-2664-vue-proposition-unifiee-divulgation-patient.md) | **Vue de proposition UNIFIÉE** (médecin/infirmière/patient, composant unique audience-aware) + **divulgation patient sécurisée** | front/back(durcissement) | 2649, 2650, 2652 | M | ✅ **LIVRÉE (PR #733)** — 1ʳᵉ étape pragmatique (sans grouped-only). Design **validé medical**, 3 revues **PRÊTE/GO/CONFORME**. Composant `ProposalList` unifié ; patient ne voit **que ses propres demandes** (`source=patient`, **filtre serveur** sur `list`+`summary`, helper `viewerProposalSources`), rendu sécurisé (bandeau « ne modifiez pas vos doses », sans badges clinicien). Médecin = décide ; infirmière = lecture. **Zéro rupture API.** Hors périmètre : nurse→patient (MDR/DPIA), infirmière corrige (provenance). |
| [US-2665](../UserStory/insulinotherapie-edition/US-2665-durcissement-provenance-proposal-ack.md) | Durcissement de provenance sur l'accusé/réponse de proposition (`proposal-ack`) | back | 2664, 2065 | XS | ✅ **LIVRÉE** — `viewerProposalSources` appliqué à la résolution d'ownership de `POST/PUT /api/team/proposal-ack/[proposalId]` : un VIEWER ne peut acquitter/répondre qu'à une proposition `source=patient` de son dossier ; tierce/inexistante → **404 uniforme** (non énumérant), pro → **403** inchangé. Test route `tests/unit/proposal-ack-route.test.ts` (7 cas). |
