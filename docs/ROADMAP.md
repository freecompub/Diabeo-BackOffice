...
| [US-2664](../UserStory/insulinotherapie-edition/US-2664-vue-proposition-unifiee-divulgation-patient.md) | **Vue de proposition UNIFIÉE** (médecin/infirmière/patient, composant unique audience-aware) + **divulgation patient sécurisée** | front/back(durcissement) | 2649, 2650, 2652 | M | 🟡 **EN COURS** — 1ʳᵉ étape pragmatique (sans grouped-only). Design **validé medical**. Patient ne voit **que ses propres demandes** (`source=patient`, **filtre serveur** imposé — fait+testé), rendu sécurisé (bandeau « ne modifiez pas vos doses », sans badges clinicien). Médecin = décide ; infirmière = lecture (dont patient). « 1 pending/créneau, premier arrivé » inchangé. **Zéro rupture API.** Hors périmètre : nurse→patient (MDR/DPIA), infirmière corrige (provenance). |
