...
| [US-2659](../UserStory/insulinotherapie-edition/US-2659-titration-basale-mdi-et-baisse-patient.md) | Titration basale **stylo (MDI)** (single/split, treat-to-target à jeun) + **baisse basale proposable par le patient** selon le mode de délivrance | back/migration/iOS | 2651, 2650, 2657 | L | 🟡 **SPÉCIFIÉE** — cadrage clinique validé (`medical-domain-validator` 2026-07-11). Comble le trou « basale stylo non titrée » + lève l'asymétrie « baisse patient interdite ». Slices S0 socle schéma (`basalDoseKind`) → S1 single → S2 split → S3 baisse patient. **Aucun code avant validation finale de l'US.** |
