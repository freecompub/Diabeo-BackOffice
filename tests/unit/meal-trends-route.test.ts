/**
 * Test — GET /api/analytics/meal-trends : gardes US-2637 (garde-fou épopée). Opt-out du
...
vi.mock("@/lib/services/meal-trends.service", () => ({ mealtimePattern: { mealTrends: vi.fn() } }))
...
import { GET } from "@/app/api/analytics/meal-trends/route"
...
const mTrends = vi.mocked(mealtimePattern.mealTrends)
...
beforeEach(() => {
  ...
  mTrends.mockResolvedValue([] as any)
})

describe("GET /api/analytics/meal-trends — gardes US-2637", () => {
  it("blocks a patient who opted out of provider sharing (before projection)", async () => {
    ...
    expect(mTrends).not.toHaveBeenCalled()
  })
  ...
  it("passes through when consent is granted", async () => {
    ...
    expect(mTrends).toHaveBeenCalledWith(42, "30d", 1, expect.any(Object), { source: "cgm" })
  })
