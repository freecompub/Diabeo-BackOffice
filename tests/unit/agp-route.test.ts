/**
 * Test — GET /api/analytics/agp : gardes US-2635 (garde-fou épopée). Opt-out du
...
vi.mock("@/lib/services/analytics.service", () => ({ analyticsService: { agp: vi.fn() } }))
...
import { GET } from "@/app/api/analytics/agp/route"
...
const mAgp = vi.mocked(analyticsService.agp)
...
beforeEach(() => {
  ...
  mAgp.mockResolvedValue([] as any)
})

describe("GET /api/analytics/agp — gardes US-2635", () => {
  it("blocks a patient who opted out of provider sharing (before projection)", async () => {
    ...
    expect(mAgp).not.toHaveBeenCalled()
  })
  ...
  it("passes through when consent is granted", async () => {
    ...
    expect(mAgp).toHaveBeenCalledWith(42, "30d", 1, expect.any(Object))
  })
