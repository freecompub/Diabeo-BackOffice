...
vi.mock("@/lib/services/analytics.service", () => ({ analyticsService: { bgmDailyPatternByMoment: vi.fn() } }))
...
import { GET } from "@/app/api/analytics/bgm-daily-pattern/route"
...
const mPat = vi.mocked(analyticsService.bgmDailyPatternByMoment)
...
describe("GET /api/analytics/bgm-daily-pattern — gardes US-2639", () => {
  it("blocks a patient who opted out of provider sharing (before projection)", async () => {
    ...
    expect(mPat).not.toHaveBeenCalled()
  })
  ...
  it("passes through when consent is granted", async () => {
    ...
    expect(mPat).toHaveBeenCalledWith(42, "30d", 1, expect.any(Object))
  })
