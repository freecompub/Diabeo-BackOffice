import { PrismaClient, Pathology, Role, Language, Sex, InsulinDeliveryMethod, BasalConfigType, GlucoseTargetPreset, DayMomentType } from "@prisma/client"
...
async function main() {
  ...
  const patientUserDT1 = await prisma.user.upsert({
    where: { email: "patient.dt1@diabeo.test" },
    update: {},
    create: {
  ...
  const patientUserDT2 = await prisma.user.upsert({
    where: { email: "patient.dt2@diabeo.test" },
    update: {},
    create: {
  ...
  console.log("Users created:", { admin: admin.id, doctor: doctor.id, nurse: nurse.id, patientDT1: patientUserDT1.id, patientDT2: patientUserDT2.id })
  ...
  await prisma.glucoseTarget.createMany({
    data: [
      { settingsId: settingsDT1.id, targetGlucose: 120, targetRangeLower: 0.70, targetRangeUpper: 1.80, preset: GlucoseTargetPreset.standard, isActive: true },
    ],
    skipDuplicates: true,
  })
  ...
  // ISF — 3 creneaux
  const t = (h: number, m: number) => new Date(`1970-01-01T${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:00Z`)
  ...
  // Basal configuration — pump with 4 slots
  const basalConfig = await prisma.basalConfiguration.upsert({
    where: { settingsId: settingsDT1.id },
    update: {},
    create: {
      settingsId: settingsDT1.id,
      configType: BasalConfigType.pump,
  ...
  await prisma.pumpBasalSlot.createMany({
    data: [
      { basalConfigId: basalConfig.id, startTime: new Date("2026-01-01T00:00:00Z"), endTime: new Date("2026-01-01T06:00:00Z"), rate: 0.65, durationHours: 6 },
      { basalConfigId: basalConfig.id, startTime: new Date("2026-01-01T06:00:00Z"), endTime: new Date("2026-01-01T12:00:00Z"), rate: 0.85, durationHours: 6 },
      { basalConfigId: basalConfig.id, startTime: new Date("2026-01-01T12:00:00Z"), endTime: new Date("2026-01-01T22:00:00Z"), rate: 0.75, durationHours: 10 },
      { basalConfigId: basalConfig.id, startTime: new Date("2026-01-01T22:00:00Z"), endTime: new Date("2026-01-02T00:00:00Z"), rate: 0.70, durationHours: 2 },
    ],
    skipDuplicates: true,
  })
  ...
  await prisma.glucoseTarget.createMany({
    data: [
      { settingsId: settingsDT2.id, targetGlucose: 130, targetRangeLower: 0.90, targetRangeUpper: 2.00, preset: GlucoseTargetPreset.elderly, isActive: true },
    ],
    skipDuplicates: true,
  })
  ...
  await prisma.basalConfiguration.upsert({
    ...
    update: {},
    create: {
      settingsId: settingsDT2.id,
      configType: BasalConfigType.single_injection,
      insulinBrand: "lantus",
      dailyDose: 22,
    },
  })
  ...
  // ─── 10. CGM data — 30 days for DT1 patient ──────────────

  const now = new Date()
  const cgmData: { patientId: number; valueGl: number; timestamp: Date; isManual: boolean }[] = []

  for (let day = 0; day < 30; day++) {
    for (let reading = 0; reading < 288; reading++) { // 5-min intervals
      const ts = new Date(now)
      ts.setDate(ts.getDate() - day)
      ts.setHours(0, reading * 5, 0, 0)

      // Simulate realistic glucose pattern: baseline 1.20 g/L with noise
      const hour = ts.getHours()
      let base = 1.20
      if (hour >= 7 && hour <= 9) base = 1.50 // post-breakfast spike
      if (hour >= 12 && hour <= 14) base = 1.40 // post-lunch
      if (hour >= 19 && hour <= 21) base = 1.45 // post-dinner
      if (hour >= 2 && hour <= 5) base = 1.00 // dawn dip
  ...
  for (const m of moments) {
    await prisma.userDayMoment.create({ data: m })
  }
