...
| 30 | **Application groupée de la dose basale STYLO à l'acceptation médecin** (US-2660) | Comble le follow-up d'US-2659 : l'accept-with-apply d'une proposition basale **stylo** n'écrivait rien (`styloBasalApplyNotSupported` fail-closed → le médecin ajustait la dose à la main). Désormais `accept(applyImmediately)` **écrit** la dose ciblée par `basalDoseKind` (`dailyDose`/`morningDose`/`eveningDose`, U totales) sur l'**unique** `BasalConfiguration` du patient (`settings_id @unique` + `patient_id @unique` ⇒ 1 config/patient ; `updateMany` scopé patient via `settings` touche ≤ 1 ligne — anti-IDOR, cohérent avec la lecture `resolveCurrentValue`). **Grouped-only** (ADR #23/#26) : pas de route par-valeur ressuscitée, l'écriture passe par le chemin d'acceptation. **Deux gardes fail-closed** (rollback transactionnel, jamais d'« accepté + appliqué » fantôme) : (a) compare-and-swap `baselineMoved` (409) si la dose live a dérivé depuis la proposition ; (b) `styloBasalNotFound` si le `WHERE` — qui exige la dose ciblée **NON NULLE** — matche 0 ligne, ce qui couvre le cas où le médecin a **effacé** la dose depuis (`liveCurrentValue` renvoie alors `null` → compare-and-swap inactif ; sans la garde `{ not: null }` on réintroduirait silencieusement une dose supprimée). **`styloBasalApplyNotSupported` supprimé**. **Contrat iOS → `swift-expert`** : l'acceptation d'une proposition stylo peut renvoyer `applied: true` (au lieu de l'ancien refus). |
...
*Dernière révision : 2026-07-11 — ADR #30 (US-2660 : application groupée de la dose basale stylo à l'acceptation médecin). Historique des PRs : `git log`.*