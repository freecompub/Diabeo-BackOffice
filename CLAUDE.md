...
*Dernière mise à jour : 2026-05-15 — Groupe 10 Batch E Food monitoring livré (PR #404, 4 US US-2248/2251/2253/2260, ~16 SP, plan simplifié b sans migration). Service `food-monitoring.service.ts` avec 3 sub-queries Promise.all : foodJournalQuery (DiabetesEvent eventTypes:{has: insulinMeal} + _count.mealPhotos + comment truncate 500c défensif + SECURITY note), adherenceQuery (score composite 0-100 = 0.6×regularité + 0.4×bolusCoverage, fallback regularity-only si totalMeals=0 — UI doit disambiguer via totalMeals, distinct days bucketés Intl.DateTimeFormat fr-CA Europe/Paris pour éviter skew 22:30 UTC→Paris+1j), glycemiaMealContextQuery (batch CGM fetch [earliest-2h, latest+2h] + bucketing in-memory pre/post par meal ; edge `t===mealMs` exclu déterministe ; perf O(meals×cgm) acceptable <50 patients, V1+ binary search). NON_DELETED_PATIENT filter sur 3 queries (L2 defense-in-depth). Audit unifié resource=DIABETES_EVENT (US-2268 forensique `getByPatient` cohérent). 3 routes `/api/patients/[id]/{food-journal,adherence,glycemia-meal-context}` NURSE+ + canAccessPatient + requireGdprConsent. US-2260 templates pathologie : `/api/team/templates` (US-2078) existe ; pathology column V2. 2 rounds review appliqués (9 findings : M1 Paris-tz, L1 truncate comment, L2 deletedAt, L3 JSDoc ambiguïté clinique, L4 perf TODO, L5 edge case t===meal, L6 7 tests boundaries, L7 drop unused re-export, L8 unify audit resource). V1 63 → 67 DONE (48%). Total 135/292 (46%). 1657/1657 tests verts. ⚠️ V2 deferrals in-code : US-2250 workflow validation glucides FSM (nouvelle table), US-2252 cron alerte non-saisie + OrchestrationLog (nouvelle table idempotence), US-2260 pathology field MessageTemplate, Redis cache adherence/context >50 patients, UI patient detail tabs intégration (journal + adherence + context), medical-domain-validator review pré-MVP sur weights 0.6/0.4.*
