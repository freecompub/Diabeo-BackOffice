...
*Dernière mise à jour : 2026-05-14 — Groupe 9b Batch 3 Dashboard administrateur livré (PR #403, 3 US US-2410/2412/2415, ~19 SP, plan simplifié pas de nouvelle migration). Root `/dashboard` redirect désormais complet : DOCTOR → /medecin, NURSE → /infirmier, **ADMIN → /admin** (split étendu), VIEWER → /patient/dashboard (layout). Page `(dashboard)/admin` server-component + 3 cards : AdminKpiSection (Promise.all 4 metrics : cabinets total, staff via User.status=active 2-step query, patients actifs CGM 14j via raw SQL COUNT DISTINCT, audit events 7j, polling 5min), BillingCard heuristique (TeleconsultationActe.invoicedAt IS NULL avec completedFilter chained sur toutes les counts incl. recentlyBilled, label "Montant non facturé (arrondi)" + aria-live="polite", polling 10min), ComplianceCard (lastBackup avec filter completedAt: not null pour éviter Postgres NULLS FIRST, audit 24h, failed backups 30d, STALE_BACKUP_DAYS=2 + Math.max(0,…) clock-skew guard, polling 5min). US-2415 Sidebar couvert par NavigationShell existant (minRole ADMIN gate sur /users + /audit déjà en place). 2 rounds reviews appliqués (11 findings : H1 filter consistency recentlyBilled, H2 raw SQL COUNT DISTINCT scale, H3 4 tests RBAC + edge cases, M1 NULLS FIRST, M2 staff via User.status=active, M3 backupAgeDays clamp + STALE constant, M4 label arrondi, L1 date constants extracted, L2 aria-live, L3 redirect `/` au lieu de /login, L4 doc auditedRequireRole). V1 60 → 63 DONE (45%). Total 131/292 (45%). 1642/1642 tests verts. ⚠️ V2 deferrals documentés in-code : `Invoice` table US-2107 (US-2412 partie 2), KPI activité cabinet US-2411 (V3 deps US-2150/US-2200), Conformité RGPD US-2413 (V3 deps US-2190/2191/2192), multi-cabinet switcher + sidebar badges count US-2415 v2.*
