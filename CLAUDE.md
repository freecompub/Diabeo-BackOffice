...
*Dernière mise à jour : 2026-05-14 — Groupe 9b Batch 2 Dashboard infirmier livré (PR #401, 5 US US-2405/2406/2407/2408/2409, ~31 SP, plan simplifié pas de nouvelle migration). Root `/dashboard` redirect désormais split : DOCTOR/ADMIN → /medecin, NURSE → /infirmier, VIEWER → /patient/dashboard (layout). Page `(dashboard)/infirmier` server-component + 4 cards : NurseKpiSection (Promise.all 4 metrics : RDV à préparer, événements à valider, urgences observées, propositions à connaître, polling 60s), TodoListCard READ-ONLY (3 sources merge — Appointment+DiabetesEvent+AdjustmentProposal pending — score `100 - minutesUntilAppt/14.4` clamped, polling 60s), TeamInboxCard (DelegationRequest in/out cabinet-scoped via getAccessiblePatientIds, exclu expired, polling 60s), RecallListCard heuristique (silentMonitoring CGM >7j + apptUnconfirmed >3j + neverSynced distinct du jamais-synced, boutons tel:/sms: URI avec aria-label patient, polling 120s). Service `nurse-dashboard.service.ts` avec patientScopeWhere réutilisé. **todayBounds fix DST-aware** Paris→UTC midnight (Intl longOffset embed dans ISO literal) appliqué aussi sur doctor-dashboard.service.ts (corrige bug PR #399). 2 rounds reviews appliqués (13 findings : H1 cabinet scope DelegationRequest, H3 timezone, M1 imminent scoring, M2 neverSynced, M3 wasTruncated audit, M5 mapErrorToResponse doc, L3/L4 a11y aria-labels). V1 55 → 60 DONE (43%). Total 128/292 (44%). 1633/1633 tests verts. ⚠️ V2 deferrals documentés in-code : `NurseTaskItem` (US-2407 checkbox completion + 30s undo + FCM notification doctor), `TeamMessage` (US-2408 libre chat équipe), `PatientRecallLog` + Twilio SMS server-side (US-2409 partie 2), intégration algorithme risque US-2800 (V4).*
