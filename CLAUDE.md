...
*Dernière mise à jour : 2026-05-15 — Groupe 7 Batch 1 Facturation Foundation livré (PR #406, 3 US US-2103/2105/2107, ~11 SP, plan A). Migration `20260515100000_groupe7_batch1_invoice_foundation` : 3 tables (`invoices`, `invoice_items`, `invoice_sequences`) + 2 enums (`InvoiceStatus` FSM `draft→issued→paid→refunded`, cancelled terminal ; `PaymentMethod`) + 3 colonnes `HealthcareService.{siret,tvaIntra,iban}` + 11 CHECK constraints + 3 triggers PG (`enforce_invoice_immutability` post-draft + FSM DB-level + lock number/issued_at/snapshots, `block_invoice_delete_after_issued`, `block_invoice_items_after_issued`). 2 services : `invoice-numbering.service.ts` (`reserveNextInvoiceNumber` gap-less via INSERT…ON CONFLICT + SELECT FOR UPDATE + UPDATE + runtime guard `pg_current_xact_id_if_assigned()` anti tx-skip, format `FR-2026-000001`, conformité DGFiP art. 242 nonies A CGI) et `invoice.service.ts` (createDraft/issue/markPaid/cancel/getById/listByCabinet/listByPatient + `atomicTransition` via `updateMany WHERE status` + `InvoiceConcurrencyError` 409 retryable distinct de `InvoiceStateError` invalidTransition). Snapshots cabinet (SIRET Luhn validé via `validateSiret` exporté de healthcare-management.service) et **customer chiffré AES-256-GCM** `{ patientRef, encryptedPii: base64, encryptedAt }` + Zod `.strict()` whitelist double-couche encode+decode (anti-leak Batch 2 PDF) — defense-in-depth restaurée symétrie users.firstname. 5 routes `/api/billing/invoices` + `[id]/{issue,pay,cancel}` (RBAC `assertCabinetMember` defense-in-depth + VIEWER own patient + `assertJsonContentType()` 415 REST hygiene). Helper `mockInvoiceTxGuard()` partagé tests/helpers/prisma-mock.ts. **3 rounds review** (code-reviewer) appliqués : 58 findings (36 initiaux + 15 re-review-2 + 7 re-review-3) — 0 résiduel Critical/High. AuditResource enum +1 : `INVOICE`. Audit `kind:` typé via `InvoiceAuditKind` union + `AUDIT_KIND` const satisfies. `STRIPE_PAYMENT_INTENT_ID_REGEX` constante partagée route/service. V1 71 → 74 DONE (52%). Total 139/292 → 142/292 (49%). 1736/1736 tests verts. ⚠️ V2 follow-ups : Stripe webhooks idempotents (US-2106 Batch 3 bloqué provision Stripe Connect), PDF facture+IBAN (US-2102 Batch 2 — `decryptCustomerSnapshot` exporté pour consommation), TVA multi-pays via CountryTaxRule (US-2110 Batch 2), refunds (US-2109 Batch 4), relances cron J+7/15/30 (US-2108 Batch 4), GIN index `audit_logs.metadata->'cabinetId'` à ajouter en V1, validation Luhn cabinet admin UI (US-2118).*
