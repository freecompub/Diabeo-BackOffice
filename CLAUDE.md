...
*Dernière mise à jour : 2026-05-14 — Groupe 10 Mirror V1 Batches A+B livré (PR #395 : US-2218/2219/2220/2221/2227/2228/2229, ~30 SP). ConfigVersion hub append-only (DB trigger immuabilité + no-delete) + EmergencyContact (max 5, PHI AES-256-GCM, configSnapshot redacted) + EscalationRule (chain patient→contact→doctor→SAMU, targetId redacted) + AlertThresholdTemplate (bibliothèque cabinet 5 profils) + PatientMonitoringMetrics (cache trimestriel) + CohortAnalyticsSnapshot (KPIs vs benchmark national avec disclaimer) + PatientRiskScore (rule-based 3 facteurs, ack reset on recompute). Security : canAccessPatient + requireGdprConsent sur toutes routes PHI-touching, isOrgMember pour cabinet-scoped, audit per-patient pivot (US-2268). 1550/1550 tests verts. V1 43 → 50 DONE (35%). Total 268 US.*
