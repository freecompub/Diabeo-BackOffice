...
*Dernière mise à jour : 2026-05-14 — Groupe 9b Batch 1 Dashboard médecin livré (PR #399, 5 US US-2400/2401/2402/2403/2404, ~34 SP). Page server-component `/medecin` + role-based redirect `/dashboard` (DOCTOR/NURSE/ADMIN → /medecin, VIEWER déjà routé vers /patient/dashboard par layout). 4 cards : EmergencyCard polling 30s (two-pass critical+recent, dedup id, criticité ketone_dka > severe_hypo > ketone_moderate > hypo > severe_hyper > hyper > manual), AppointmentCard polling 5min (Europe/Paris bounds + formatHour, badge imminent <30min avec sr-only), PatientsAtRiskCard DOCTOR-only (on-demand : hypos ≥3/7j + silence ≥5j sans CGM, exclut urgences ouvertes, top 3 par score, ADMIN portfolio cap 1000), KpiSection 10min (Promise.all 8 queries, trend up/down/flat, TIR gates sur both windows > 0). Service `doctor-dashboard.service.ts` avec `patientScopeWhere` (3 états : empty/admin/scoped, deletedAt:null toujours appliqué incl. ADMIN — RGPD Art. 17). Hook `usePollingFetch` : isStale (data older than 2× intervalMs), visibilitychange listener (pause si tab caché), 401 module-level authDead flag (1 redirect /login peu importe le nb de cards). Audit per-patient pivot sur urgences + appointments (ADR #18 forensique CNIL), summary-only sur KPI (aggregate). StaleBanner shared avec icône Clock. 2 rounds reviews appliqués (29 findings : 0 Critical + 7 High + 10 Medium + 14 Low — incl. timezone Paris fix, two-pass urgences, Promise.all KPI). **MVP désormais 100%** (68/68). V1 53 → 55 DONE (39%). Total 123/292 (42%). 1618/1618 tests verts.*
