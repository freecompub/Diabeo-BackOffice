...
*Dernière mise à jour : 2026-05-14 — Groupe 9c patient-web Batch 1 livré (PR #394 : US-3356/3361/3362/3363, 32 SP). Dashboard patient self-service : `(patient)` route group + role-based redirect VIEWER→/patient/dashboard ; 24h CGM chart + 4 KPI (TIR, moy, CV, GMI estimée HbA1c) ; AGP 7d AgpPercentileChart (stacked Areas p10-p25-p50-p75-p90 + sr-only table WCAG AA) ; QuickActionsPanel scaffold (modals Batch 2). Auth patient déjà en place : VIEWER + JWT cookie + RBAC `resolvePatientIdFromQuery`. 1520/1520 tests verts. V1 39 → 43 DONE (31%). Total 268 US.*
