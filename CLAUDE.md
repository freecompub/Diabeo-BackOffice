...
| 22 | Autorité d'activation de l'auto-application = **gouvernance plateforme portée par `ADMIN`** (pas de rôle `GOVERNANCE` dédié) (US-2657) | La DPIA fait signer DPO + RSSI + responsable MDR — fonctions **plateforme (fabricant)**, pas cabinet. Le médecin (hôpital OU indépendant) est côté *demande*, jamais *octroi* → séparation des devoirs au niveau plateforme, sans garde « quatre yeux » locale (impraticable pour un libéral seul). Activation gatée `ADMIN` + `maturityLevel === EXPERT` + `dpiaRef` obligatoire (fail-closed) ; un downgrade de maturité remet `autoApply = false`. |
| 23 | `SlotSetProposal` = **proposition GROUPÉE** remplaçant la voie par-valeur `AdjustmentProposal` pour l'édition patient EXPERT (US-2657) | Une édition non auto-appliquée est proposée en bloc (jeu de créneaux entier), quel que soit le nombre de valeurs modifiées. `createSetProposal` supersède les propositions pending (d'ensemble ET par-valeur). Invariant « 1 pending / (patient × paramètre) » garanti par index unique partiel. |
| 24 | Kill-switch global `AUTO_APPLY_GLOBALLY_ENABLED` **fail-safe (OFF par défaut)** + triple verrou (US-2657) | L'auto-application n'est possible que si kill-switch global ON ∧ `Patient.autoApply` ∧ `GovernanceApproval` active (vérifiée à l'exécution). Défaut/malformé → OFF → voie proposition. Harnais livré **inerte** jusqu'au câblage C3b. |
...
| 26 | **Grouped-only : retrait des routes d'écriture par-créneau ISF/ICR/basal** (US-2657) | Applique pleinement l'ADR #23 « que du groupé, quel que soit le rôle ». Les écritures par-créneau (`POST`/`PATCH` `sensitivity-factors` & `carb-ratios` ; `POST`/`PATCH`/`DELETE` `pump-slots`) sont **supprimées** — un édit unitaire ne supersédait pas une `SlotSetProposal` pending (fenêtre de « dérive de base » : un ajustement médecin récent, ex. post-hypo, écrasé par l'acceptation d'un jeu périmé). Unique voie d'écriture = remplacement **GROUPÉ** (`PUT` → `replaceSlotSet` ISF/ICR + **nouveau** `replacePumpSlotSet` basal), qui supersède les propositions `pending`. Méthodes service par-créneau conservées (chemin gouverné `auto-apply`). Rupture de contrat API → coordination iOS `swift-expert`. |
| 27 | **Provenance `AdjustmentProposal` (union taguée `ProposalSource`) + cap patient PAR TYPE** (US-2652) | La provenance d'une proposition est dérivée **serveur** (`source` ∈ `algorithm`/`patient`/`nurse`/`doctor`, auteur du token de session, jamais du body) — anti-usurpation. Le cap PATIENT est **par type d'opération** (`min(%, absolu)`, ISF/ICR/basale/dose-fixe basal|bolus) et non plus un `%` global : un `%` seul est mal comporté aux extrêmes (10 % = saut énorme sur grosse basale). `kind` dose fixe résolu via `PatientInsulin.usage` ; pédiatrie via ConfigVersion active ; direction (baisse interdite famille basale) via `deriveRiskDirection`. Invariant `auto-apply ≤ patient ≤ moteur` préservé. Étend ADR #21 : `<PatientRecord>` = 1 composant présentational, **N transports** (page `?patientId` / drawer `cTok` / **own-id patient**). |
