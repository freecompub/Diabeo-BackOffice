...
| 32 | **Normalisation des unités glycémiques : g/L canonique** (épic unités CGM/BGM) | Le stockage **et** le contrat API des mesures de glycémie CGM/BGM sont normalisés sur **une unité canonique unique : g/L** (`1 g/L = 100 mg/dL`, jamais ×18). **Inverse la décision initiale « mg/dL canonique » (2026-07-08)** : l'app iOS n'existant pas encore, le seul coût qui justifiait mg/dL (rupture contrat + coordination Swift) tombe à zéro, et le cœur du code (algorithme, stockage CGM, statistiques, objectifs, ~30 seuils cliniques, **source MyDiabby**) est **déjà en g/L**. **Le vrai défaut corrigé n'est pas l'unité mais le DOUBLE-stockage BGM** (`glycemia_gl` **+** `glycemia_mgdl`, `POST` acceptant l'un OU l'autre sans dérivation ni garde de cohérence → *fail-open*). **Décisions** : (a) **BGM une seule colonne** `glycemia_gl` **nullable** (glycémie optionnelle) + `CHECK (glycemia_gl IS NULL OR 0.20–6.00)` (backfill `COALESCE(glycemia_gl, glycemia_mgdl/100)` puis **`DROP glycemia_mgdl`**) ; (b) **CGM inchangé** (`value_gl`) + `CHECK value_gl BETWEEN 0.20 AND 6.00` **posé en migration versionnée** (sort le drift de `cgm_partitioning.sql`) ; (c) **module de conversion unique** `src/lib/glucose/units.ts` (pur, sans dép Prisma → client+serveur ; `glToMgdl`/`mgdlToGl`/`mgdlToMmoll`/`formatGlucose`) remplaçant les 6 conversions dupliquées ; (d) **API g/L** (sortie CGM `valueGl` **inchangée** ; `POST/GET …/glycemia` **g/L-only**) ; (e) **affichage à la présentation** selon `UserUnitPreferences.unitGlycemia` (3=g/L, 4=mg/dL, 5=mmol/L) via `formatGlucose()` — fin des `round(valueGl*100)` dupliqués ; (f) **logique clinique et seuils INCHANGÉS**. Îlots mg/dL assumés : `GlucoseTarget.targetGlucose`, `DiabetesEvent`, `EmergencyAlert`, décision `emergency.service` (converti à la frontière). `DROP COLUMN` **destructif** acceptable (backoffice **pas encore en production**, cf. ADR #31). **Aucune rupture de contrat, aucune migration de la table CGM partitionnée.** Détail : `docs/architecture/adr-normalisation-unites-glycemie.md`. Slices S0 (module+refactor) → S1 (CHECK+backfill) → S2 (API/services g/L-only) → S3 (affichage préférence) → S4 (DROP+export). |
...
*Dernière révision : 2026-07-14 — ADR #32 (normalisation unités glycémie : **g/L canonique** stockage + API ; inverse la cible mg/dL du 2026-07-08 ; corrige le double-stockage BGM ; module de conversion unique `glucose/units.ts` ; affichage par préférence patient). Historique des PRs : `git log`.*