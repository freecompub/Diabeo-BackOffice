...
*Dernière mise à jour : 2026-05-15 — Groupe 10 Batch D Shares+Messaging livré (PR #405, 4 US US-2239/2240/2242/2261, ~16 SP, plan a). Migration mineure : `ALTER TYPE config_version_type ADD VALUE third_party_share, shared_notifications` (idempotent). 3 services : `share-config.service.ts` (thirdPartyShare partages école/EHPAD + sharedNotifications matrice alertType×caregivers via ConfigVersion hub PR #396), `share-audit.service.ts` (pure query AuditLog SQL OR-kinds + scanned/truncated metadata pour forensique HDS), `scheduled-messages.service.ts` (wrapper PushScheduledNotification : schedule one-shot + cancel cross-tenant guard + cancel.notFound audit anti brute-force). 6 routes `/api/patients/[id]/{third-party-share,shared-notifications,share-audit,scheduled-messages[/notifId]}` NURSE+/DOCTOR + canAccessPatient + requireGdprConsent. **Refactor `patient-modes.service.ts`** : `SUPPORTED_MODE_TYPES` renommé `SUPPORTED_VERSIONED_CONFIG_TYPES` (étendu à 5 types : 3 modes + 2 shares), `patientModeWorkflow` kind audit dérivé du configType (`${configType}.validate/deactivate/history`) — sémantique forensique correcte post-Batch D. `createConfigVersion` exporté + runtime guard `extraCreate` pediatric_mode-only (M3). 2 rounds review appliqués (12/13 findings : H1 SQL filter OR-kinds + limit 500 + scanned/truncated metadata, H2 kind rename + SUPPORTED_VERSIONED_CONFIG_TYPES, M1 expiresAt end-of-day Europe/Paris via Intl longOffset DST-aware, M2 cancel.notFound audit row, M3 runtime guard extraCreate, M4 audit metadata enrichi, M5 z.object strict 7 alertTypes + caregiverId FK check User.status=active + reject unknown IDs, M6 scheduledAt cap +365j + templateVariables 4KB JSON stringified). AuditResource enum +1 : `PUSH_SCHEDULED_NOTIFICATION`. V1 67 → 71 DONE (50%). Total 139/292 (48%). 1675/1675 tests verts. ⚠️ V2 deferrals : `OrchestrationLog` table (idempotence dedup), retry exponential backoff push, patient opt-out par type, telemetry Sentry sur snapshot.invalid failures, UI patient detail tabs intégration.*
