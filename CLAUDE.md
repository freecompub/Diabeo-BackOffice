...
*Dernière mise à jour : 2026-05-14 — Groupe 10 Batch C livré (PR #396 : US-2233/2234/2235, 16 SP) + hotfix re-review (PR #397). Modes spéciaux pédiatrique/Ramadan/voyage via hub ConfigVersion (enum étendu pediatric_mode/ramadan_mode/travel_mode). PediatricCaregiver PHI (name/phone) AES-256-GCM, snapshot redacted hasName/hasPhone. permissionLevel ∈ {read, write, propose} (HAS pédiatrique / ISPAD 2022) — "propose" crée AdjustmentProposal pending, JAMAIS de mutation directe. Ramadan : durée stricte 29-30j (lunaire), sahur→iftar window cohérent avec allowedFastingHours ±1.5h, warning extendedFasting > 16h (IDF-DAR 2021 §6.4). Voyage : basalMultiplier 0.7..1.3 + warning > ±20% (ATTD/EASD 2022), basalDelayHours 0..12h + warning > 8h, computeBasalProtocol retourne protocole transitoire (24-48h, pas trip-long) avec requiresDoctorReview=true + reference="ATTD-EASD-2022". Zod parse sur configSnapshot read → snapshotInvalid graceful degradation. ⚠️ Configs informationnels uniquement (calculator integration deferred). PR #397 hotfix : CHECK constraint propose, Zod bounds lockstep, P2002 → 409, UNIQUE(version_id,rank), DRY createConfigVersion helper, signature unifiée { version, warnings }, ADR #18 resourceId. mapErrorToResponse map maintenant P2002 → 409 uniqueConflict en plus de P2034. 1596/1596 tests verts. V1 50 → 53 DONE (38%). Total 268 US.*
