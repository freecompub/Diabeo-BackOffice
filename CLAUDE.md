...
| 31 | **Proposition GROUPÉE intégrale : bascule du moteur + généralisation à la basale POMPE** (US-2663) | Le moteur d'ajustement émet désormais des **propositions GROUPÉES** (`SlotSetProposal`, disposition entière d'un levier) au lieu de N `AdjustmentProposal` par-valeur, **derrière des flags réversibles OFF par défaut** — un par famille de levier (`ENGINE_GROUPED_ISF_ICR` ; `ENGINE_GROUPED_PUMP`) pour un rollout/rollback indépendant. **Socle mutualisé** : CAS d'ensemble à l'acceptation **générique par clé** (`assertBaselineUnchangedBy` — ISF/ICR `startHour`, pompe `startTime`/`endTime`/`rate`), `createSetProposal` en **objet d'options**, `parseSlots`/`captureBaselineSlots`/validation **polymorphes** par forme. **Garde-fous de parité** (répliqués de la voie par-valeur) : CAS par créneau changé (magnitude périmée jamais injectée), garde direction `reason↔delta`, no-op, baseline injecté (**fenêtre TOCTOU fermée** : disposition et snapshot partagent une lecture ⇒ le CAS d'acceptation couvre les créneaux inchangés). **Discriminateur pompe⇄stylo par `configType` LIVE** (fail-closed `basalConfigNotPump` — jamais des U/h écrits sur une dose stylo). Décisions cliniques (analyzeurs/deadband/dé-escalade/cooldown/flags) **INCHANGÉES** — seule la voie d'écriture change. `replacePumpSlotSet` rétrofité (CAS + supersession `SlotSetProposal` basalRate — l'invariant « aucune SlotSetProposal basale » tombe). **Livré** : ISF/ICR (S3b-1, PR #742), basale POMPE (S3c, PR-A), **dose fixe — apply usage-aware fail-closed** (S3d PR1 : clé `(usage, moment)`, `replaceFixedDoseSet` résolution 0/1/>1, filtre insuline active, correction du bug par-valeur usage-blind ; moteur+UI en S3d PR2). **À suivre** : moteur dose fixe (abandon titration multi-doses/moment), basale STYLO (apply atomique US-2660, single/split), voie manuelle groupée (S4), retrait par-valeur + **contrat iOS** (S5, ruptures concentrées). Flag OFF ⇒ voie par-valeur inchangée (aucune rupture iOS avant S5). Parité imparfaite documentée : un créneau hors-borne rejette tout le levier ce run (fail-closed, à trancher produit avant flag ON). |
...
*Dernière révision : 2026-07-13 — ADR #31 (US-2663 : proposition groupée intégrale — bascule moteur ISF/ICR + généralisation basale POMPE). Historique des PRs : `git log`.*